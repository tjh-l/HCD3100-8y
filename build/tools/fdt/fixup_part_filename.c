...
static int fdtput_main(int argc, char *argv[])
{
	int opt;
...
int main(int argc, char *argv[])
{
	...
	while (1) {
		...
		c = getopt_long(argc, argv, "i:o:l:n:h", lopts, NULL);
	...
	if (!input || !output || !label || !newfilename) {
		print_usage(argv[0]);
		return EXIT_FAILURE;
	}
	...
	if (strncmp(input, output, max(strlen(input), strlen(output))))
		utilfdt_write(output, dtb);
	...
	do {
		...
		for (i = 1; i <= npart; i++) {
			...
			if (strncmp(partname, label, max(strlen(partname), strlen(label))))
				continue;
			fdtput_main(7, ((char *[]){ "fdtput", "-t", "s", output, "/hcrtos/sfspi/spi_nor_flash/partitions", strbuf, newfilename }));
		}
	} while (0);
	...
	do {
		...
		for (i = 1; i <= npart; i++) {
			...
			if (strncmp(partname, label, max(strlen(partname), strlen(label))))
				continue;
			fdtput_main(7, ((char *[]){ "fdtput", "-t", "s", output, "/hcrtos/sfspi/spi_nand_flash/partitions", strbuf, newfilename }));
		}
	} while (0);
	...
	do {
		...
		for (i = 1; i <= npart; i++) {
			...
			if (strncmp(partname, label, max(strlen(partname), strlen(label))))
				continue;
			fdtput_main(7, ((char *[]){ "fdtput", "-t", "s", output, "/hcrtos/nand/partitions", strbuf, newfilename }));
		}
	} while (0);
	...
	do {
		...
		for (i = 1; i <= npart; i++) {
			...
			if (strncmp(partname, label, max(strlen(partname), strlen(label))))
				continue;
			fdtput_main(7, ((char *[]){ "fdtput", "-t", "s", output, "/hcrtos/external_partitions", strbuf, newfilename }));
		}
	} while (0);
