...
static void print_usage(const char *prog)
{
	...
	puts("  -v --version      firmware version\n"
	     ...
	     "  -r --filpmode     the output flip mode\n"
	     "  -d --dtb          DTB binary\n"
...
int main(int argc, char *argv[])
{
	...
	while (1) {
		...
		c = getopt_long(argc, argv, "v:p:V:t:o:r:fd:", lopts, NULL);
	...
	if (dtb_path) {
		...
		np = fdt_get_node_offset_by_path("/hcrtos/i2so");
		if (np >= 0) {
			uint32_t vol = 0;
			if (!fdt_get_property_u_32_index(np, "volume", 0, &vol))
				sysdata.volume = vol;
		}
		...
		if (np >= 0) {
			uint32_t tvtype = 0;
			if (!fdt_get_property_u_32_index(np, "tvtype", 0, &tvtype))
				sysdata.tvtype = tvtype;
		}
		np = fdt_get_node_offset_by_path("/hcrtos/rotate");
		if (np >= 0) {
			uint32_t flipmode = 0;
			if (!fdt_get_property_u_32_index(np, "flipmode", 0, &flipmode))
				sysdata.flip_mode = flipmode;
		}
		np = fdt_get_node_offset_by_path("/hcrtos/persistentmem");
		if (np >= 0) {
			const char *str = NULL;
			if (!fdt_get_property_string_index(np, "fs-type", 0, &str)) {
				if (!strcmp(str, "yaffs2") || !strcmp(str, "littlefs"))
					is_fs = 1;
			}
		}
	}

	uint32_t lcd_pwm_backlight = 100;
	np = fdt_get_node_offset_by_path("/hcrtos/backlight");
	if (np >= 0) {
		fdt_get_property_u_32_index(np, "default-brightness-level", 0, &lcd_pwm_backlight);
	}
	sysdata.lcd_pwm_backlight = lcd_pwm_backlight;

	printf("genpersistentmem sysdata.firmware_version: %u\r\n", sysdata.firmware_version);
	printf("genpersistentmem sysdata.tvtype: %d\r\n", sysdata.tvtype);
	printf("genpersistentmem sysdata.volume: %d\r\n", sysdata.volume);
	printf("genpersistentmem sysdata.product_id: %s\r\n", sysdata.product_id);
	printf("genpersistentmem sysdata.lcd_pwm_backlight: %d\r\n", sysdata.lcd_pwm_backlight);
	printf("genpersistentmem sysdata.flip_mode: %d\r\n", sysdata.flip_mode);
	...
	printf("offset of sysdata.flip_mode %ld\n", offsetof(struct sysdata, flip_mode));
