...
static int node_create(void *buffer, uint32_t size, struct persistentmem_node_create *node)
{
	...
	if (len != sizeof(struct vnode)) {
		printf("Write error, len:%d, data_len:%ld\r\n", len, sizeof(struct vnode));
...
static void dump_sysdata(struct sysdata *sysdata)
{
	...
	if (sysdata->firmware_version & 0xff000000)
		printf("\t%u\r\n", sysdata->firmware_version);
	else
		printf("\tv%d.%d.%d, ", sysdata->firmware_version >> 16, (sysdata->firmware_version >> 8) & 0xff, sysdata->firmware_version & 0xff);
...
int main(int argc, char *argv[])
{
	...
	ret = fread(gdev->mtd, 1, mtd_size, fp);
	...
	node.buf = &sysdata;
	if (persistentmem_ioctl_internal(PERSISTENTMEM_IOCTL_NODE_GET, (unsigned long)&node) < 0) {
