...
#define image_info(image, fmt...) printf(fmt)
#define image_error(image, fmt...) printf(fmt)
#define image_debug(image, fmt...) printf(fmt)
...
struct image {
	...
	off_t last_offset;
	char *outbuf;
	unsigned long long outbuf_size;
	unsigned int gpt_align;
};
...
static int block_device_size(struct image *image, const char *blkdev,
		      unsigned long long *size);
...
int insert_image(struct image *image, struct image *sub,
		 ...
		 unsigned char byte);
static int insert_data(struct image *image, const void *data, const char *outfile,
...
static uint32_t crc32_next(const void *data, size_t len, uint32_t last_crc);
