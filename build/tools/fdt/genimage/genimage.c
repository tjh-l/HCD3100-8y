...
static int hdimage_insert_gpt(struct image *image, struct list_head *partitions)
{
	...
	list_for_each_entry(part, partitions, list) {
		if (!part->in_partition_table) {
			printf("not in partition table\r\n");
			continue;
		}

		printf("insert partition table for %s, size %llu\r\n", part->name, part->size);
...
static int hdimage_generate(struct image *image)
{
	...
	printf("prepare_image hd->file_size is %llu\r\n", hd->file_size);
	ret = prepare_image(image, GPT_SECTORS*512);
	if (ret < 0)
		return ret;
...
static int hdimage_setup(struct image *image)
{
	struct partition *part;
	...
	hd->align = image->gpt_align;
	hd->extended_partition = 0;
	disk_signature = NULL;
	table_type = "gpt";
	hd->gpt_location = 0;
	hd->gpt_no_backup = cfg_true;
	hd->fill = cfg_true;
	hd->disk_uuid = NULL;
	...
	if (image->size == 0) {
		image_error(image, "image size is not specified\n");
		return -EINVAL;
	}
	...
	list_for_each_entry(part, &image->partitions, list) {
		...
		if (part->image) {
			...
			if (part->offset + child->size > hd->file_size) {
				...
				printf("%s:%d hd->file_size is %llu\r\n", __func__, __LINE__, hd->file_size);
			}
		}
		else if (part->extended) {
			hd->file_size = part->offset - hd->align + 512;
			printf("%s:%d (extended)hd->file_size is %llu\r\n", __func__, __LINE__, hd->file_size);
		} else if (part->offset + part->size > hd->file_size) {
			hd->file_size = part->offset + part->size;
			printf("%s:%d hd->file_size is %llu\r\n", __func__, __LINE__, hd->file_size);
		}
	}
...
{
	printf("Usage: %s [-dsp]\n", prog);
	puts("  -d --device    mkgpt on device\n"
	     "  -a --align     alignment of gpt header, default 0x100000\n"
	     "  -s --size      size of device capacity\n"
	     "  -p --part      partition, e.g: \n"
	     "                       size@offset(name)      : 0xe0000@0x0(uboot)\n"
	     "                       -@offset(name)         : -@0x200000(root)\n"
	     "                       size(name)             : 1920k(u-boot)\n"
	     "                       -(name)                : -(u-boot)\n"
	     "  -h --help      print help usage\n");
}

int mkgpt(int argc, char *argv[])
{
	struct image *image = xzalloc(sizeof(*image));
	struct partition *part;
	int part_num = 0;
	int fd, rc = -1;
	...
	while (1) {
		...
		c = getopt_long(argc, argv, "d:s:p:a:h", lopts, NULL);
		...
		case 'p': {
			...
			if (delim) {
				...
				name_len = p - name;
				s = p + 1;
				part->name = xzalloc(name_len + 1);
				memcpy((void *)part->name, name, name_len);
			} else {
				name = xzalloc(128);
				snprintf(name, 128, "Partition_%03d", part_num);
				part->name = name;
			}

			list_add_tail(&part->list, &image->partitions);
			part->in_partition_table = cfg_true;
			printf("part size 0x%llx(%lld) name %s, autoresize %d\r\n", part->size, part->size, part->name, part->autoresize);
			part_num++;
			break;
		}
		case 's': {
			char *s = optarg;
			image->size = memparse(s, &s);
			break;
		}
		case 'a': {
			char *s = optarg;
			image->gpt_align = memparse(s, &s);
			break;
		}
		case 'h':
		default:
	...
	if (image->outfile == NULL) {
		print_usage(argv[0]);
		goto error_ret;
	}

	hdimage_setup(image);
	hdimage_generate(image);

	printf("outbuf_size is %llu\r\n", image->outbuf_size);
	fd = open_file(image, image->outfile, 0);
	if (fd < 0) {
		printf("open %s fail\r\n", image->outfile);
		goto error_ret;
	}
	lseek(fd, 0, SEEK_SET);
	if (write(fd, image->outbuf, image->outbuf_size) < image->outbuf_size) {
		printf("write %s fail\r\n", image->outfile);
		close(fd);
		goto error_ret;
	}
