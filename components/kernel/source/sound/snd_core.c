			 ...
			 struct snd_pcm_params *params)
{
	...
	list_for_each_entry_safe (dai, next, &platform->dais, head) {
		...
		ret = dai->driver->hw_params(dai, params);
		if (ret) {
			if (dai->driver->ioctl) {
...
static int snd_free_record(struct snd_soc_platform *platform)
{
	return platform->driver->free_record(platform);
}
...
static int snd_ioctl(struct file *filep, int cmd, unsigned long arg)
{
	...
	case SND_IOCTL_FREE_RECORD: {
		ret = snd_free_record(platform);
		break;
	}
