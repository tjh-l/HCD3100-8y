...
{
#if (defined CONFIG_CPU_MIPS32R1) && (defined CONFIG_SOC_HC16XX) && !(defined BR2_PACKAGE_APPS_AVP)
...
void __attribute__((weak)) ddr_access_priority_init(void)
{
	...
	*(volatile uint32_t *)0xb880100c |= (0xA << 24);
    ...
    //4K DE
    /**************************************************/
    *(volatile uint32_t *)0xb8801010 &= ~(0x1 << 26);
    *(volatile uint32_t *)0xb880107C &= ~(0xFF << 16);
    *(volatile uint32_t *)0xb880107C |= (0x10 << 16);
    *(volatile uint32_t *)0xb8801010 &= ~(0x3 << 4);
//    *(volatile uint32_t *)0xb8801010 |= (0x2 << 4);
    *(volatile uint32_t *)0xb8801010 |= (0x0 << 4);
    
    *(volatile uint32_t *)0xb8801010 &= ~(0x1 << 28);
    *(volatile uint32_t *)0xb8801080 &= ~(0xFF);
    *(volatile uint32_t *)0xb8801080 |= (0x10);
    *(volatile uint32_t *)0xb8801010 &= ~(0x3 << 8);
    *(volatile uint32_t *)0xb8801010 |= (0x2<< 8);

    *(volatile uint32_t *)0xb8801010 &= ~(0x1 << 31);
    *(volatile uint32_t *)0xb8801080 &= ~(0xFF << 24);
    *(volatile uint32_t *)0xb8801080 |= (0x10 << 24);
    *(volatile uint32_t *)0xb8801010 &= ~(0x3 << 14);
    *(volatile uint32_t *)0xb8801010 |= (0x2 << 14);
    /**************************************************/
					...
					int depth, void *data)
{
	...
	char *st = (char *)fdt_get_property_data_by_name(node, "status", NULL);
	u32 i, nreg, base, offset, value;

	if (st!= NULL)
		if (strcmp(st, "disabled") == 0)
			return 0;
	...
	for (i = 0; i < nreg; i++) {
		...
		if (value == 1) {
			...
			void *caddr = (void *)MIPS_UNCACHED_ADDR(base);
			REG32_CLR_BIT(caddr, BIT(offset));
		}
	}
...
{
	REG32_SET_BIT(0xb8800094, BIT16);
}
...
#ifdef CONFIG_SOC_HC16XX
	REG32_SET_BIT(strap_pin_addr, BIT17); //disabled sflash cs/clk/mosi/miso pin
