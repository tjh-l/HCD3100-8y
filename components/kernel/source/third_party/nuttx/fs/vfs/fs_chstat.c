                  ...
                  FAR struct stat *buf, int flags, int resolve)
{
...
#if 0 // hcRTOS not support UID/GID
  ...
  if (gmtime_r((FAR const time_t *)&(tms.tv_sec), &tm0) == NULL)
  {
	return -1;
  }

  if (flags & (CH_STAT_ATIME | CH_STAT_MTIME))
  	buf->st_ctime = mktime(&tm0);

  if (flags & CH_STAT_ATIME)
	{
	  if (buf->st_atime == UTIME_OMIT)
	    {
	      flags &= ~CH_STAT_ATIME;
	    }
	  else if (buf->st_atime == UTIME_NOW)
	    {
	      buf->st_atime = buf->st_ctime; //update time sync from change time
	    }
	  #if 0 // hcRTOS not support UID/GID
	  else if (buf->st_atime >= 1000000000)
	    {
	      goto errout;
	    }
	  #endif
  ...
  if (flags & CH_STAT_MTIME)
    {
      if (buf->st_mtime == UTIME_OMIT)
        {
          flags &= ~CH_STAT_MTIME;
        }
      else if (buf->st_mtime == UTIME_NOW)
        {
          buf->st_mtime = buf->st_ctime;
        }
	  #if 0 //hcRTOS not support UID/GID
      else if (buf->st_mtime >= 1000000000)
        {
          goto errout;
        }
	  #endif
...
int utimes(FAR const char *path, const struct timeval times[2])
{
  ...
  struct tm tm1;
	...
	if (times != NULL)
	{
		if (gmtime_r((FAR const time_t *)&(times[0].tv_sec), &tm0) == NULL)
		{
		  return -1;
		}

		if (gmtime_r((FAR const time_t *)&(times[1].tv_sec), &tm1) == NULL)
		{
		  return -1;
		}
		buf.st_atime = mktime(&tm0);
		buf.st_mtime = mktime(&tm1);
	}
	else
	{
		buf.st_atime = UTIME_NOW;
		buf.st_mtime = UTIME_NOW;
	}
...
int chattr(FAR const char *path, char* attr)
{
  int flags = 0;
  if (!attr) {
  	return -EINVAL;
  ...
  if (!strcmp(attr,"-e")) {
  	flags = CH_STAT_CLRHIDE;
  }
  else if (!strcmp(attr,"+e")) {
	flags = CH_STAT_SETHIDE;
  }
  return chstat(path, NULL, flags, 1);
}

int lsattr(FAR const char *path, int *attr)
{ 
  int flags = 0;
 
  flags = CH_STAT_GETHIDE;
  return chstat(path, (struct stat *)attr, flags, 1);
}
