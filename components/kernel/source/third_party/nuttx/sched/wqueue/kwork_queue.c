...
static void __work_qqueue(FAR struct kwork_wqueue_s *wqueue,
                        FAR struct work_s *work, worker_t worker,
                        FAR void *arg, clock_t delay, int is_hrtimer)
{
  struct tms tms;
  ...
  work->delay  = delay;            /* Delay until work performed. It is usecond for High Resolution Timer work and msecond for other work */
  work->doing  = false;
  ...
  if (is_hrtimer)
    {
      work->qtime  = uxPortTimerHrTickGetLo(); /* Usecond Time work queued */
    }
  else
    {
...
{
  return __work_qqueue(wqueue, work, worker, arg, delay, 0);
}
  ...
  if (qid == HRTWORK)
    {
      ...
      __work_qqueue((FAR struct kwork_wqueue_s *)&g_hrtwork, work, worker,
                  arg, delay, 1);
      if (xPortIsInISR())
        {
        ...
        {
          xEventGroupSetBits(g_work_event, HRTWORK);
        }
      return OK;
    }
  else
