...
void work_process(FAR struct kwork_wqueue_s *wqueue, int wndx, const EventBits_t waitbit)
{
  ...
  if (waitbit == HRTWORK)
    {
      stick = uxPortTimerHrTickGetLo(); /* time in usecond work queued */
    }
  else
    {
      stick = times(&tms);
      ...
      if ((unsigned)next > 10)
        {
          if (next != WORK_DELAY_MAX)
            {
              work_hrtimer_start(next);
            }
          xEventGroupWaitBits(g_work_event, waitbit, pdTRUE, pdFALSE, WORK_DELAY_MAX);
        }
    }
  else
#endif
    {
      xEventGroupWaitBits(g_work_event, waitbit, pdTRUE, pdFALSE, next);
    }
