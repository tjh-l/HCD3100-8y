...
#define spareSize 16
static int chunkSize = (2048 - spareSize);
static int pagesPerBlock = 64;
...
// added NCB **CHECK**
	...
	yaffs_pack_tags2(&dummy_dev, &pt,&t,0);
...
int main(int argc, char *argv[])
{
	...
	if(argc < 3)
	{
		...
		printf("           chunckSize is the nand flash page size, default 2048\n");
		printf("           eraseSize  is the nand flash physical erase size, default 131072\n");
        printf("           'convert'  produce a big-endian image from a little-endian machine\n");
		exit(1);
	}

	if ((argc == 6) && (!strncmp(argv[5], "convert", strlen("convert"))))
	{
		convert_endian = 1;
	}
	...
	if (argc >= 4)
	{
		chunkSize = strtoul(argv[3], NULL, 0) - spareSize;
		printf("chunkSize %d\n", chunkSize + spareSize);
	}

	if (argc >= 5)
	{
		pagesPerBlock = strtoul(argv[4], NULL, 0);
		pagesPerBlock = pagesPerBlock / (chunkSize + spareSize);
		printf("eraseSize 0x%x\n", pagesPerBlock * (chunkSize + spareSize));
	}
