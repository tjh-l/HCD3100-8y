...
static struct ifaddrs* ifaddrs_init(void)
{
	...
	ifa->ifa_addr = malloc(sizeof(struct sockaddr_storage));
	ifa->ifa_netmask = malloc(sizeof(struct sockaddr_storage));
	ifa->ifa_next = NULL;
...
int getifaddrs(struct ifaddrs **ifap)
{
	...
	ifconfig.ifc_buf = (char *)ifr;
	ifconfig.ifc_len = sizeof(ifr);
	if (ioctl(fd, SIOCGLIFCONF, &ifconfig) < 0)
		goto error;
	if (ifconfig.ifc_len % sizeof(struct ifreq)) {
		errno = EINVAL;
		goto error;
	...
	for (int i = 0; i < ifno; i++) {
		memcpy(ifa->ifa_name, ifr[i].ifr_name, IF_NAMESIZE);
		ifa->ifa_name[IF_NAMESIZE] = '\0';
		memcpy(ifa->ifa_addr, &ifr[i].ifr_addr, sizeof(struct sockaddr_in6));
		ifr[i].ifr_flags = 0;
		if (ioctl(fd, SIOCGIFFLAGS, &ifr[i]) < 0)
			goto error;
		ifa->ifa_flags = ifr[i].ifr_flags;// & ~IFF_LOOPBACK;

		struct sockaddr_in6 *sock_in;

		sock_in = (struct sockaddr_in6 *)ifa->ifa_netmask;
		sock_in->sin6_family = AF_INET6;
		memset(sock_in->sin6_addr.__in6_union.__s6_addr32, 0xff, 8);
