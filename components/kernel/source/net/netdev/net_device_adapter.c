...
static void hc_pbuf_free_custom(struct pbuf *p)
{
	...
	skb = ppc->skb;
	if (skb->flags & SKBUF_FLAG_IS_DRIVER_ALLOC) {
		ndev = skb->dev;
		memset(&req,0,sizeof(struct iwreq));
		req.u.data.pointer = (void *)ppc->skb;
		if (ndev && ndev->netdev_ops && ndev->netdev_ops->ndo_do_ioctl)
			ndev->netdev_ops->ndo_do_ioctl(ndev, (struct ifreq *)&req, SIOCSIWFREESKBBUF);
	} else {
...
static struct sk_buff *convert_pbuf_to_skb(struct net_device *ndev, struct pbuf *p)
{
	...
	memset(&req,0,sizeof(struct iwreq));
	req.u.data.pointer = (void *)p;
	if (ndev && ndev->netdev_ops && (ndev->netdev_ops->ndo_do_ioctl) &&
	        (ndev->netdev_ops->ndo_do_ioctl(ndev, (struct ifreq *)&req, SIOCGIWALLOCSKBBUF) == 0))
		skb = (struct sk_buff *)req.u.data.pointer;
	else
...
int __attribute__((weak))p2p_send_hook(void *netif, struct sk_buff **skb)
{
	(void)netif;
	(void)skb;
	return 0;
...
#if LWIP_MONITOR
	extern int netif_monitor_send_hook(struct sk_buff *skb);
	if(netif_monitor_send_hook(skb) == MONITOR_SKB_EAT)
		return;
#endif
