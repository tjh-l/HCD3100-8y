...
struct pool_head;
struct pool_head *gMM[MMZ_NR_POOL] = { NULL };
...
void free(void * ptr)
{
	if ((NULL == ptr) || (ptr >= gMM[MMZ_ID_SYSMEM]->top)) {
		return;
	}

#ifdef CONFIG_MEM_DEBUG_MONITOR
	check_free(gMM[MMZ_ID_SYSMEM], ptr);
#endif

	taskENTER_CRITICAL();
	__free(gMM[MMZ_ID_SYSMEM], ptr);
	taskEXIT_CRITICAL();
...
{
	void *ptr;
	struct pool_head *pool = gMM[MMZ_ID_SYSMEM];
...
{
	if (id < 0 || id >= MMZ_NR_POOL_MMZ || gMM[id] == NULL)
		return 0;
...
int OsKHeapInit(void)
{
	...
	gMM[MMZ_ID_SYSMEM] = pool_init(start, size, blklog);
...
size_t mmz_available(int id)
{
	...
	struct pool_head *pool = gMM[id];
	return pool->bytes_free + (size_t)(pool->top - pool->ptr);
}
...
static int mem_free(int argc, char *argv[])
{
	...
	printf("system_mem_total: %lu\n", system_mem_total());
	printf("system_mem_free: %lu\n", system_mem_free());
	printf("mem_total: %lu\n\n", mem_total());
	printf        ("Code size info:\n");
	printf        (" Range           %p  -  %p\n", (void *)&__RAM_BASE, (void *)&_ebss);
	print_sizeinfo(" Code size       ", 0, size);
	print_sizeinfo("   overlay size  ", (size_t)&__RAM_BASE, (size_t)&__text_start);
	...
	print_sizeinfo("   all-data size ", (size_t)&__rodata_end, (size_t)&__bss_end);
...
static void out_sizeinfo(const char *tag, size_t start, size_t end, char *str)
{
	size_t size = end - start;
	sprintf(str, "%s   %5.02f MB\n", tag, (float)size / MB);
}

static void out_mem_pool(struct pool_head *pool, char *str)
{
	out_sizeinfo(" Used      ", 0, (size_t)pool->bytes_used, str);
	out_sizeinfo(" Free      ", 0, pool->bytes_free + (size_t)(pool->top - pool->ptr),str + strlen(str));
}

#define OUTPUT_LENGTH	1024
//output a string of the simple memory free status
char *mem_stats(void)
{
	int i;
	static char  *m_memstr = NULL;
	if (NULL == m_memstr) {
		m_memstr = (char*)malloc(OUTPUT_LENGTH);
	}
	memset(m_memstr, 0, OUTPUT_LENGTH);

	strcpy(m_memstr, "System Heap info:\n");
	out_mem_pool(gMM[MMZ_ID_SYSMEM], m_memstr + strlen(m_memstr));
	for (i = 0; i < MMZ_NR_POOL_MMZ; i++) {
		if (gMM[i] == NULL)
			continue;

		sprintf(m_memstr, "%sMMZ(%d) Heap info:\n", m_memstr, i);
		out_mem_pool(gMM[i], m_memstr + strlen(m_memstr));
	}
...
{
	return gMM[MMZ_ID_SYSMEM]->size;
}

size_t system_mem_free(void)
{
	return gMM[MMZ_ID_SYSMEM]->bytes_free + (size_t)(gMM[MMZ_ID_SYSMEM]->top - gMM[MMZ_ID_SYSMEM]->ptr);
}
...
size_t mem_total(void)
{
	...
	np = fdt_get_node_offset_by_path("/hcrtos/memory-mapping/totalmem");
	if (np < 0)
		return 0;

	ret = fdt_get_property_u_32_index(np, "reg", 0, &start);
	if (ret != 0) {
		return 0;
	}

	ret = fdt_get_property_u_32_index(np, "reg", 1, &size);
	if (ret != 0) {
		return 0;
	}

	total_size = (size_t)(start + size);;

	return total_size;
}

static int system_mem_test(int argc, char *argv[])
{
    printf("system_mem_total: %lu\n", system_mem_total());
    printf("system_mem_free: %lu\n", system_mem_free());
    printf("mem_total: %lu\n", mem_total());
...
static int print_nodes(void *ptr, struct node_info *n, void *usrdata)
{
	if (n->magic != NODE_MAGIC || check_magic(ptr + n->size, (void *)n)) {
		...
		if (n->magic == NODE_MAGIC) {
			printf("%p\tBut node magic is correct, node %p\n", ptr, (void *)n);
#ifdef CONFIG_MEM_DEBUG_RECORD_TASKID
			printf("%p\t%ld\t%d\tra: ", (void *)ptr, n->size, n->taskid);
#else
			printf("%p\tra: ", (void *)ptr);
#endif
			print_ra(n->lr, CONFIG_MEM_DEBUG_LR_CNT);
...
static int scan_pools(node_func_t func, void *usrdata)
{
	...
	scan_pool(gMM[MMZ_ID_SYSMEM], func, usrdata);
