...
static void *early_get_fdt(void)
{
#ifdef CONFIG_OF_EMBED
...
void *get_fdt(void)
{
	...
	return early_get_fdt();
}

void set_fdt(void *dt)
{
	fdtp = dt;
}
...
void fdt_setup(void)
{
	...
	if (!fdtp)
		asm volatile("nop;.word 0x1000ffff;nop;");
	memcpy(fdtp, dt, size);
	REG32_WRITE(0xb8800004, (uint32_t)fdtp);
#endif
}
...
int fdt_get_subnode_offset(int parentoffset, const char *name)
{
	return fdt_subnode_offset(fdtp, parentoffset, name);
}
...
int fdt_set_property_u32_index(void *fdt, int nodeoffset, const char *name,
				  int index, uint32_t val)
{
	return fdt_setprop_u32_index(fdt, nodeoffset, name, index, val);
}
