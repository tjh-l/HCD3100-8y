...
VOID ms7200drv_dvout_mode_config(DVOUT_CONFIG_T *t_dvout_config)
{
    ...
    switch(t_dvout_config->u8_sy_mode)
    {
        ...
        case DVOUT_SY_MODE_BTAT1004:
            ...
            break;
        case DVOUT_SY_MODE_EMBEDDED:
            HAL_ModBits(REG_DVO_CTRL_2, MSRT_BITS4_3, MSRT_BIT3);
            break;
        case DVOUT_SY_MODE_2XEMBEDDED:
...
VOID ms7200drv_dvout_data_uv_flip(VOID)
{
    UINT8 u8_swap = HAL_ReadByte(REG_DVO_CTRL_2) & MSRT_BIT5;
    HAL_ToggleBits(REG_DVO_CTRL_2, MSRT_BIT5 , !u8_swap);
}
