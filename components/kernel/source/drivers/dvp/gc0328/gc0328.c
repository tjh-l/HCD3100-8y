...
#include "../dvp_i2c.h"
#include "gc0328_reg.h"

struct gc0328_chrdev
{
...
static void gc0328_dts_init(struct gc0328_chrdev *chrdev)
{
    int np;

    np = fdt_get_node_offset_by_path("/hcrtos/dvp-gc0328");
...
static int gc0328_read_reg(uint8_t cmd , uint8_t *p_data , uint32_t len)
{
    int ret = 0;
...
static int gc0328_write_reg(uint8_t cmd , uint8_t *p_data , uint32_t len)
{
    int ret = 0;
...
static int gc0328_write_reg_byte(uint8_t cmd , uint8_t data)
{
    int ret = 0;
    uint8_t array[2];
...
static int gc0328_init_config(struct regval_list *reg_list , int len)
{
    ...
    for (i = 0;i < len / 2;i++)
    {
        ret = gc0328_write_reg(p_reg_list[i].reg_num , &(p_reg_list[i].value) , 1);
    }
    printf("exit %s\n" , __FUNCTION__);
...
static void gc0328_hw_reset(struct gc0328_chrdev *chrdev)
{
    if (chrdev->reset_gpio != 0xffffffff)
    {
...
{
    struct gc0328_chrdev *chrdev = NULL;

    chrdev = malloc(sizeof(struct gc0328_chrdev));
    memset(chrdev , 0 , sizeof(struct gc0328_chrdev));
    file->f_priv = chrdev;

    gc0328_dts_init(chrdev);
    gc0328_hw_reset(chrdev);
...
{
    struct gc0328_chrdev *chrdev = file->f_priv;
    ...
    file->f_priv = NULL;
    printk("gc0328_close end\n");

    return 0;
}

static int gc0328_ioctl(struct file *file , int cmd , unsigned long arg)
{
    int ret = -1;
    ...
    switch (cmd)
    {
        ...
        {
            ret = 0;
            gc0328_init_config(module_init_regs , sizeof(module_init_regs));
            break;
        }
...
static const struct file_operations gc0328_fops = {
    .open = gc0328_open,
    .close = gc0328_close,
    .read = dummy_read,
    .write = dummy_write,
...
static int gc0328_init(void)
{
    ...
    np = fdt_node_probe_by_path("/hcrtos/dvp-gc0328");
    if (np < 0)
    {
    ...
    register_driver("/dev/dvp-gc0328" , &gc0328_fops , 0666 , NULL);

    return 0;
}

module_driver(gc0328 , gc0328_init , NULL , 0)
