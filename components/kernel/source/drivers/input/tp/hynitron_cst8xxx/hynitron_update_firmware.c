...
int cst8xx_boot_update_fw(struct hynitron_ts_data *mclient)
{
		...
		if ((hyn_ts_data->fw_length) > 10) {
			...
			if (update_fw_flag || chip_sumok_flag == 0) {
				...
				if (chipchecksum != checksum) {
					cst78xx_update(startAddr, length,
						       (hyn_ts_data->p_hynitron_upgrade_firmware + 6));
...
static int cst2xx_enter_download_mode (void)
{
    ...
    buf[0] = 0xD1;
    buf[1] = 0x11;
    ret = cst3xx_i2c_write(hyn_ts_data, buf, 2);
    if (ret < 0) {
         return -1;
    }
...
static int cst2xx_download_program (unsigned char* data, int len)
{
    ...
    for (i = 0; i < (len / 1024); i++) {
	    ...
	    ret = cst3xx_i2c_write(hyn_ts_data, i2c_buf, 8);
	    if (ret < 0) {
...
#else
	ret = cst3xx_i2c_write(hyn_ts_data, pData, 1024 + 4);
	if (ret < 0) {
            HYN_ERROR ("program failed.\n");
            goto ERR_OUT;
        }
    ...
    pData[6] = 0x00;
    pData[7] = 0x00;
    ret = cst3xx_i2c_write(hyn_ts_data, pData, 8);
    if (ret < 0) {
    ...
    ret = cst3xx_i2c_write(hyn_ts_data, pData, 4);
    if (ret < 0) {
...
static int cst2xx_update_firmware(struct hynitron_ts_data *client, unsigned char *pdata, int data_len)
{
	int ret;
	int retry;
	retry = 0;
...
static int cst2xx_boot_update_fw (struct hynitron_ts_data* client)
{
...
{	
	...
	for (i=0; i<total_kbyte; i++) {
		...
		ret = cst3xx_i2c_write(hyn_ts_data, i2c_buf, 4);
		if (ret < 0)
			goto error_out;
		...
		ret = cst3xx_i2c_write(hyn_ts_data, i2c_buf, 514);
		if (ret < 0)
			goto error_out;
		#else
		...
		for(j=0; j<128; j++) {
			...
			ret = cst3xx_i2c_write(hyn_ts_data, temp_buf, 6);
			if (ret < 0)
				goto error_out;		
		}
    	#endif
...
static int cst3xxse_update_firmware(struct hynitron_ts_data * client,unsigned char *pdata)
{
	int ret;
	int retry;
	unsigned char buf[4];
...
static int cst3xxse_boot_update_fw(struct hynitron_ts_data * client)
{
	int ret=-1;
...
static int cst6xx_check_checksum(struct hynitron_ts_data * client)
{
	int ret;
	int i;
	unsigned int  checksum;
...
static int cst6xx_into_program_mode(struct hynitron_ts_data * client)
{
	int ret;
	unsigned char buf[4];
...
static int cst6xx_exit_program_mode(struct hynitron_ts_data * client)
{
	int ret;
	unsigned char buf[3];
...
static int cst6xx_erase_program_area(struct hynitron_ts_data * client)
{
	int ret;
	unsigned char buf[3];
...
static int cst6xx_write_program_data(struct hynitron_ts_data * client,
		const unsigned char *pdata)
{
	int i, ret;
	unsigned char *i2c_buf;
...
static int cst6xx_update_firmware(struct hynitron_ts_data * client, const unsigned char *pdata)
{
	int ret;
	int retry_timer = 0;
	int retry = 0;
...
static int cst6xx_boot_update_fw(struct hynitron_ts_data *client)
{
	int ret;
...
static int cst644k_into_program_mode(struct hynitron_ts_data * client)
{
	int ret;
	unsigned char buf[4];
...
static int cst644k_write_program_data(struct hynitron_ts_data * client,
		const unsigned char *pdata)
{
	int i, ret;
	unsigned char *i2c_buf;
...
static int cst644k_check_checksum(struct hynitron_ts_data * client)
{
	int ret;
	int i;
	unsigned int  checksum;
...
static int cst644k_exit_program_mode(struct hynitron_ts_data * client)
{
	int ret;
	unsigned char buf[3];
...
static int cst644k_update_firmware(struct hynitron_ts_data * client, const unsigned char *pdata)
{
	int ret;
	int retry_timer = 0;
	int retry = 0;
...
static int cst644k_boot_update_fw(struct hynitron_ts_data *client)
{
	int ret;
...
static int cst3xx_check_checksum(struct hynitron_ts_data * client)
{
	int ret;
	int i;
	unsigned int  checksum;
...
static int cst3xx_into_program_mode(struct hynitron_ts_data * client)
{
	int ret;
	unsigned char buf[4];
...
static int cst3xx_exit_program_mode(struct hynitron_ts_data * client)
{
	int ret;
	unsigned char buf[3];
...
static int cst3xx_erase_program_area(struct hynitron_ts_data * client)
{
	int ret;
	unsigned char buf[3];
...
static int cst3xx_write_program_data(struct hynitron_ts_data * client,
		const unsigned char *pdata)
{
	int i, ret;
	unsigned char *i2c_buf;
...
static int cst3xx_update_firmware(struct hynitron_ts_data * client, const unsigned char *pdata)
{
	int ret;
	int retry_timer = 0;
	int retry = 0;
...
static int cst3xx_boot_update_fw(struct hynitron_ts_data *client)
{
	int ret;
...
static int cst3240_check_checksum(struct hynitron_ts_data * client)
{
	int ret;
	int i;
	unsigned int  checksum;
...
static int cst3240_into_program_mode(struct hynitron_ts_data * client)
{
	int ret;
	unsigned char buf[4];
...
static int cst3240_exit_program_mode(struct hynitron_ts_data * client)
{
	int ret;
	unsigned char buf[3];
...
static int cst3240_write_program_data(struct hynitron_ts_data * client,
		const unsigned char *pdata)
{
	int i, ret,k;
	unsigned char *i2c_buf;
...
static int cst3240_update_firmware(struct hynitron_ts_data * client, const unsigned char *pdata)
{
	int ret;
	int retry_timer = 0;
	int retry = 0;
...
static int cst3240_boot_update_fw(struct hynitron_ts_data *client)
{
	int ret = -1;
...
static int cst92xx_check_checksum(struct hynitron_ts_data * client)
{
	int ret;
	int i;
	unsigned int  checksum;
...
static int cst92xx_into_program_mode(struct hynitron_ts_data * client)
{
	int ret;
	unsigned char buf[4];
...
static int cst92xx_exit_program_mode(struct hynitron_ts_data * client)
{
	int ret;
	unsigned char buf[3];
...
static int cst92xx_erase_flash_data(struct hynitron_ts_data * client)
{
	int ret = -1;
	unsigned char i2c_buf[8];
...
static int cst92xx_write_program_data(struct hynitron_ts_data * client,
		const unsigned char *pdata)
{
	int i, ret,k;
	unsigned char *i2c_buf;
...
static int cst92xx_update_firmware(struct hynitron_ts_data * client, const unsigned char *pdata)
{
	int ret;
	int retry_timer = 0;
	int retry = 0;
...
static int cst92xx_boot_update_fw(struct hynitron_ts_data *client)
{
	int ret;
...
int cst8xx_firmware_info(struct hynitron_ts_data *mclient)
{
	int ret = -1;
	...
	ret = hyn_i2c_read(hyn_ts_data, reg_buf, 1, buf, 2);

	if (ret < 0) {
		dev_err("%s: i2c read error.\n", __func__);
		return -1;
	}
...
int hyn_boot_update_fw(struct hynitron_ts_data * client)
{
	...
	if (hyn_ts_data->apk_upgrade_flag == 0) {
		if (hyn_ts_data->chip_ic_checkcode == 0xffffffff) {
			...
			if (proj_id < 0) {
				HYN_ERROR(
					"hyn not find config chip type firmware,please check config chip type or firmware.\n");
			...
			if (proj_id < 0) {
				HYN_ERROR(
					"not find matched TP firmware,please check chip type and chip project id. !\n");
				proj_id = hyn_find_fw_idx(0);
				if (proj_id > 0) {
					hyn_ts_data->fw_name = hynitron_fw_grp[proj_id].name;
					hyn_ts_data->p_hynitron_upgrade_firmware = hynitron_fw_grp[proj_id].fw;
...
static int cst0xx_bootloader_enter(struct hynitron_ts_data *client)
{
	HYN_FUNC_ENTER();
	HYN_FUNC_EXIT();
	return 0;
...
static int cst2xx_bootloader_enter(struct hynitron_ts_data *client)
{
	...
	for(retry=0;retry<5;retry++){
		...
		if(client->chip_ic_bootloader_addr!=client->addr){
			client->addr=client->chip_ic_bootloader_addr;
		}	
...
static int cst3xx_bootloader_enter(struct hynitron_ts_data *client)
{
	int ret=-1;
	u8  retry;
	unsigned char buf[4];
    ...
    for(retry=0;retry<10;retry++){
		...
		if (ret < 0)  continue;
 		...
 		if(client->config_chip_series==HYN_CHIP_CST3XX){
			if (buf[0] == 0xAC) break;
...
static int cst3240_bootloader_enter(struct hynitron_ts_data * client)
{
	int ret=-1;
	u8  retry;
	unsigned char buf[4];
...
static int cst92xx_bootloader_enter(struct hynitron_ts_data *client)
{
	int ret=-1;
	u8  retry;
	unsigned char buf[4];
...
static int cst6xx_bootloader_enter(struct hynitron_ts_data *client)
{
	int ret=-1;
	u8  retry;
	unsigned char buf[4];
...
static int cst3xxse_bootloader_enter(struct hynitron_ts_data *client)
{
	int ret=-1;
	u8  retry;
	unsigned char buf[4];
...
static int cst8xx_bootloader_enter(struct hynitron_ts_data *client)
{
	char retryCnt = 10;
	HYN_FUNC_ENTER();
...
//201
static int cst644K_bootloader_enter(struct hynitron_ts_data *client)
{
	int ret=-1;
	u8  retry;
	unsigned char buf[4];
...
int hyn_detect_bootloader(struct hynitron_ts_data *client)
{
    ...
    switch(client->config_chip_series){
		case HYN_CHIP_CST0XX: 
	...
	if(client->chip_ic_main_addr!=client->addr){
		client->addr=client->chip_ic_main_addr;
	}
