...
void hc_clk_disable_all(void)
{
	...
	/* CVBS-ADC */
	REG32_SET_BIT(0xb8800180, BIT24);
	REG32_SET_BIT(0xb8800180, BIT25);
	REG32_SET_BIT(0xb8800180, BIT26);
	REG32_SET_BIT(0xb8800180, BIT27);
	REG32_SET_BIT(0xb8800180, BIT28);
	REG32_SET_BIT(0xb8800180, BIT29);
	REG32_SET_BIT(0xb8800180, BIT0);
	REG32_SET_BIT(0xb8800180, BIT1);
	...
	*((uint32_t *)CLOCK_GATE_REG_0) = 0x0e0f3f8e;
	val = 0x3fffffff;
	if (REG32_READ(0xb8818a00) & BIT30) {
		/* keep USB0 clock for hcprogrammer */
		val &= ~BIT24;
	}
	if (REG32_READ(0xb8818a00) & BIT31) {
		/* keep USB1 clock for hcprogrammer */
		val &= ~BIT25;
	}
	*((uint32_t *)CLOCK_GATE_REG_1) = val;
