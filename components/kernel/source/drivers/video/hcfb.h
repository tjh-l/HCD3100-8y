...
#define HCFB_BUFFER_SOURCE_STATIC_ALLOC 3
	...
	uint8_t clut_mode;
	int8_t filter_select; // manual select, -1 as inactive
	uint8_t scale_mode;
	uint8_t scale_dir;
	uint8_t scale_ep_on;
	uint16_t h_scale;
	...
	uint8_t *static_header;
	uint8_t use_static_header;
	uint32_t static_header_size;
	uint32_t static_header_offset;
	...
	uint8_t pre_multiply; // HW to multiply color with alpha
#define FILT_PREF_DEFAULT       0
#define FILT_PREF_AUTO          1
#define FILT_PREF_FORCE_BL      2
#define FILT_PREF_FORCE_4X3     3
	uint8_t filter_preference;
	uint8_t csc_bypass;
	size_t extra_fb_len;
	hcfb_mmap_cache_e mmap_cache;
	bool gma_onoff;
	uint8_t id;
	hcfb_viewport_t view_port;
};
