...
static void spi_nor_set_4byte_opcodes(struct spi_nor *nor)
{
	...
	nor->erase_opcode_se = spi_nor_convert_3to4_erase(nor->erase_opcode_se);
...
static int spi_nor_erase_sector_se(struct spi_nor *nor, u32 addr)
{
	int i;
	...
	if (nor->spimem) {
		struct spi_mem_op op =
			SPI_MEM_OP(SPI_MEM_OP_CMD(nor->erase_opcode_se, 0),
				   SPI_MEM_OP_ADDR(nor->addr_width, addr, 0),
				   SPI_MEM_OP_NO_DUMMY,
				   SPI_MEM_OP_NO_DATA);
	...
	return spi_nor_controller_ops_write_reg(nor, nor->erase_opcode_se,
						nor->bouncebuf, nor->addr_width);
}
...
static int spi_nor_erase(struct mtd_info *mtd, struct erase_info *instr)
{
	...
	div_u64_rem(instr->addr, mtd->erasesize_se, &rem1);
	div_u64_rem(instr->len, mtd->erasesize_se, &rem2);
	...
	/* "sector"-at-a-time erase */
		...
		if (!rem1 && !rem2) {
			while (len) {
				...
				ret = spi_nor_erase_sector_se(nor, addr);
				if (ret)
					goto erase_err;

				addr += mtd->erasesize_se;
				len -= mtd->erasesize_se;
...
static int spi_nor_select_erase(struct spi_nor *nor)
{
	...
	if (spi_nor_has_uniform_erase(nor)) {
		...
		erase = NULL;
		erase = spi_nor_select_uniform_erase(map, 65536u);
		if (!erase)
			return -EINVAL;
		nor->erase_opcode_se = erase->opcode;
		mtd->erasesize_se = erase->size;
		return 0;
	}
