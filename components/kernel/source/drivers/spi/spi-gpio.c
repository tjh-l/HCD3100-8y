...
static int spi_gpio_probe(const char *node)
{
	...
	nd = fdt_node_probe_by_path("/hcrtos/spi-gpio/spidev@2");
	if (!fdt_get_property_string_index(nd, "mutex-lock", 0, &mutex_status) &&
	      !strcmp(mutex_status, "sf_lock")) {
		sfspi_pinmux = fdt_get_property_pinmux(np_sfspi, "active");
		pinmux_select_setting(sfspi_pinmux);
	}
