...
int __spi_mem_exec_op2(struct spi_mem *mem, const struct spi_mem_op *op)
{
	...
	if (op->addr.nbytes) {
		for (int i = 0; i < op->addr.nbytes; i++)
			tmpbuf[op->addr.nbytes - i] = op->addr.val >> (8 * (i));

		x->n_addr = op->addr.nbytes;
		x->addr = tmpbuf + 1;
	}

	if (op->dummy.nbytes) {
		memset(tmpbuf + op->addr.nbytes + 1, 0xff, op->dummy.nbytes);
		x->dummy_data = op->dummy.nbytes;
	}

	if (op->data.nbytes) {
		if (op->data.dir == SPI_MEM_DATA_IN) {
			x->rx_buf = op->data.buf.in;
			x->rx_nbits = op->data.buswidth;
			x->rx_len = op->data.nbytes;
		} else {
			x->tx_buf = op->data.buf.out;
			x->tx_nbits = op->data.buswidth;
			x->tx_len = op->data.nbytes;
		}
	}
...
int __spi_mem_exec_op(struct spi_mem *mem, const struct spi_mem_op *op)
{
...
{
	if ((op->data.nbytes == 0 && op->addr.nbytes != 0) || op->cmd.opcode == SPINOR_OP_PP ||
	    op->cmd.opcode == SPINOR_OP_PP_1_1_4 || op->cmd.opcode == SPINOR_OP_PP_1_4_4 ||
	    op->cmd.opcode == SPINOR_OP_PP_4B || op->cmd.opcode == SPINOR_OP_PP_1_1_4_4B)
		return __spi_mem_exec_op(mem, op);
	else
		return __spi_mem_exec_op2(mem, op);
}
...
static int __spi_probe(const char *master, const char *node, uint8_t type)
{
	...
	fdt_get_property_u_32_index(np, "sclk", 0, &clk);
...
int spi_probe(const char *master, const char *node, uint8_t type)
{
	return __spi_probe(master, node, type);
}
...
static int spi_nor_init(void)
{
	...
	__spi_probe("hc-sf-spi", "/hcrtos/sfspi/spi_nor_flash1", MTD_NORFLASH);
#endif
