...
static inline u32 get_ptr_data(void *ptr, u8 bits)
{
	u8 step = 0;
	u32 word = 0;
	...
	if (((uint32_t)ptr % step) != 0) {
		for (int i = 0; i < step; i++)
			word |= *(uint8_t *)(ptr + i) << (8 * i);
	} else if (step == 4) {
		word = *(uint32_t *)(ptr);
	} else if (step == 2) {
		word = *(uint16_t *)(ptr);
...
static inline void set_ptr_data(void *ptr, u8 bits, u32 word)
{
	u8 step = 0;
...
static unsigned bitbang_txrx_16(struct	spi_device *spi,
	...
	while (likely(count > 1)) {
		...
		if (tx) {
			word = get_ptr_data((void *)tx, bits);
			tx++;
		}
		word = txrx_word(spi, ns, word, bits);

		if (rx) {
			set_ptr_data((void *)rx, bits, word);
			rx++;
		}
