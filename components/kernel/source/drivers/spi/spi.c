...
static void spi_set_cs(struct spi_device *spi, bool enable ,bool hw_transfers)
{
	if (hw_transfers)
		return;
				    ...
				    struct spi_message *msg)
{
	...
	list_for_each_entry (xfer, &msg->transfers, transfer_list) {
		if (xfer->tx_buf || xfer->rx_buf || xfer->addr || xfer->cmd) {
			if (cs_work == false) {
				spi_set_cs(msg->spi, true, xfer->hw_transfers);
				cs_work = true;
			}
		...
		if (xfer->cs_change) {
			if (list_is_last(&xfer->transfer_list,
				...
				spi_set_cs(msg->spi, false, xfer->hw_transfers);
				usleep(10);
				spi_set_cs(msg->spi, true, xfer->hw_transfers);
			}
		}
...
out:
	if (ret < 0 || !keep_cs) {
		if (cs_work == true) {
			if (last_xfer != NULL)
				spi_set_cs(msg->spi, false, last_xfer->hw_transfers);
			cs_work = false;
		}
	}
