...
struct lcd_map
{
	...
	int (*lcd_init)(void);
	int (*lcd_set_mode_info)(struct hc_lcd_mode_info *info);
	struct hc_lcd_mode_info* (*lcd_get_mode_info)(void);
	int (*lcd_set_init_sequence)(struct hc_lcd_init_sequence *info);
	int default_off_val;
};
