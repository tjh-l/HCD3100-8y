...
static int prepare_block(struct norflash_block *blk)
{
	...
	if (crc_data != 0xFFFFFFFF && ncrc_data == 0xFFFFFFFF) {
		log_d("norflash_next write complete, but erase not finished.\n"
	...
	if (crc_data == 0xFFFFFFFF && ncrc_data != 0xFFFFFFFF) {
		log_d("norflash write complete, but erase not finished.\n"
	...
	if (!blk_is_new && nblk_is_new) {
		...
		log_e("both norflash and norflash_next are broken, clean the blocks and continue!!!\n");
		erase_block(blk);
		erase_next_block(blk);
		assert(scan_block(blk) == NODE_END);
	}
...
static int node_get_size(void *buffer, uint32_t size, struct persistentmem_node *node)
{
	struct vnode *pvnode;
...
static int persistentmem_ioctl(struct file *filep, int cmd, unsigned long arg)
{
	...
	case PERSISTENTMEM_IOCTL_NODE_GETSIZE: {
		persistentmem_lock();
		if (gdev->valid)
			rc = node_get_size(gdev->blk.buffer, gdev->blk.buf_size, (struct persistentmem_node *)arg);
		else
			rc = -1;
		persistentmem_unlock();
		break;
