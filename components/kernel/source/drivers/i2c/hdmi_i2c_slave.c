...
static void hc_hdmi_i2cs_isr(uint32_t param)
{
	...
	if (reg->isr.stop_int == 1) {
		...
		if (reg->dev_control.slave_send_data_en == 1) {
            ...
            if (priv->index >= priv->write_buflen)
            {
                complete(&priv->transfer_complete);
            }
		}
...
struct i2c_slave_s *hc_hdmi_i2cs_initialize(int port, struct i2c_slave_s *param)
{
	int np;
	char name[128];
	struct hc_hdmi_i2cs_priv_s *priv = (struct hc_hdmi_i2cs_priv_s *)param;
	...
	priv->i2cs.ops = &hc_hdmi_i2cs_slaveops;
    ...
    init_completion(&priv->transfer_complete);
	priv->refs++;

  	return (struct i2c_slave_s *)priv;
}

int hc_hdmi_i2cs_deinit(struct i2c_slave_s *param)
{
	struct hc_hdmi_i2cs_priv_s *priv = (struct hc_hdmi_i2cs_priv_s *)param;
...
int hc_hdmi_i2cs_wait_for_complete(struct i2c_slave_s *param)
{
    struct hc_hdmi_i2cs_priv_s *priv = (struct hc_hdmi_i2cs_priv_s *)param;
    ...
    printf("WAIT I2C\n");
    if (wait_for_completion_timeout(&priv->transfer_complete , 1000) != 0)
    {
        printf("T I2C OK\n");
        return 0;
    }
