...
typedef struct vdec_transcode_data_info {
	int width;
	int height;
	img_transcode_format_e data_format;
} vdec_transcode_data_info_t;

typedef struct vdec_avsync_threshold {
	int hold_threshold; //!< unit: ms, valid range: > 0, invalid range: <=0
	int drop_threshold; //!< unit: ms, valid range: > 0, invalid range: <=0
	int hold2free_threshold; //!< unit: ms, valid range: > 0, invalid range: <=0
	int drop2free_threshold; //!< unit: ms, valid range: > 0, invalid range: <=0
} vdec_avsync_threshold_t;
...
#define VIDDEC_GET_TRANSCODE_INFO           _IOR(VIDEO_IOCBASE, 52, struct vdec_transcode_data_info)
...
#define VIDDEC_SET_AVSYNC_THRESHOLD         _IOW(VIDEO_IOCBASE, 107, struct vdec_avsync_threshold)
#define VIDDEC_GET_AVSYNC_THRESHOLD         _IOR(VIDEO_IOCBASE, 108, struct vdec_avsync_threshold)
#define VIDDEC_SET_SYNC_MODE                _IO (VIDEO_IOCBASE, 109) //!< param: enum AVSYNC_TYPE
