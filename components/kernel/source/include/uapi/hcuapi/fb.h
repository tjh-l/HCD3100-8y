...
#define HCFBIOGET_SCALE_ONOFF 		_IOR(HCFB_IOCBASE, 6, unsigned long)
#define HCFBIOSET_SET_VIEWPORT 		_IOW(HCFB_IOCBASE, 7, hcfb_viewport_t)
...
typedef struct hcfb_viewport {
	int x; /* allow negative */
	int y; /* allow negative */
	uint16_t width;
	uint16_t height;
	uint8_t fit_mode;  /* fit_mode: center only, for now */
	uint8_t apply_now; /* 0: not apply no, need to apply via FBIOPAN_DISPLAY; 1: apply immediately */
	uint8_t	enable; /* 0: disable viewport setting; 1: enable viewport setting */
} hcfb_viewport_t;
