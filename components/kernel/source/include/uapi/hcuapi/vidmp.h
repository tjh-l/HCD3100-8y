...
typedef enum VIDEO_PBP_MODE {
	VIDEO_PBP_OFF,
	VIDEO_PBP_2P_ON,	//!< Two pictures
} video_pbp_mode_e;
...
typedef enum IMG_TRANSCODE_MODE {
	/*
	 * FIT: 长宽保持比例，全部缩到边框内
	 * CENTER: 长宽保持比例，缩放到恰好能遮住显示窗，取正中间，裁掉多余部分
	 * STRETCH: 长宽不需要保持比例，尺寸进行缩放和拉伸以充满显示视窗
	 */
	IMG_TRANSCODE_MODE_FIT = 0,
	IMG_TRANSCODE_MODE_CENTER,
	IMG_TRANSCODE_MODE_STRETCH,
...
struct video_config {
	...
	video_pbp_mode_e pbp_mode; //!< pictrue by picture mode(or picture in picture mode)
	int8_t          stc_id;
	int             dis_type;  //!< dis_type_e
	int             dis_layer; //!< dis_layer_e
	int             auto_audio_master; //!< auto change to audio master if video delay has been setted
