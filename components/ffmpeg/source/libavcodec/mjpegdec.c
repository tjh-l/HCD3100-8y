...
static inline void thread_yield(TickType_t *prev_tick, int y)
{
	int cur_tick;
	if (y % 64 == 63) {
		cur_tick = xTaskGetTickCount();
		if ((cur_tick - *prev_tick) > PICTURE_DECODE_THREAD_YIELD_INTERVAL) {
			//av_log(NULL, AV_LOG_DEBUG, "previous_tick_count %d, current_tick_count %d\n",
			//	*prev_tick, cur_tick);
			*prev_tick = cur_tick;
			taskYIELD();
		}
...
static void auto_set_lowres(AVCodecContext *avctx) {
    int width = avctx->width;
    int height = avctx->height;

    if (avctx->lowres > 0) {
        avctx->vdec_knobs |= VDEC_KNOB_LOWRES_COMPACT;
        return;
    }
...
av_cold int ff_mjpeg_decode_init(AVCodecContext *avctx)
{
    ...
    s->avctx = avctx;
    auto_set_lowres(avctx);
    printf("auto set lowres=%d\n", avctx->lowres);
...
int ff_mjpeg_decode_sof(MJpegDecodeContext *s)
{
    ...
    unk_pixfmt:
        ...
        if (ff_get_buffer_override_pixfmt(s->avctx, s->picture_ptr, AV_GET_BUFFER_FLAG_REF,
                (s->avctx->vdec_knobs & VDEC_KNOB_DECODE_TO_NV12) ? AV_PIX_FMT_NV12 : AV_PIX_FMT_NONE) < 0)
            return -1;
        s->picture_ptr->pict_type = AV_PICTURE_TYPE_I;
        s->picture_ptr->key_frame = 1;
        s->got_picture            = 1;
    ...
    if (s->progressive) {
        ...
        int bwxbh = bw * bh;
        int compact = !!(s->avctx->vdec_knobs & VDEC_KNOB_LOWRES_COMPACT);
        s->block_size = compact ? 64 >> (s->avctx->lowres << 1) : 64;
        for (i = 0; i < s->nb_components; i++) {
            int size = bwxbh * s->h_count[i] * s->v_count[i];
            av_freep(&s->blocks[i]);
            av_freep(&s->ac_bitmap[i]);
            av_freep(&s->last_nnz[i]);
            s->blocks[i]       = av_mallocz_array(size, sizeof(**s->blocks) * s->block_size);
            if (s->avctx->lowres > 0 && compact)
                s->ac_bitmap[i] = av_mallocz_array(size, sizeof(**s->ac_bitmap));
...
int translate_permute(MJpegDecodeContext *s, int idx) {
    const int lowres = s->avctx->lowres;
    if (lowres == 0) {
        /* 8 x 8 */
        return idx;
    } else {
        const int dx = idx & 0x7;
        const int dy = idx >> 3;
        const int max = 8 >> lowres;
        if (dx < max && dy < max) {
            if (s->avctx->vdec_knobs & VDEC_KNOB_LOWRES_COMPACT) {
                return (dy << (3 - lowres)) | dx;
            } else {
                return idx;
            }
        } else {
...
static av_always_inline
void set_bit(uint64_t *ac_bitmap, int bit) {
    *ac_bitmap |= (uint64_t)1u << bit;
}

static av_always_inline
int test_bit(const uint64_t *ac_bitmap, int bit) {
    return (*ac_bitmap & ((uint64_t)1u << bit)) != 0;
}

static av_always_inline
int16_t *get_block(MJpegDecodeContext *s, int c, int block_idx) {
    return s->blocks[c] + s->block_size * block_idx;
}
            ...
            if (code) {
                ...
                if (i >= se) {
                    if (i == se) {
                        ...
                        if ((jp = translate_permute(s, j)) >= 0)
                            block[jp] = level * (quant_matrix[se] << Al);
                        if (ac_bitmap)
                            set_bit(ac_bitmap, j);
                        break;
                    }
                ...
                j = s->scantable.permutated[i];
                if ((jp = translate_permute(s, j)) >= 0)
                    block[jp] = level * (quant_matrix[i] << Al);
                if (ac_bitmap)
                    set_bit(ac_bitmap, j);
            } else {
...
for (; ; i++) {                                                     \
    ...
    j = s->scantable.permutated[i];                                 \
    if (ac_bitmap ? test_bit(ac_bitmap, j) : block[j])              \
        REFINE_BIT(j)                                               \
    else if (run-- == 0)                                            \
        break;                                                      \
}
...
static int decode_block_refinement(MJpegDecodeContext *s, int16_t *block,
                                   ...
                                   int ss, int se, int Al, int *EOBRUN)
{
    int code, i = ss, j, jp, sign, val, run;
    ...
    if (*EOBRUN) {
        ...
        for (; ; i++) {
            ...
            if (code & 0xF) {
                ...
                val--;
                if ((jp = translate_permute(s, j)) >= 0)
                    block[jp] = ((quant_matrix[i] << Al) ^ val) - val;
                if (ac_bitmap)
                    set_bit(ac_bitmap, j);
                if (i == se) {
                    if (i > *last_nnz)
...
static int ljpeg_decode_rgb_scan(MJpegDecodeContext *s, int nb_components, int predictor, int point_transform)
{
    ...
    int vpred[6];
#ifndef __linux__
    int prev_tick = xTaskGetTickCount();
#endif
                ...
                for (j = 0; j < n; j++) {
                    ...
                    if (!s->progressive) {
                        ...
                        int block_idx  = s->block_stride[c] * (v * mb_y + y) +
                                         (h * mb_x + x);
                        int16_t *block = get_block(s, c, block_idx);
                        if (Ah)
                                            ...
                                            int se, int Ah, int Al)
{
    ...
    for (mb_y = 0; mb_y < s->mb_height; mb_y++) {
        ...
        int16_t *block = get_block(s, c, block_idx);
        uint64_t *ac_bitmap = s->ac_bitmap[c] ? &s->ac_bitmap[c][block_idx] : NULL;
        uint8_t *last_nnz    = &s->last_nnz[c][block_idx];
        if (get_bits_left(&s->gb) <= 0) {
            av_log(s->avctx, AV_LOG_ERROR, "bitstream truncated in mjpeg_decode_scan_progressive_ac\n");
            return AVERROR_INVALIDDATA;
        }
        for (mb_x = 0; mb_x < s->mb_width; mb_x++, block+=s->block_size, last_nnz++, ac_bitmap+=!!ac_bitmap) {
                int ret;
                if (s->restart_interval && !s->restart_count)
                    s->restart_count = s->restart_interval;

                if (Ah)
                    ret = decode_block_refinement(s, block, ac_bitmap, last_nnz, s->ac_index[0],
                                                  quant_matrix, ss, se, Al, &EOBRUN);
                else
                    ret = decode_block_progressive(s, block, ac_bitmap, last_nnz, s->ac_index[0],
                                                   quant_matrix, ss, se, Al, &EOBRUN);
                if (ret < 0) {
                    av_log(s->avctx, AV_LOG_ERROR,
                           "error y=%d x=%d\n", mb_y, mb_x);
...
static void mjpeg_idct_scan_progressive_ac(MJpegDecodeContext *s)
{
    ...
    for (c = 0; c < s->nb_components; c++) {
        ...
        for (mb_y = 0; mb_y < mb_height; mb_y++) {
            ...
            int block_idx    = mb_y * s->block_stride[c];
            int16_t *block = get_block(s, c, block_idx);
            for (mb_x = 0; mb_x < mb_width; mb_x++, block+=s->block_size) {
...
static av_always_inline
void blit(int idct_sz, const uint8_t *block, int linesize, uint8_t *out) {
    for (int row = 0; row < idct_sz; ++row) {
        memcpy(out, block, idct_sz);
        block += idct_sz;
        out += linesize;
    }
}
...
static av_always_inline
void blit_uv(const int idct_sz, const uint8_t *block_u, const uint8_t *block_v, int linesize, uint8_t *out) {
    for (int row = 0; row < idct_sz; ++row) {
        const uint8_t *u = block_u;
        const uint8_t *v = block_v;
        uint8_t *t = out;
        for (int col = 0; col < idct_sz; ++col) {
            *t++ = *u++;
            *t++ = *v++;
        }
        block_u += idct_sz;
...
static av_always_inline
void blit_uv_422to420(const int idct_sz, const uint8_t *block_u, const uint8_t *block_v, int linesize, uint8_t *out) {
    for (int row = 0; row < (idct_sz >> 1); ++row) {
        const uint8_t *row_ly_u = block_u;
        const uint8_t *row_uy_u = block_u + idct_sz;
        const uint8_t *row_ly_v = block_v;
        const uint8_t *row_uy_v = block_v + idct_sz;
        uint8_t *t = out;
        for (int col = 0; col < idct_sz; ++col) {
            *t++ = (*row_ly_u++ + *row_uy_u++) >> 1;
            *t++ = (*row_ly_v++ + *row_uy_v++) >> 1;
        }
        block_u += (idct_sz << 1);
        block_v += (idct_sz << 1);
        out += linesize;
...
static av_always_inline
void blit_uv_422vto420(const int idct_sz, const uint8_t *block_u, const uint8_t *block_v, int linesize, uint8_t *out) {
    for (int row = 0; row < idct_sz; ++row) {
        const uint8_t *row_u = block_u;
        const uint8_t *row_v = block_v;
        uint8_t *t = out;
        for (int col = 0; col < (idct_sz >> 1); ++col) {
            *t++ = (row_u[0] + row_u[1]) >> 1;
            *t++ = (row_v[0] + row_v[1]) >> 1;
            row_u += 2;
            row_v += 2;
...
static av_always_inline
void blit_uv_444to420(const int idct_sz, const uint8_t *block_u, const uint8_t *block_v, int linesize, uint8_t *out) {
    for (int row = 0; row < (idct_sz >> 1); ++row) {
        const uint8_t *row_ly_u = block_u;
        const uint8_t *row_uy_u = block_u + idct_sz;
        const uint8_t *row_ly_v = block_v;
        ...
        uint8_t *t = out;
        for (int col = 0; col < (idct_sz >> 1); ++col) {
            *t++ = (row_ly_u[0] + row_ly_u[1] + row_uy_u[0] + row_uy_u[1]) >> 2;
            *t++ = (row_ly_v[0] + row_ly_v[1] + row_uy_v[0] + row_uy_v[1]) >> 2;
            row_ly_u += 2;
            row_uy_u += 2;
            row_ly_v += 2;
            row_uy_v += 2;
...
static
void idct_prog_ac(MJpegDecodeContext *s, int mb_width, int mb_height,
                  int linesize, uint8_t *data) {
    int mb_x, mb_y;
    const int bytes_per_pixel = 1 + (s->bits > 8);
    const int idct_sz = 8 >> s->avctx->lowres;
    for (mb_y = 0; mb_y < mb_height; mb_y++) {
        uint8_t *ptr     = data + (mb_y * linesize * 8 >> s->avctx->lowres);
        int block_idx    = mb_y * s->block_stride[0];
        int16_t *block = get_block(s, 0, block_idx);
        for (mb_x = 0; mb_x < mb_width; mb_x++, block+=s->block_size) {
            // s->idsp.idct_put(ptr, linesize, *block);
            s->idsp.idct_put(s->idct_block, idct_sz, block);
            if (s->bits & 7)
                shift_output(s, ptr, linesize);
            blit(idct_sz, s->idct_block, linesize, ptr);
            ptr += bytes_per_pixel*8 >> s->avctx->lowres;
        }
    }
}
...
static
void idct_prog_ac_uv(MJpegDecodeContext *s, int mb_width, int mb_height,
                     int linesize, uint8_t *data) {
    int mb_x, mb_y;
    const int bytes_per_pixel = 1 + (s->bits > 8);
    const int idct_sz = 8 >> s->avctx->lowres;
    const int h_deci = s->h_max / s->h_count[1];
    const int v_deci = s->v_max / s->v_count[1];
    const int out_uv_count = (h_deci == 1 ? (idct_sz >> 1) : idct_sz) << 1;
    void (*blit_func)(const int, const uint8_t*, const uint8_t*, int, uint8_t*);
    if (h_deci == 2 && v_deci == 2) {
        blit_func = blit_uv;
    } else if (h_deci == 2 && v_deci == 1) {
        av_assert0(s->avctx->lowres < 3); // FIXME
        blit_func = blit_uv_422to420;
    } else if (h_deci == 1 && v_deci == 2) {
        av_assert0(s->avctx->lowres < 3); // FIXME
        blit_func = blit_uv_422vto420;
    } else {
        av_assert0(s->avctx->lowres < 3); // FIXME
        blit_func = blit_uv_444to420;
    }
    for (mb_y = 0; mb_y < mb_height; mb_y++) {
        uint8_t *ptr     = data + mb_y * linesize * (v_deci == 1 ? (idct_sz >> 1) : idct_sz);
        int block_idx_u  = mb_y * s->block_stride[1];
        int block_idx_v  = mb_y * s->block_stride[2];
        int16_t *block_u = get_block(s, 1, block_idx_u);
        int16_t *block_v = get_block(s, 2, block_idx_v);
        for (mb_x = 0; mb_x < mb_width; mb_x++, block_u+=s->block_size, block_v+=s->block_size) {
            s->idsp.idct_put(s->idct_block, idct_sz, block_u);
            s->idsp.idct_put(s->idct_block_v, idct_sz, block_v);
            if (s->bits & 7)
                shift_output(s, ptr, linesize);
            blit_func(idct_sz, s->idct_block, s->idct_block_v, linesize, ptr);
            ptr += bytes_per_pixel * out_uv_count;
        }
    }
}

static void mjpeg_idct_scan_progressive_ac_lxf(MJpegDecodeContext *s)
{
    int mb_x, mb_y;
    int c;
    const int bytes_per_pixel = 1 + (s->bits > 8);
    ...
    for (c = 0; c < 2; c++) {
        uint8_t *data = s->picture_ptr->data[c];
        int linesize  = s->linesize[c];
        av_assert0(s->linesize[c] == s->picture_ptr->linesize[c]);
        int h = s->h_max / s->h_count[c];
        int v = s->v_max / s->v_count[c];
        int mb_width     = (s->width  + h * block_size - 1) / (h * block_size);
        int mb_height    = (s->height + v * block_size - 1) / (v * block_size);
        ...
        if (c == 0) {
            idct_prog_ac(s, mb_width, mb_height, linesize, data);
        } else {
            idct_prog_ac_uv(s, mb_width, mb_height, linesize, data);
        }
    }
}
                         ...
                         int *unescaped_buf_size)
{
    ...
    if (s->padding_backup_addr) {
        memcpy(s->padding_backup_addr, s->padding_backup, AV_INPUT_BUFFER_PADDING_SIZE);
        s->padding_backup_addr = NULL;
    }
    ...
    if (start_code == SOS && !s->ls) {
        ...
        *unescaped_buf_ptr  = s->pkt->data;
        *unescaped_buf_size = dst - s->pkt->data;
        s->padding_backup_addr = s->pkt->data + *unescaped_buf_size;
        memcpy(s->padding_backup, s->padding_backup_addr, AV_INPUT_BUFFER_PADDING_SIZE);
        memset(s->padding_backup_addr, 0, AV_INPUT_BUFFER_PADDING_SIZE);

        av_log(s->avctx, AV_LOG_DEBUG, "escaping removed %"PTRDIFF_SPECIFIER" bytes\n",
               (buf_end - *buf_ptr) - (dst - s->pkt->data));
...
eoi_parser:
            if (!avctx->hwaccel && avctx->skip_frame != AVDISCARD_ALL &&
                ...
                if (s->avctx->vdec_knobs & VDEC_KNOB_DECODE_TO_NV12) {
                    mjpeg_idct_scan_progressive_ac_lxf(s);
                } else {
