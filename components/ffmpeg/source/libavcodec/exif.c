...
int avcodec_decode_exif(void *logctx, uint8_t *buf, int size, AVDictionary **metadata)
{
	int ret;
	int le, ifd_offset;
	GetByteContext gb;
	...
	bytestream2_init(&gb, buf, size);
	ret = ff_tdecode_header(&gb, &le, &ifd_offset);
	if (ret || ifd_offset <= 0) {
		return -EINVAL;
	}

	while (ifd_offset > 0) {
		bytestream2_seek(&gb, ifd_offset, SEEK_SET);
		ifd_offset = ff_exif_decode_ifd(logctx, &gb, le, 0, metadata);
	}
