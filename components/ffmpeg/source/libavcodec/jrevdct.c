...
#define DCT4_COMPACT_STRIDE 4
void ff_j_rev_dct4_compact(DCTBLOCK data)
{
  int32_t tmp0, tmp1, tmp2, tmp3;
  int32_t tmp10, tmp11, tmp12, tmp13;
  int32_t z1;
  ...
  for (rowctr = DCTSIZE-1; rowctr >= 0; rowctr--) {
    ...
    if ((d2 | d4 | d6) == 0) {
      ...
      dataptr += DCT4_COMPACT_STRIDE;     /* advance pointer to next row */
      continue;
    }
  ...
  for (rowctr = DCTSIZE-1; rowctr >= 0; rowctr--) {
    ...
    d0 = dataptr[DCT4_COMPACT_STRIDE*0];
    d2 = dataptr[DCT4_COMPACT_STRIDE*1];
    d4 = dataptr[DCT4_COMPACT_STRIDE*2];
    d6 = dataptr[DCT4_COMPACT_STRIDE*3];
    ...
    dataptr[DCT4_COMPACT_STRIDE*0] = tmp10 >> (CONST_BITS+PASS1_BITS+3);
    dataptr[DCT4_COMPACT_STRIDE*1] = tmp11 >> (CONST_BITS+PASS1_BITS+3);
    dataptr[DCT4_COMPACT_STRIDE*2] = tmp12 >> (CONST_BITS+PASS1_BITS+3);
    dataptr[DCT4_COMPACT_STRIDE*3] = tmp13 >> (CONST_BITS+PASS1_BITS+3);

    dataptr++;                  /* advance pointer to next column */
  }
}
#undef DCT4_COMPACT_STRIDE
...
#define DCT2_COMPACT_STRIDE 2
void ff_j_rev_dct2_compact(DCTBLOCK data){
  int d00, d01, d10, d11;

  data[0] += 4;
  d00 = data[0+0*DCT2_COMPACT_STRIDE] + data[1+0*DCT2_COMPACT_STRIDE];
  d01 = data[0+0*DCT2_COMPACT_STRIDE] - data[1+0*DCT2_COMPACT_STRIDE];
  d10 = data[0+1*DCT2_COMPACT_STRIDE] + data[1+1*DCT2_COMPACT_STRIDE];
  d11 = data[0+1*DCT2_COMPACT_STRIDE] - data[1+1*DCT2_COMPACT_STRIDE];

  data[0+0*DCT2_COMPACT_STRIDE]= (d00 + d10)>>3;
  data[1+0*DCT2_COMPACT_STRIDE]= (d01 + d11)>>3;
  data[0+1*DCT2_COMPACT_STRIDE]= (d00 - d10)>>3;
  data[1+1*DCT2_COMPACT_STRIDE]= (d01 - d11)>>3;
}
#undef DCT2_COMPACT_STRIDE
