...
typedef struct MJpegDecodeContext {
    ...
    int buffer_size;
    uint8_t *buffer;
    uint8_t padding_backup[AV_INPUT_BUFFER_PADDING_SIZE];
    void *padding_backup_addr;
    ...
    int16_t *blocks[MAX_COMPONENTS]; ///< intermediate sums (progressive mode)
    uint64_t *ac_bitmap[MAX_COMPONENTS];
    ...
    uint8_t idct_block[64]; // FIXME >8bpp?
    uint8_t idct_block_v[64];
