...
static void put_pixels_clamped4_compact_c(const int16_t *block, uint8_t *av_restrict pixels,
                                 int line_size)
{
    int i;
...
static void put_pixels_clamped2_compact_c(const int16_t *block, uint8_t *av_restrict pixels,
                                 int line_size)
{
    int i;
...
static void ff_jref_idct4_compact_put(uint8_t *dest, ptrdiff_t line_size, int16_t *block)
{
    ff_j_rev_dct4_compact (block);
    put_pixels_clamped4_compact_c(block, dest, line_size);
}
...
static void ff_jref_idct2_compact_put(uint8_t *dest, ptrdiff_t line_size, int16_t *block)
{
    ff_j_rev_dct2_compact (block);
    put_pixels_clamped2_compact_c(block, dest, line_size);
}
...
av_cold void ff_idctdsp_init(IDCTDSPContext *c, AVCodecContext *avctx)
{
    ...
    const int compact = avctx->vdec_knobs & VDEC_KNOB_LOWRES_COMPACT;

    if (avctx->lowres==1) {
        c->idct_put  = compact ? ff_jref_idct4_compact_put : ff_jref_idct4_put;
        c->idct_add  = ff_jref_idct4_add;
        c->idct      = ff_j_rev_dct4;
        c->perm_type = FF_IDCT_PERM_NONE;
    } else if (avctx->lowres==2) {
        c->idct_put  = compact ? ff_jref_idct2_compact_put : ff_jref_idct2_put;
