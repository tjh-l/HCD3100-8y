...
int ff_get_buffer_override_pixfmt(AVCodecContext *avctx, AVFrame *frame, int flags, enum AVPixelFormat pixfmt_override) {
    if (pixfmt_override == AV_PIX_FMT_NONE) {
        return ff_get_buffer(avctx, frame, flags);
    } else {
        const enum AVPixelFormat ctx_pixfmt = avctx->pix_fmt;
        avctx->pix_fmt = pixfmt_override;
        int ret = ff_get_buffer(avctx, frame, flags);
        avctx->pix_fmt = ctx_pixfmt;
        return ret;
    }
}
