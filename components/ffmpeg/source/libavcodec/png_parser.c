...
static int png_parse_info(AVCodecParserContext *s, AVCodecContext *avctx, const uint8_t *buf, int buf_size)
{
    uint32_t len, tag;
    buf += 8;
    ...
    while (buf_size > 16) {
        ...
        if (len >= 8 && tag == MKTAG('I', 'H', 'D', 'R')) {
            avctx->width = AV_RB32(buf);
            buf += 4;
            buf_size -= 4;
            
            avctx->height = AV_RB32(buf);
            buf += 4;
            buf_size -= 4;
            
            //printf("get_png_info: width %d height %d\n", avctx->width, avctx->width);
            return 0;
        } else {
            buf += len + 4;
            buf_size -= len + 4;
...
flush:
    ...
    if (avctx->width == 0 || avctx->height == 0)
        png_parse_info(s, avctx, buf, buf_size);
