...
static int asf_read_seek_check(struct AVFormatContext *s)
{
    ff_asf_guid g;
    ASFContext *asf = s->priv_data;
    int64_t current_pos = avio_tell(s->pb);
    ...
    if (avio_size(s->pb) <= 52428800)
        ret = 0;
    avio_seek(s->pb, current_pos, SEEK_SET);
    asf->index_scaned = 1;
    asf->index_loaded = ret ? 0 : 1;
    return ret;
}
...
AVInputFormat ff_asf_demuxer = {
    ...
    .seek_check     = asf_read_seek_check,
