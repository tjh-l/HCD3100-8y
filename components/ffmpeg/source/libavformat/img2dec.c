...
int ff_img_read_packet(AVFormatContext *s1, AVPacket *pkt)
{
...
#if ARCH_MIPS
    if (!s->is_pipe) {//(size[0] + size[1] + size[2]) > (0x10000 - sizeof(AvPktHd))) {
        int total_size = size[0] + size[1] + size[2] + 32 * 1024;
        void *user_addr;
        void *kernel_addr;
    ...
    for (i = 0; i < 3; i++) {
        if (f[i]) {
            URLContext *h = (URLContext *)f[i]->opaque;
            if (h && h->av_class == &ffurl_context_class && h->priv_data)
                av_opt_set(h->priv_data, "blocksize", "262144", 0);
...
#if ARCH_MIPS
    if (pkt->kernel_addr) {
        memset(pkt->data + pkt->size, 0, 32 * 1024);
    }
#endif
