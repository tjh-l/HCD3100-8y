...
#define IS_MPEGTS_HEADER2(data) (((data)[0] == 0x47) && \
                                (((data)[1] & 0x80) == 0x00))
...
static const AVOption options[] = {
    ...
    {"skip_clear", "skip clearing programs", offsetof(MpegTSContext, skip_clear), AV_OPT_TYPE_BOOL,
     {.i64 = 0}, 0, 1, 0 },
    {"parse_stream_info_done", "avformat_find_stream_info has done", offsetof(MpegTSContext, parse_stream_info_done), AV_OPT_TYPE_BOOL,
     {.i64 = 0}, 0, 1, 0 },
    { NULL },
};
...
static int new_pes_packet(PESContext *pes, AVPacket *pkt)
{
    ...
    } else if (!pes->ts->parse_stream_info_done && pes->total_size == MAX_PES_PAYLOAD &&
        pkt->size < (MAX_PES_PAYLOAD >> 2)) {
        AVBufferRef *buf = buffer_pool_get(pes->ts, pkt->size + AV_INPUT_BUFFER_PADDING_SIZE);
        if (buf) {
            memcpy (buf->data, pes->buffer->data, pkt->size);
            av_buffer_unref(&pes->buffer);
            pkt->buf = buf;
            pkt->data = buf->data;
        }
...
static int mpegts_sync_check(AVFormatContext *s)
{
    ...
    while (i < end_i) {
        if (IS_MPEGTS_HEADER2 (data + i) && IS_MPEGTS_HEADER2 (data + i + ts->raw_packet_size)){
