...
/**
 * Find encoding for decode_str.
 */
static int find_encoding(AVIOContext *pb, int len)
{
    unsigned char *data = pb->buf_ptr;
    int i = 0;
    len -= 1;
                      ...
                      uint8_t **dst, int *maxread)
{
    ...
    encoding = encoding ? encoding : find_encoding(pb, left);
    switch (encoding) {
