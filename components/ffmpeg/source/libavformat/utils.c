...
FF_ENABLE_DEPRECATION_WARNINGS
        ...
        if (!st->need_parsing || !st->parser) {
            ...
            st->codecpar->lowres = st->internal->avctx->lowres;
        } else {
            /* free packet */
            av_packet_unref(pkt);
        }
            ...
            if (next_pkt->dts != AV_NOPTS_VALUE) {
                ...
                while (pktl && next_pkt->pts == AV_NOPTS_VALUE) {
                    if (pktl->pkt.stream_index == next_pkt->stream_index) {
                        if (av_compare_mod(next_pkt->dts, pktl->pkt.dts, 2ULL << (wrap_bits - 1)) < 0) {
                        ...
                        } else if (pktl->pkt.flags & AV_PKT_FLAG_KEY) {
                            if (pktl->pkt.duration > 0)
                                next_pkt->pts = pktl->pkt.dts - pktl->pkt.duration;
                            else
                                next_pkt->pts = pktl->pkt.dts - 1;
                            if (last_dts != AV_NOPTS_VALUE) {
                                // Once last dts was set to AV_NOPTS_VALUE, we don't change it.
                                last_dts = next_pkt->pts;
                            }
                        }
                    }
                    pktl = pktl->next;
                               ...
                               int64_t timestamp, int flags)
{
    ...
    if (s->iformat->seek_check && timestamp != 0) {
...
static void estimate_timings_from_pts(AVFormatContext *ic, int64_t old_offset)
{
    ...
    do {
        ...
        if (offset < 0)
            offset = 0;
        av_opt_set(ic, "pes_err", "1", AV_OPT_SEARCH_CHILDREN);
        avio_seek(ic->pb, offset, SEEK_SET);
        read_size = 0;
        for (;;) {
            if (read_size >= DURATION_MAX_READ_SIZE << (FFMIN(FFMAX(retry - 1, 0), 1)))
                break;
    ...
    av_opt_set(ic, "pes_err", "0", AV_OPT_SEARCH_CHILDREN);
...
FF_ENABLE_DEPRECATION_WARNINGS
    ...
    for (;;) {
        ...
        if (st->codec_info_nb_frames>1) {
            ...
            if (!analyzed_all_streams || mpeg_stream) {
                if (!strcmp(ic->iformat->name, "flv") && ic->nb_streams >= 2 && st->codec_info_nb_frames >= 20) {
                    av_log(ic, AV_LOG_VERBOSE, "flv break find_stream_info\n");
                    break;
                }
            ...
            if (!strcmp(ic->iformat->name,"wav") || (!orig_nb_streams && !analyzed_all_streams)) {
...
find_stream_info_err:
    ...
    if (ret >= 0 && ic->priv_data && ic->iformat && ic->iformat->priv_class) {
        av_opt_set(ic->priv_data, "parse_stream_info_done", "1", 0);
    }
    return ret;
...
find_again:
    for (i = 0; i < nb_streams; i++) {
        ...
        if (wanted_stream_nb >= 0 && real_stream_index != wanted_stream_nb)
            continue;
        if (type == AVMEDIA_TYPE_AUDIO && (!ignore_audio_infos && !(par->channels && par->sample_rate))) {
            continue;
        }
        if (decoder_ret) {
...
find_best:
    ...
    else if (!ignore_audio_infos && program && type == AVMEDIA_TYPE_AUDIO){
        ignore_audio_infos = 1;
        goto find_again;
    }
    return ret;
}
