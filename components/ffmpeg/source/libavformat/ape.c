...
static int ape_read_seek(AVFormatContext *s, int stream_index, int64_t timestamp, int flags)
{
    ...
    if (flags & AVSEEK_FLAG_BACKWARD) {
        ret = avio_seek(s->pb, st->index_entries[index].pos, SEEK_SET);
    } else {
        if (st->index_entries[index].timestamp >= timestamp) {
            ret = avio_seek(s->pb, st->index_entries[index].pos, SEEK_SET);
        } else if (index + 1 < st->nb_index_entries) {
            ret = avio_seek(s->pb, st->index_entries[index + 1].pos, SEEK_SET);
            index = index + 1;
        } else {
            ret = avio_seek(s->pb, 0, SEEK_END);
