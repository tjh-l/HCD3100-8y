...
struct variant {
    ...
    char codecs_group[MAX_FIELD_LEN];
};
    ...
    const char *url, const char *base)
{
    ...
    if (info) {
        ...
        strcpy(var->codecs_group, info->codecs);
    }
...
#define MAX_USER_AGENTS 3
static char* user_agents[MAX_USER_AGENTS] = {"VLC/3.0.20",
    ...
    AVDictionary **opts, AVDictionary *opts2, int *is_http_out)
{
    ...
    if (c->user_agent_index >= 0 && c->user_agent_index < MAX_USER_AGENTS) {
        av_dict_set(&tmp, "user_agent", user_agents[c->user_agent_index], 0);
    }

retry:
    if (user_agent_index >= 0 && user_agent_index < MAX_USER_AGENTS) {
        av_dict_set(&tmp, "user_agent", user_agents[user_agent_index], 0);
    }
    ...
    if (ret >= 0) {
        ...
        if (user_agent_index >= 0) {
            c->user_agent_index = user_agent_index;
        }
        ...
        user_agent_index++;
        if (user_agent_index < MAX_USER_AGENTS) {
            goto retry;
        }
    }
    ...
    struct playlist *pls, AVIOContext *in)
{
    ...
    while ((list_from_mem && !in->eof_reached) || (!list_from_mem && !avio_feof(in))) {
        ...
        if (av_strstart(line, "#YT-EXT-CONDENSED-URL:", &ptr)) {
            ...
            if (yt_str) {
                yt_base_url = av_malloc(strlen(yt_str));
                if (!yt_base_url) {
                    continue;
                }
            ...
            if (yt_str) {
                char *saveptr = NULL;
                char *inptr;
                yt_param = av_malloc(strlen(yt_str));
                if (!yt_param) {
                    continue;
                }
                ...
                for (int i = 0; i < MAX_PARAM_LEN; i++) {
                    if (!(yt_params[i] = av_strtok(inptr, ",", &saveptr))) {
                        break;
                    }
                    inptr = NULL;
            ...
            if (yt_str) {
                yt_prefix = av_malloc(strlen(yt_str));
                if (!yt_prefix) {
                    continue;
                }
            ...
            if ((discount_time <= 45 * AV_TIME_BASE) && discount_seg) {
...
restart:
    ...
    reload:
        ...
        if (c->http_multiple == 1 && v->input_next_requested && v->input_next) {
...
static int hls_read_header(AVFormatContext *s)
{
    ...
    if (c->n_renditions > 0) {
        /* TODO: create a thread to parse others playlists */
        /* match audio video (TODO:subtitle) */
        int bandwidth = 0;
        int has_valid_audio = 0;
        int has_valid_video = 0;
        char *audiogroup = NULL;
        char *codecsgroup = NULL;
        struct variant *var = NULL;
        struct rendition *ren = NULL;
        struct playlist *pls = NULL;

        av_log(s, AV_LOG_INFO, "separate audio num %d\n", c->n_renditions);
        for (i = 0;i < c->n_variants;i++) {
            var = c->variants[i];
            if (strstr(var->codecs_group, "vp09") || strstr(var->codecs_group, "vp9") ||
                var->playlists[0]->broken) {
                var->playlists[0]->broken = 1;
                continue;
            }
            if (!bandwidth) {
                bandwidth = var->bandwidth;
                audiogroup = var->audio_group;
                codecsgroup = var->codecs_group;
            } else if (((var->bandwidth > bandwidth && var->bandwidth <= c->best_bandwidth) ||
                (var->bandwidth < bandwidth && var->bandwidth >= c->best_bandwidth))) {
                bandwidth = var->bandwidth;
                audiogroup = var->audio_group;
                codecsgroup = var->codecs_group;
            }
        ...
        av_log(s, AV_LOG_INFO, "bandwidth:%d codecsgroup:%s\n", bandwidth, codecsgroup);
        ...
        if (bandwidth) {
            for (i = 0; i < c->n_variants;i++) {
                var = c->variants[i];
                pls = var->playlists[0];
                if (var->bandwidth == bandwidth) {
                    var->playlists[0]->needed = 1;
                    has_valid_video = 1;
                    break;
                }
            }
        ...
        if (has_valid_video) {
            for (i = 0;i < c->n_renditions;i++) {
                ren = c->renditions[i];
                pls = ren->playlist;
                if (audiogroup && !strcmp(ren->group_id, audiogroup) && ren->playlist) {
                    if (ren->playlist->broken)
                        continue;
                    pls->needed = 1;
                    has_valid_audio = 1;
                    break;
        ...
        if (has_valid_audio && has_valid_video) {
            c->has_separate_audio = 1;
        } else if (!has_valid_audio && !has_valid_video) {
            goto fail;
        } else {
            av_log(s, AV_LOG_WARNING, "av not separate.\n");
        }

        if (c->has_separate_audio) {
            for (i = 0; i < c->n_renditions;i++) {
                av_log(s, AV_LOG_INFO, "c->renditions[%d]->playlist->needed %d\n", i, c->renditions[i]->playlist->needed);
                if (c->renditions[i]->playlist->needed) {
                    av_dict_set(&s->metadata, "audio_url", c->renditions[i]->playlist->url, 0);
                }
            }

            for (i = 0; i < c->n_variants;i++) {
                av_log(s, AV_LOG_INFO, "c->variants[%d]->playlists[0]->needed %d\n", i, c->variants[i]->playlists[0]->needed);
                if (c->variants[i]->playlists[0]->needed) {
                    av_dict_set(&s->metadata, "video_url", c->variants[i]->playlists[0]->url, 0);
                }
            }
    ...
    if (c->n_playlists > 1 || c->playlists[0]->n_segments == 0) {
        ...
        if (!bandwidth) {
            av_log(s, AV_LOG_WARNING, "No separate video!!!!\n");
            goto fail;
        }
        av_log(s, AV_LOG_INFO, "bandwidth:%d codecsgroup:%s\n", bandwidth, codecsgroup);
        ...
        for (i = 0; i < c->n_variants;i++) {
            ...
            if (var->bandwidth == bandwidth) {
                if ((ret = parse_playlist(c, pls->url, pls, NULL)) < 0) {
                    ...
                    av_log(s, AV_LOG_WARNING, "parse video error, variant's num:%d\n", i);
                    goto fail;
                }
    ...
    if (!c->variants[0]->playlists[0]->finished &&
        ...
        c->variants[0]->playlists[0]->needed) {
    ...
    if (c->variants[0]->playlists[0]->finished ||
        ...
        for (var_num = 0; var_num < c->n_variants; var_num++) {
            if (c->variants[var_num]->playlists[0]->n_segments > 0)
                break;
        }
        for (i = 0; i < c->variants[var_num]->playlists[0]->n_segments; i++)
            duration += c->variants[var_num]->playlists[0]->segments[i]->duration;
        s->duration = duration;
        printf("n_segments %d\n", c->variants[var_num]->playlists[0]->n_segments);
    }
    ...
    for (i = 0; i < c->n_playlists; i++) {
        ...
        if (pls->n_segments == 0 || pls->broken || !pls->needed)
            continue;
    ...
    for (i = 0; i < c->n_playlists; i++) {
        ...
        av_log(s, AV_LOG_WARNING, "playlists num:%d\n", i);
        ...
        if (ret < 0) {
            ...
            printf("Error when loading first segment '%s'\n", url);
            avformat_free_context(pls->ctx);
            pls->ctx = NULL;
            av_free(url);
            goto fail;
        ...
        if (ret < 0) {
            printf("open failed, %d, %d\n", pls->yt_m3u8, pls->n_init_sections);
            goto fail;
        }
    ...
    int64_t timestamp, int flags)
{
    ...
    if ((seek_timestamp - seg_start_time >= 10 * 1000 * 1000) && !(flags & AVSEEK_FLAG_BACKWARD) && 
        (seq_no - seek_pls->start_seq_no) < (seek_pls->n_segments - 1)) {
        seg_start_time += seek_pls->segments[seq_no - seek_pls->start_seq_no]->duration;
        seq_no++;
    }
...
static const AVOption hls_options[] = {
    ...
    {"best_bandwidth", "Use best_bandwidth",
        OFFSET(best_bandwidth), AV_OPT_TYPE_INT, {.i64 = 3000000}, 0, INT_MAX, FLAGS},
    {"has_separate_audio", "has separate audio",
        OFFSET(has_separate_audio), AV_OPT_TYPE_INT, {.i64 = 0}, 0, INT_MAX, FLAGS},
    {NULL}
};
