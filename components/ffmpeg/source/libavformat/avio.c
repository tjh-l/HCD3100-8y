...
static const AVOption options[] = {
    ...
    {"connect_retry_times", "connect retry times", offsetof(URLContext, connect_retry_times), AV_OPT_TYPE_INT, { .i64 = 3 }, 0, INT32_MAX, AV_OPT_FLAG_DECODING_PARAM },
    { NULL }
};
                                  ...
                                  const AVIOInterruptCB *int_cb)
{
    ...
    opt = av_opt_find(&uc->av_class, "connect_retry_times", NULL, 0, 0);
    if (opt) {
        uc->connect_retry_times = opt->default_val.i64;
    }
...
retry:
    ...
    if((err == AVERROR(EIO)) && try_times) {
        av_log(uc, AV_LOG_VERBOSE, "connect failed, retry times %d, %s\n", try_times, uc->filename);
        try_times--;
        if (!uc->interrupt_callback.callback ||
            !uc->interrupt_callback.callback(uc->interrupt_callback.opaque)) {
            goto retry;
        }
    }
