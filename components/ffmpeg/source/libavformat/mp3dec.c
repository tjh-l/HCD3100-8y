...
static void mp3_type_find (uint8_t *buf, int in_size, int *found_prob)
{
    uint8_t *data = NULL;
...
{
    int score = 0;
    mp3_type_find (p->buf, p->buf_size, &score);
    return score;
}
...
static int mp3_read_header(AVFormatContext *s)
{
    ...
    s->pb->maxsize = -1;
    off = avio_tell(s->pb);
    //some mp3 files have both id3v2 and id3v1 tag,both need to get.
