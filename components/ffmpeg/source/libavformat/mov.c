...
#include "pthread.h"
#define DIFFCOUNT   50
static pthread_mutex_t g_mmz_mutex = PTHREAD_MUTEX_INITIALIZER;
...
static int mov_read_ftyp(MOVContext *c, AVIOContext *pb, MOVAtom atom)
{
    ...
    if (strstr(comp_brands_str, "dash"))
        c->isdash = 1;
...
static int mov_read_mvhd(MOVContext *c, AVIOContext *pb, MOVAtom atom)
{
    ...
    if (c->fc->duration > 330 * (int64_t)60  * 1000 * 1000) {
        return AVERROR(ENOMEM);
    }
...
static int mov_read_stsz(MOVContext *c, AVIOContext *pb, MOVAtom atom)
{
    AVStream *st;
    MOVStreamContext *sc;
    unsigned int i, entries, sample_size, field_size;
    int ret;
...
static int mmz_fd = -1;
static int mmz_size = 0;
static void *mmz_tmp = NULL;
static void mov_mmz_free(uint8_t *data)
{
#ifdef __linux__
    if (mmz_fd >= 0) {
        printf("mmz free %p, %d\n", data, mmz_fd);
        munmap(data, mmz_size);
        mmz_size = 0;
        close(mmz_fd);
        mmz_fd = -1;
    }
...
static int mov_mmz_malloc(int size, void **user_addr)
{
#ifdef __linux__
    int ret;
    struct mmz_blk mmz = {0, 32, size, 0};
    int fd;
    ...
    *user_addr = mmz_memalign(0, 32, size);
    printf("mmz malloc user:0x%x\n",(int)*user_addr);
    return (*user_addr) ? 0 : -1;
#endif
}
...
static void mov_build_index(MOVContext *mov, AVStream *st)
{
    ...
    if (!(st->codecpar->codec_type == AVMEDIA_TYPE_AUDIO &&
        ...
        if (sc->sample_count >= UINT_MAX / sizeof(*st->index_entries) - st->nb_index_entries)
            return;
        pthread_mutex_lock(&g_mmz_mutex);
        mmz_size = (st->nb_index_entries + sc->sample_count) * sizeof(*st->index_entries);
        av_log(mov->fc, AV_LOG_INFO, "mov %s stream 1 building index in mmz memory, size:%d.\n", (st->codecpar->codec_type == AVMEDIA_TYPE_VIDEO) ?
                                                                                        "video" : "audio", mmz_size);
        if ((mmz_fd = mov_mmz_malloc(mmz_size, &st->index_entries)) < 0) {
            mmz_size = 0;
            st->nb_index_entries = 0;
            pthread_mutex_unlock(&g_mmz_mutex);
            av_log(mov->fc, AV_LOG_ERROR, "mov %s stream 1 build index failed.\n", (st->codecpar->codec_type == AVMEDIA_TYPE_VIDEO) ?
                                                                                        "video" : "audio");
            return;
        }
        mmz_tmp = st->index_entries;
        av_log(mov->fc, AV_LOG_INFO, "mov %s stream 1 built index in mmz memory.\n", (st->codecpar->codec_type == AVMEDIA_TYPE_VIDEO) ?
                                                                                        "video" : "audio");
        ...
        if (total >= UINT_MAX / sizeof(*st->index_entries) - st->nb_index_entries)
            return;
        pthread_mutex_lock(&g_mmz_mutex);
        mmz_size = (st->nb_index_entries + total) * sizeof(*st->index_entries);
        av_log(mov->fc, AV_LOG_INFO, "mov %s stream 2 building index in mmz memory, size:%d.\n", (st->codecpar->codec_type == AVMEDIA_TYPE_VIDEO) ?
                                                                                        "video" : "audio", mmz_size);
        if ((mmz_fd = mov_mmz_malloc(mmz_size, &st->index_entries)) < 0) {
            mmz_size = 0;
            st->nb_index_entries = 0;
            pthread_mutex_unlock(&g_mmz_mutex);
            av_log(mov->fc, AV_LOG_ERROR, "mov %s stream 2 build index failed.\n", (st->codecpar->codec_type == AVMEDIA_TYPE_VIDEO) ?
                                                                                        "video" : "audio");
            return;
        }
        mmz_tmp = st->index_entries;
        av_log(mov->fc, AV_LOG_INFO, "mov %s stream 2 built index in mmz memory.\n", (st->codecpar->codec_type == AVMEDIA_TYPE_VIDEO) ?
                                                                                        "video" : "audio");
...
static void free_stream_ctx(AVFormatContext *s, MOVStreamContext *sc)
{
    int j;
    if (!sc)
        return;
...
static int mov_optimize_index(MOVStreamContext* sc, AVStream* st)
{
    sc->nb_index_entries = (st->nb_index_entries + DIFFCOUNT - 1) / DIFFCOUNT;
    sc->nb_diff_index_entries = st->nb_index_entries;
    sc->index_entries = av_malloc(sc->nb_index_entries * sizeof(AVIndexEntry)); // 1 / 50
    if (!sc->index_entries)
        goto fail;
    sc->diff_index_entries = av_malloc(sc->nb_diff_index_entries * sizeof(MOVDiffIndexEntry)); // 1 / 2
    if (!sc->diff_index_entries)
        goto fail;
    for (int i = 0; i < st->nb_index_entries; i++) {
        if (i % DIFFCOUNT == 0) {
            sc->index_entries[i / DIFFCOUNT]   = st->index_entries[i];
            sc->diff_index_entries[i].diff_pos = 0;
            sc->diff_index_entries[i].diff_dts = 0;
        } else {
            sc->diff_index_entries[i].diff_pos = st->index_entries[i].pos - sc->index_entries[i / DIFFCOUNT].pos;
            sc->diff_index_entries[i].diff_dts = av_rescale(st->index_entries[i].timestamp - sc->index_entries[i / DIFFCOUNT].timestamp, 
                                                            500, sc->time_scale); // ms
        }
        sc->diff_index_entries[i].flags = st->index_entries[i].flags;
        sc->diff_index_entries[i].size  = st->index_entries[i].size;
    }
    mov_mmz_free(mmz_tmp);
    mmz_tmp = NULL;
    pthread_mutex_unlock(&g_mmz_mutex);
    st->index_entries    = sc->index_entries;
    st->nb_index_entries = sc->nb_index_entries;
...
static int mov_read_trak(MOVContext *c, AVIOContext *pb, MOVAtom atom)
{
    ...
    if ((ret = mov_read_default(c, pb, atom)) < 0) {
        free_stream_ctx(c->fc, (MOVStreamContext *)st->priv_data);
    ...
    if (st->nb_index_entries > 0 && mmz_fd >= 0) {
        av_log(c->fc, AV_LOG_INFO, "mov %s stream is optimizing index.\n", (st->codecpar->codec_type == AVMEDIA_TYPE_VIDEO) ? 
                                                                            "video" : "audio");
        ret = mov_optimize_index(sc, st);
        if (ret < 0) {
            av_log(c->fc, AV_LOG_ERROR, "mov %s stream index optimization failed.\n", (st->codecpar->codec_type == AVMEDIA_TYPE_VIDEO) ? 
                                                                            "video" : "audio");
        } else {
            av_log(c->fc, AV_LOG_INFO, "mov %s stream index is optimized.\n", (st->codecpar->codec_type == AVMEDIA_TYPE_VIDEO) ? 
                                                                            "video" : "audio");
        }
    } else if (st->nb_index_entries == 0 && !st->index_entries) {
        sc->isdash = 1;
        c->isdash = 1;
    }

    if (st->nb_index_entries <= 0 && sc->sample_count > 0) {
        free_stream_ctx(c->fc, (MOVStreamContext *)st->priv_data);
        ff_free_stream(c->fc, st);
    }
fail:
    if (mmz_fd >= 0) {
        ...
        mmz_tmp = NULL;
        pthread_mutex_unlock(&g_mmz_mutex);
        av_log(c->fc, AV_LOG_INFO, "mov mmz free.\n");
    }
...
static int mov_read_trun(MOVContext *c, AVIOContext *pb, MOVAtom atom)
{
    ...
    if (sc->has_sidx && requested_size > st->index_entries_allocated_size) {
    ...
    if (sc->has_sidx && requested_size > sc->ctts_allocated_size) {
...
static int mov_read_close(AVFormatContext *s)
{
    ...
    for (i = 0; i < s->nb_streams; i++) {
        ...
        av_freep(&sc->coll);
        if (sc->diff_index_entries)
            av_freep(&sc->diff_index_entries);
    }
...
static inline int is_picture(AVStream *s)
{
	int ret = 0;
...
static int mov_read_header(AVFormatContext *s)
{
    ...
    if (!mov->online_stream && (s->pb->seekable & AVIO_SEEKABLE_NORMAL)) {
        for (i = 0; i < s->nb_streams; i++) {
            AVStream *avst = s->streams[i];
            MOVStreamContext *msc = avst->priv_data;
            if (avst->codecpar->codec_type == AVMEDIA_TYPE_VIDEO && !is_picture(avst) &&
                avst->codecpar->codec_id != AV_CODEC_ID_NONE) {
                AVIndexEntry *current_sample = &avst->index_entries[msc->current_sample];
                avio_seek(s->pb, current_sample->pos, SEEK_SET);
            }
        }
    }
...
static AVIndexEntry *mov_find_next_sample(AVFormatContext *s, AVStream **st)
{
    ...
    int i;
    int64_t file_pos = avio_tell(s->pb);
    int64_t pos_diff, best_pos;

    if ((s->pb->seekable & AVIO_SEEKABLE_NORMAL)) {
        for (i = 0; i < s->nb_streams; i++) {
            AVStream *avst = s->streams[i];
            MOVStreamContext *msc = avst->priv_data;
            if (!msc->isdash && msc->pb && msc->current_sample < msc->nb_diff_index_entries) {
                int64_t dts;
                AVIndexEntry *current_sample = &avst->index_entries[msc->current_sample / DIFFCOUNT];
                msc->tmp_index.pos         = current_sample->pos + msc->diff_index_entries[msc->current_sample].diff_pos;
                msc->tmp_index.timestamp   = current_sample->timestamp + av_rescale(msc->diff_index_entries[msc->current_sample].diff_dts,
                                                          msc->time_scale, 500);
                msc->tmp_index.flags        = msc->diff_index_entries[msc->current_sample].flags;
                msc->tmp_index.size         = msc->diff_index_entries[msc->current_sample].size;

                dts = av_rescale(msc->tmp_index.timestamp, AV_TIME_BASE, msc->time_scale);
                av_log(s, AV_LOG_TRACE, "stream %d, sample %d, dts %"PRId64"\n", i, msc->current_sample, dts);
                pos_diff = FFABS(msc->tmp_index.pos - file_pos);
                if (!sample || (msc->pb != s->pb && dts < best_dts) ||
                    (msc->pb == s->pb && dts != AV_NOPTS_VALUE &&
                    (FFABS(best_dts - dts) <= (AV_TIME_BASE / 2) && pos_diff < best_pos) ||
                    (FFABS(best_dts - dts) > (AV_TIME_BASE / 2) && dts < best_dts))) {
                    sample = &msc->tmp_index;
                    best_dts = dts;
                    best_pos = pos_diff;
                    *st = avst;
                }
            } else if (msc->pb && msc->current_sample < avst->nb_index_entries) {
                AVIndexEntry *current_sample = &avst->index_entries[msc->current_sample];
                int64_t dts = av_rescale(current_sample->timestamp, AV_TIME_BASE, msc->time_scale);
                av_log(s, AV_LOG_TRACE, "stream %d, sample %d, dts %"PRId64"\n", i, msc->current_sample, dts);
                pos_diff = FFABS(current_sample->pos - file_pos);
                if (!sample || (msc->pb != s->pb && dts < best_dts) ||
                    (msc->pb == s->pb && dts != AV_NOPTS_VALUE &&
                    (FFABS(best_dts - dts) <= (AV_TIME_BASE / 2) && pos_diff < best_pos) ||
                    (FFABS(best_dts - dts) > (AV_TIME_BASE / 2) && dts < best_dts))) {
...
static inline int64_t mov_get_sample_timestamp(MOVStreamContext *sc, int sample)
{
    int64_t timestamp;
    timestamp = sc->index_entries[sample / DIFFCOUNT].timestamp + 
                sc->diff_index_entries[sample].diff_dts * sc->time_scale / 500;
    return timestamp;
}
...
static int mov_read_packet(AVFormatContext *s, AVPacket *pkt)
{
 ...
 retry:
    ...
    if (sc->ctts_data && sc->ctts_index < sc->ctts_count) {
        ...
        int64_t next_dts;
        if (sc->isdash)
            next_dts = (sc->current_sample < sc->nb_diff_index_entries) ?
                                st->index_entries[sc->current_sample].timestamp : st->duration;
        else
            next_dts = (sc->current_sample < sc->nb_diff_index_entries) ?
                                mov_get_sample_timestamp(sc, sc->current_sample) : st->duration;
...
static int mov_index_search_timestamp(MOVStreamContext *sc, AVStream *st, int64_t timestamp, int flags)
{
    int type, sample, dts, i, count = 0;
    type = st->codecpar->codec_type;
    sample = av_index_search_timestamp(st, timestamp, flags);

    if (sample < 0 && is_picture(st)) {
        return sample;
    }

    if (sample < 0 && !(flags & AVSEEK_FLAG_BACKWARD)) {
        sample = sc->nb_index_entries - 1;
        flags = AVSEEK_FLAG_BACKWARD;
    }

    if (type == AVMEDIA_TYPE_VIDEO && st->index_entries[sample].flags) {
        sample = sample * DIFFCOUNT;
        return sample;
    }

    if (flags & AVSEEK_FLAG_BACKWARD) {
        // seek <-- 向前查找关键帧
        for (i = sample * DIFFCOUNT; i >= 0; i--) {
            if (type == AVMEDIA_TYPE_VIDEO && sc->diff_index_entries[i].flags) {
                sample = i;
                break;
            } else if (type == AVMEDIA_TYPE_AUDIO) {
                dts = mov_get_sample_timestamp(sc, i);
                if (dts <= timestamp && !count) {
                    i += DIFFCOUNT;
                    count++;
                    continue;
                } else if (dts <= timestamp && count) {
                    sample = i;
                    break;
                }
            }
        ...
        // seek --> 向后查找关键帧
        for (i = sample * DIFFCOUNT; i < sc->nb_diff_index_entries; i++) {
            if (type == AVMEDIA_TYPE_VIDEO && sc->diff_index_entries[i].flags) {
                sample = i;
                break;
            } else if (type == AVMEDIA_TYPE_AUDIO) {
    ...
    return (sample == i) ? sample : AVERROR_EOF;
}
...
static int mov_seek_stream(AVFormatContext *s, AVStream *st, int64_t timestamp, int flags)
{
    ...
    if (sc->isdash)
        ...
        sample = mov_index_search_timestamp(sc, st, timestamp, flags | AVSEEK_FLAG_ANY);
...
static int64_t mov_get_skip_samples(AVStream *st, int sample)
{
    ...
    int64_t ts;
    if (sc->isdash)
        ts = st->index_entries[sample].timestamp;
    else
        ts = mov_get_sample_timestamp(sc, sample);
    int64_t off;
...
static int mov_read_seek(AVFormatContext *s, int stream_index, int64_t sample_time, int flags)
{
    ...
    if (!mc->isdash)
        for (i = 0; i < s->nb_streams; i++) {
            if (s->streams[i]->codecpar->codec_type == AVMEDIA_TYPE_VIDEO &&
                s->streams[i]->discard != AVDISCARD_ALL && !is_picture(s->streams[i]) &&
                s->streams[i]->codecpar->codec_id != AV_CODEC_ID_NONE) {
                stream_index = i;
                break;
            }
        }
    ...
    if (!mc->online_stream && (s->pb->seekable & AVIO_SEEKABLE_NORMAL)) {
        for (i = 0; i < s->nb_streams; i++) {
            AVStream *avst = s->streams[i];
            MOVStreamContext *msc = avst->priv_data;
            if (avst->codecpar->codec_type == AVMEDIA_TYPE_VIDEO && !is_picture(avst) &&
                avst->codecpar->codec_id != AV_CODEC_ID_NONE) {
                AVIndexEntry *current_sample = &avst->index_entries[sample / DIFFCOUNT];
                avio_seek(s->pb, current_sample->pos, SEEK_SET);
            }
        }
    }
    ...
    if (mc->seek_individually) {
        ...
        if (mc->isdash)
            ...
            seek_timestamp = mov_get_sample_timestamp(sc, sample);
