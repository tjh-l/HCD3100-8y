...
static char sBtname[] = "SRT656";
static char gBtName[20] = {0};
static bluetooth_ir_control_t bt_control;
int ba_connect = 0;
char *ba_dev_name = NULL; 
...
typedef enum _E_BLUETOOTH_CMD_SENDS_
{
    ...
    BT_CMD_SET_CHANNEL_INPUT_SELECT,
    BT_CMD_SET_POWER_ON_TO_RX,
    BT_CMD_SET_POWER_ON_TO_TX,
    BT_CMD_SET_BT_CTRL_CMD,
    BT_CMD_SET_VOLUME_LEVEL,
    BT_CMD_SET_BT_LOCAL_NAME,
    BT_CMD_GET_BT_LOCAL_NAME,
    BT_CMD_SET_POWER_ON_TO_TX_NOT_ACTION,    
...
int bluetooth_poweron_rx(void)
{
    bt_device_status_e read_state=0;
    int count=0;
    if(bt_ad6956f_set_action(BT_CMD_SET_POWER_ON_TO_TX_NOT_ACTION,0)==0)
    {
        while(1)
        {
            read_state = bt_get_device_sys_status();
...
{
    if(bt_ad6956f_set_action(BT_CMD_SET_CHANNEL_INPUT_SELECT,&value)==0)
    {
        printf("Line:482 cmds : 0x%02x  \n ",value);
        return BT_RET_SUCCESS;
    }
    else
        return BT_RET_ERROR;
...
{
    if(bt_ad6956f_set_action(BT_CMD_SET_VOLUME_LEVEL,&value)==0)
    {
        printf("Line:525 cmds : 0x%02x  \n ",value);
        return BT_RET_SUCCESS;
    }
    else
        return BT_RET_ERROR;
...
{
    if(bt_ad6956f_set_action(BT_CMD_SET_BT_CTRL_CMD,&value)==0)
    {
        printf("Line:561 cmds :BT_CMD_SET_BT_CTRL_CMD\n");
...
{
    if(bt_ad6956f_set_action(BT_CMD_SET_POWER_ON_TO_RX,0)==0)
    {
        printf("Line:575 cmds :BT_CMD_SET_POWER_ON_TO_RX\n");
...
{
    if(bt_ad6956f_set_action(BT_CMD_SET_POWER_ON_TO_TX,0)==0)
    {
        printf("Line:588 cmds : BT_CMD_SET_POWER_ON_TO_TX\n");
...
{
    if(bt_ad6956f_set_action(BT_CMD_GET_BT_LOCAL_NAME,0)==0)
    {
        return BT_RET_SUCCESS;
    }
    else
...
int bt_set_local_name(unsigned char *value)
{
    if(bt_ad6956f_set_action(BT_CMD_SET_BT_LOCAL_NAME,value)==0)
    {
        return BT_RET_SUCCESS;
    }
    else
...
static int bt_ad6956f_set_action(bt_ad6956f_cmd_sends_e cmd, unsigned char *send_buf)
{
    ...
    if(cmd!=BT_CMD_SET_BT_POWER_ON || cmd != BT_CMD_SET_POWER_ON_TO_TX_NOT_ACTION)
    {
    ...
    switch (cmd)
    {
        case BT_CMD_SET_BT_POWER_ON:
            ...
            break;
        case BT_CMD_SET_POWER_ON_TO_TX_NOT_ACTION:
            body_m.cmd_id=0x10;
            body_m.cmd_len=0x00;           
            if(gbt->get_dev_status<EBT_DEVICE_STATUS_WORKING_CONNECTED)
            {
            ...
            break;
         case BT_CMD_SET_CHANNEL_INPUT_SELECT:
            body_m.cmd_id=0x11;
            body_m.cmd_len=0x01;         
            memcpy(body_m.cmd_value,send_buf,1);            
         break;
         case BT_CMD_SET_VOLUME_LEVEL:
            body_m.cmd_id=0x0E;
            body_m.cmd_len=0x01;         
            memcpy(body_m.cmd_value,send_buf,1);   
         break;
         case BT_CMD_SET_BT_CTRL_CMD:
            body_m.cmd_id=0x4A;
            body_m.cmd_len=0x01;         
            memcpy(body_m.cmd_value,send_buf,1);   
         break;
        case BT_CMD_SET_BT_LOCAL_NAME:
            if(send_buf==NULL)goto error;
            printf("--\n--------------cmd_len = %ld \n",sizeof(sBtname));
            memcpy(body_m.cmd_value,send_buf,sizeof(sBtname));
            body_m.cmd_id=0xa1;
            body_m.cmd_len=sizeof(sBtname);
            break;
        case BT_CMD_GET_BT_LOCAL_NAME:
            body_m.cmd_id=0xa2;
            body_m.cmd_len=0x00;
            break;
        ...
        case BT_CMD_SET_POWER_ON_TO_RX:
            body_m.cmd_id=0x40;
            body_m.cmd_len=0x00;
        printf("Line:0x40; cmds :BT_CMD_SET_POWER_ON_TO_RX\n");
            break;
        case BT_CMD_SET_POWER_ON_TO_TX:
            body_m.cmd_id=0x41;
        printf("Line:0x41; cmds :BT_CMD_SET_POWER_ON_TO_TX\n");
            body_m.cmd_len=0x00;
            break;
            ...
            if (ret != BT_RET_EXIT)
            {
                switch (body_m.cmd_id)
                {
                    case 0x0A:
						...
						else if(body_m.cmd_value[0]==0x00)
                        {
                    ...
                    case 0xa7:
						...
						printf("=============== **%s** ==================\n",connet_data->name);
                        ba_dev_name = malloc(BLUETOOTH_NAME_LEN + 1);
                        strcpy(ba_dev_name, connet_data->name);
                        ba_connect = 1;
						break;  
                    case 0xa8:
						free(ba_dev_name);
					...
					case 0xa3:  
                        //printf("========0xa3========\n");
                        for(int i=0;i<body_m.cmd_len;i++){
                        gBtName[i]= body_m.cmd_value[i];               
                        printf("****************************%c************************\n",gBtName[i]);
                        }
                        printf("\n");
                            //if (memcmp(&sBtname,&sBtname,body_m.cmd_len)==0) //zhp test
                            if (strcmp(sBtname,gBtName) == 0) 
                            printf("***************OK\n");
                            else {
                                printf("change BT local name: \n");
                                bt_set_local_name(sBtname);
                                }
                        break;
                    case 0x0c:
                        memset(acq_dev_info, 0, sizeof(struct bluetooth_slave_dev));
...
static void printf_bt_ad6956f_dev_status(bt_ad6956f_cmd_sends_e status)
{
    switch (status)
    {
        case EBT_DEVICE_STATUS_NOWORKING_DEFAULT:
            log_d("EBT_DEVICE_STATUS_NOWORKING_DEFAULT\n");
            break;
        case EBT_DEVICE_STATUS_NOWORKING_STATUS_ERROR:
            log_d("EBT_DEVICE_STATUS_NOWORKING_STATUS_ERROR\n");
            break;
        case EBT_DEVICE_STATUS_NOWORKING_NOT_EXISTENT:
            log_d("EBT_DEVICE_STATUS_NOWORKING_NOT_EXISTENT\n");
            break;
        case EBT_DEVICE_STATUS_NOWORKING_BT_POWER_OFF:
            log_d("EBT_DEVICE_STATUS_NOWORKING_BT_POWER_OFF\n");
            break;
        case EBT_DEVICE_STATUS_WORKING_EXISTENT:
            log_d("EBT_DEVICE_STATUS_WORKING_EXISTENT\n");
            break;
        case EBT_DEVICE_STATUS_WORKING_INQUIRY_DATA_SEARCHED:
            log_d("EBT_DEVICE_STATUS_WORKING_INQUIRY_DATA_SEARCHED\n");
            break;
        case EBT_DEVICE_STATUS_WORKING_INQUIRY_STOP_SEARCHING:
            log_d("EBT_DEVICE_STATUS_WORKING_INQUIRY_STOP_SEARCHING\n");
            break;
        case EBT_DEVICE_STATUS_WORKING_DISCONNECTED:
            log_d("EBT_DEVICE_STATUS_WORKING_DISCONNECTED\n");
            break;
        case EBT_DEVICE_STATUS_WORKING_CONNECTED:
            log_d("EBT_DEVICE_STATUS_WORKING_CONNECTED\n");
            break;
        case EBT_DEVICE_STATUS_WORKING_GET_CONNECTED_INFO:
            log_d("EBT_DEVICE_STATUS_WORKING_GET_CONNECTED_INFO\n");
            break;
        default:
            log_d("other\n");
            break;
...
{
    if(body==NULL)return;
    #if 1//(ELOG_OUTPUT_LVL>=ELOG_LVL_DEBUG)//zhp 20230321    
    printf("ad6956 a cmds : %02x %02x %02x %02x %02x %02x %02x ",(body->frame_head>>8),(body->frame_head&0x00ff),(body->frame_len&0x00ff),(body->frame_len>>8),body->frame_id,body->cmd_id,body->cmd_len);
    for(int i=0;i<body->cmd_len;i++)
    {
        printf("%02x ",body->cmd_value[i]);
    }
