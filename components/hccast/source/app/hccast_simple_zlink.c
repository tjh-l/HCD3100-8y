...
int zlink_test_main_audio_start_f(enum MEDIA_TYPE media_type, void *user_data)
{
    ...
    memset(&cfg, 0, sizeof(struct audio_config));
	cfg.codec_id = HC_AVCODEC_ID_PCM_S16LE;
    cfg.sync_mode = AVSYNC_TYPE_FREERUN;
    cfg.bits_per_coded_sample = 16;
    cfg.channels = 2;
    cfg.bit_rate = 0;
    cfg.sample_rate = 48000;
    cfg.block_align = 0;
    cfg.extradata_size = 0;
    cfg.audio_flush_thres = 200;


    g_auddec_fd = open("/dev/auddec", O_RDWR);
    if (g_auddec_fd < 0)
    {
    ...
    if (ioctl(g_auddec_fd, AUDDEC_INIT, &cfg) != 0)
    {
    ...
    ioctl(g_auddec_fd, AUDDEC_START, 0);
...
{
    printf("[%s - %d] %d %d %d %d\n", __func__, __LINE__, len, sample, channels, bits);
    ...
    pkthd.flag = AV_PACKET_ES_DATA;
    pkthd.pts = 0;
    if (write(g_auddec_fd, (uint8_t *)&pkthd, sizeof(AvPktHd)) != sizeof(AvPktHd))
    {
        printf("Write APktHd fail\n");
        
        return -1;
    }

    if (write(g_auddec_fd, data, len) != len)
    {
