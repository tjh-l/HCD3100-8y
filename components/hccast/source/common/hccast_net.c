...
int hccast_net_get_if_flags(char *ifname)
{
    int ret;
    short if_flags;
    struct ifreq ifr;
    ...
    if (ret == -1 && errno == 19)
    {
        hccast_log(LL_ERROR, "Interface %s : No such device.\n", ifname);
        close(sfd);
        return ret;
    }
    ...
    return (if_flags & 0xFF);
}

int hccast_net_get_mac_by_ifname(char *ifname, char *mac)
{
    int fd, rtn;
    struct ifreq ifr;
    ...
    {
        hccast_log(LL_ERROR, "SIOCSIFHWADDR");
    }
...
int hccast_net_set_mac_by_ifname(char *ifname, char *mac)
{
    int fd, rtn;
    struct ifreq ifr;
...
int hccast_net_set_dns(char *ifname, char *dns)
{
    ...
    else
    {
        ...
        else if (1 == ret)
        {
            ...
            {
                hccast_log(LL_ERROR, "write error\n");
            }
        }
        else if (2 == ret)
        {
...
int hccast_net_set_if_updown(const char *ifname, hccast_net_if_flag_e flag)
{
    ...
    if (flag == HCCAST_NET_IF_UP)
    {
        ifr.ifr_flags |= IFF_UP | IFF_RUNNING;
    }
    else if (flag == HCCAST_NET_IF_DOWN)
    {
        ifr.ifr_flags &= ~(IFF_UP | IFF_RUNNING);
    }
    else if (flag == HCCAST_NET_IF_UP_ETH)
    {
