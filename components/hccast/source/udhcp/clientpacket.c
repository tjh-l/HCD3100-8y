...
int send_discover(struct client_config_t *client_config, unsigned long xid, unsigned long requested)
{
    ...
    add_requests(&packet);
    LOG(LOG_DEBUG, "ifindex(%d) Sending discover...", client_config->ifindex);
...
int get_raw_packet(struct dhcpMessage *payload, int fd)
{
    ...
    bytes = recvfrom(fd, temp_buf, sizeof(temp_buf), MSG_DONTWAIT, NULL, NULL);
    if (bytes < 0)
    {
    ...
    memcpy(&packet, temp_buf, sizeof(struct udp_dhcp_packet));
