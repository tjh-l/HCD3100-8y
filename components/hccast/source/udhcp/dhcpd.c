...
static void* udhcpd_main(void* arg)
{
    ...
    if (UDHCP_IF_WLAN0 == udhcpd_conf->ifname)
    {
...
#endif
        //! g_hs_server_config save ptr, next load config will free. dont memset
        ...
        if (strlen(udhcpd_conf->subnet_mask_def))
        {
            ret &= read_ip(udhcpd_conf->subnet_mask_def, &g_hs_server_config.subnet_mask);
        }
    ...
    else if (UDHCP_IF_P2P0 == udhcpd_conf->ifname)
    {
...
#endif
        //! g_p2p_server_config save ptr, next load config will free. dont memset
        ...
        if (strlen(udhcpd_conf->subnet_mask_def))
        {
            ret &= read_ip(udhcpd_conf->subnet_mask_def, &g_p2p_server_config.subnet_mask);
        }
    ...
    if (UDHCPC_HOTSPOT_EN & udhcpd_conf->option)
    {
        hotspot_en = true;
    }
    ...
    while (udhcpd_conf->run)  /* loop until universe collapses */
    {
        ...
        {
            tv.tv_sec = 0; //timeout_end - time(0);
            tv.tv_usec = 200 * 1000;
        }

        if (!server_config->auto_time || tv.tv_sec > 0 || tv.tv_usec > 0)
        {
        ...
        switch (state[0])
        {
            case DHCPDISCOVER:
                ...
                if (udhcpd_conf->priv.func)
                {
                    ret = (udhcpd_conf->priv.func)(UDHCP_EVENT_CUSTOM_HOTSPOT, NULL, &dns_ips[0]);
                }

                if (ret > DNS_NUM)
                    server_config->dns[DNS_NUM] = DNS_NUM;
                else
                    server_config->dns[DNS_NUM] = ret;

                memcpy(server_config->dns, dns_ips, server_config->dns[DNS_NUM] * sizeof(uint32_t));
            ...
            case DHCPREQUEST:
                ...
                printf("received REQUEST requested %lx server_id = %p \n",
                ...
                if (lease)   /*ADDME: or static lease */
                {
                    if (server_id)
                    {
                        ...
                        if (server_id_align == server_config->server && requested &&
                            requested_align == lease->yiaddr)
                        {
                            retval = sendACK2(&packet, lease->yiaddr, server_config, hotspot_en);
                ...
                else if (requested)
                {
                    ...
                    if ((lease = find_lease_by_yiaddr(requested_align, server_config)))
                    {
                        if (lease_expired(lease))
                        {
                            ...
                            /* use it now. */
                            sendACK2(&packet, lease->yiaddr, server_config, hotspot_en);
                        }
                        else 
                        {
                            /* someone is using. */
                    ...
                    {
                        /* Let client to use the ip. */
                        sendACK2(&packet, requested_align, server_config, hotspot_en);
                    }
                }
                else
                {
...
int udhcpd_start(udhcp_conf_t *conf)
{
    ...
    if (conf && 0 == conf->run)
    {
        ...
        if (pthread_create((pthread_t*)&conf->pid, &attr, (void*)udhcpd_main, conf) != 0)
        {
