...
static hccast_media_type_e get_media_type(const char *uri, const char *meta)
{
    ...
    else if (strstr(meta, "<upnp:class>") != NULL)
    {
        char *p = strstr(meta, "object.item.");
        memset(meta_str, '\0', 32);

        // <upnp:class>object.item.xxx</upnp:class>
        if (sscanf(p, "object.item.%30[^<]", meta_str) == 1)
        {
            if (strcasestr(meta_str, "imageItem")) // object.item.imageItem.photo
            {
                type = HCCAST_MEDIA_PHOTO;
            }
            else if (strcasestr(meta_str, "videoItem")) // object.item.videoItem
            {
                type = HCCAST_MEDIA_MOVIE;
            }
            else if(strcasestr(meta_str, "audioItem")) // object.item.audioItem.musicTrack
            {
                type = HCCAST_MEDIA_MUSIC;
            }
            else
...
static int output_ffplayer_is_valid_ip(const char *str) 
{
    int dots = 0;
    ...
    return dots == 3 && str[0] != '.' && str[strlen(str)-1] != '.';
}

//URL format: scheme://host:port/path
static int output_ffplayer_check_url_legal(char* url)
{
    int ret = 0;
    int port_num = 80;
    ...
    {	
        ...
        //step3: check ip valid.
        if (!output_ffplayer_is_valid_ip(server_ip))
        {
            return -1;
        }
...
{
    int hostap_en = 0;
    hccast_log(LL_NOTICE,"[DLNA][Set uri]to '%s', meta_cb: 0x%08lx\n", uri, (uint32_t)meta_cb);
...
static int output_ffplayer_get_volume(int *v)
{
    if (dlna_callback)
    {
        dlna_callback (HCCAST_DLNA_GET_SAVE_AUDIO_VOL, (void*)&m_volume, NULL);
    }
...
static int output_ffplayer_get_mute(int *m)
{
    ...
    if (dlna_callback)
    {
        dlna_callback (HCCAST_DLNA_GET_SAVE_AUDIO_VOL, (void*)&vol, NULL);
    }
