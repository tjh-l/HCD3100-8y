...
#define FREQ_TO_CHANNEL_24G(freq) (freq - CHANNEL_1_FREQ) / 5
#define FREQ_TO_CHANNEL_5G(freq) (freq - CHANNEL_36_FREQ) / 5

#define CHANNEL_24G_TO_FREQ(channel) ((channel-1)*5+CHANNEL_1_FREQ)
...
int wifi_ctrl_is_connecting(void);
int wifi_ctrl_is_disconnecting(void);
int wifi_ctrl_is_scanning(void);
int wifi_ctrl_is_busy(void);
...
bool wifi_ctrl_is_initted(void);

int wifi_ctrl_init(hccast_wifi_mgr_event_callback func);

int wifi_ctrl_uninit(void);
