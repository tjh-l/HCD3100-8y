...
static int p2p_ctrl_wext_p2p_add_network(bool isAll, p2p_network_list_st *list, p2p_network_st *network)
{
    ...
    if (isAll)
    {
        for (int i = 0; i < P2P_MAX_NETWORK_LIST; i++)
        {
            ...
            sprintf(profile_bssid, "profilefound=%d%s%s%ld%s", 1, list->network[i].bssid, list->network[i].mac, strlen(list->network[i].ssid), list->network[i].ssid);
    ...
    else
    {
        ...
        char profile_bssid[256] = {0};
        sprintf(profile_bssid, "profilefound=%d%s%s%ld%s", 1, network->bssid, network->mac, strlen(network->ssid), network->ssid);
...
int p2p_ctrl_wext_device_power_off(void)
{
    ...
    ret = iwpriv_cmd(argc, argv, NULL);

    if (-2 == ret)
    {
        hccast_log(LL_ERROR, "%s %d: ret %d\n", __func__, __LINE__, ret);
        ret = HCCAST_WIFI_ERR_CMD_IWPRIV_NO_DEVICE;
    }
    else if (ret)
    {
        hccast_log(LL_ERROR, "%s %d: ret %d\n", __func__, __LINE__, ret);
...
static udhcp_conf_t g_p2p_udhcpd_conf =
{
    .func   = udhcpd_cb,
...
static int p2p_ctrl_wext_udhcpc_start()
{
    udhcpc_start(&g_p2p_udhcpc_conf);
    return HCCAST_WIFI_ERR_NO_ERROR;
}

static int p2p_ctrl_wext_udhcpc_stop()
{
...
static int p2p_ctrl_p2p_add_group(int group_id)
{
    int ret = HCCAST_WIFI_ERR_NO_ERROR;
    char reply[1024] = {0};
    unsigned int len = sizeof(reply) - 1;
...
static int p2p_ctrl_p2p_remove_network()
{
    char *argv[5];
    int argc;
...
void p2p_ctrl_wext_device_init(void)
{
    ...
    //p2p_ctrl_wext_wpas_attr_init(); ref p2p_ctrl_wext_set_enable
}
...
static int p2p_ctrl_get_status(void)
{
    char *argv[10];
    int argc;
    char *resp = calloc(BUFFER_LEN, sizeof(char));
...
bool p2p_ctrl_wext_set_enable(bool enable)
{
    ...
    if (enable)
    {
        ...
        p2p_ctrl_wext_p2p_add_network(true, &g_p2p_network_list, NULL);
    }
    else
    {
...
int p2p_ctrl_wext_set_channel_fuse(unsigned int bitmap)
{
    ...
    // argc = 2
    argv[argc++] = "p2p_set";
    sprintf(cmd, "invaild_op_ch_list=%d", bitmap);
    argv[argc++] = cmd;
    iwpriv_cmd(argc, argv, NULL);
    usleep(1000);
...
// rtos
static void *p2p_ctrl_wext_thread(void *arg)
{
    ...
    while (g_p2p_thread_running)
    {
        if (hccast_mira_get_stat())
        {
            ...
            if (p2p_state_last != p2p_state_curr)
            {
                ...
                if (P2P_STATE_GONEGO_ING == p2p_state_curr || P2P_STATE_RECV_INVITE_REQ_DISMATCH == p2p_state_curr)   /* P2P_STATE_GONEGO_ING = 9*/
                {
                    g_mira_param.state_update_func(HCCAST_P2P_STATE_CONNECTING);
                ...
                {
                    //wait 100 ms for driver xmit frame success
                    usleep(200*1000);
                    g_mira_param.state_update_func(HCCAST_P2P_STATE_CONNECTING);
                    ...
                    if (P2P_STATE_RECV_INVITE_REQ_MATCH == p2p_state_curr)
                    {
                        ...
                        for (int i = 0; i < P2P_MAX_NETWORK_LIST; i++)
                        {
                            if (!memcmp(g_p2p_network_list.network[i].ssid, ssid, sizeof(g_p2p_network_list.network[i].ssid))\
                                    && !strncasecmp(g_p2p_network_list.network[i].bssid+3, bssid+3, sizeof(g_p2p_network_list.network[i].bssid)-3))
                            {
                                found_id = i;
                                break;
                            }
                    ...
                    {
                        char ssid[256] = {0}, bssid[128]={0}, mac[128] = {0};

                        p2p_ctrl_get_peer_ifa(ssid, 256, bssid, 128, mac, 128);;
#ifdef HC_RTOS
                        wifi_ctrl_p2p_wps_pbc_ex(bssid, mac);
#endif
                ...
                else if (g_p2p_wps_pbc && !g_p2p_connected && (P2P_STATE_PROVISIONING_DONE == p2p_state_curr))     /* P2P_STATE_PROVISIONING_DONE = 14 */
                {
                    g_mira_param.state_update_func(HCCAST_P2P_STATE_CONNECTED);
                    g_p2p_connected = true;
                    if (2 == p2p_ctrl_wext_device_is_go())
                    {
                        p2p_ctrl_wext_udhcpc_stop();
                        p2p_ctrl_wext_udhcpc_start();
                    }
                    ...
                    || (g_p2p_wps_pbc && P2P_STATE_RECV_INVITE_CONNECT_FAIL == p2p_state_curr))
                {
                    g_p2p_wps_pbc = false;
                    g_p2p_connected = false;
                    p2p_ctrl_wext_set_enable(false);
                    g_mira_param.state_update_func(HCCAST_P2P_STATE_CONNECT_FAILED);
...
int p2p_ctrl_wext_init(p2p_param_st *params)
{
    ...
    if (pthread_create(&g_p2p_tid, &attr, p2p_ctrl_wext_thread, NULL) != 0)
    {
