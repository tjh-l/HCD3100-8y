...
{
    if (index < 0 || index >= sizeof(hccast_wifi_encrypt_str) / sizeof(hccast_wifi_encrypt_str[0]))
    {
        return NULL;
    }
...
static void http_response_json(int client, unsigned char *json)
{
    ...
    char buf[BUF_LEN];
    snprintf(buf, BUF_LEN, "Content-Length: %ld\r\n", strlen(json));
    send(client, buf, strlen(buf), 0);
    snprintf(buf, BUF_LEN, "Connection: Close\r\n");
    send(client, buf, strlen(buf), 0);
    sprintf(buf, "\r\n");
    send(client, buf, strlen(buf), 0);
...
static int web_interface_setting(int client, char *param)
{
    ...
    else if (!strcmp(key, "airp2p_switch_method"))
    {
        if (!strcmp(value, "1"))
        {
            temp = 1;
            if (0 != httpd_callback(HCCAST_HTTPD_SET_AIRP2P_SWITCH_MODE, (void *)temp, NULL))
            {
                return -1;
            }
        }
    ...
    else if (!strcmp(key, "airp2p_channel"))
    {
        temp = atoi(value);
        httpd_callback(HCCAST_HTTPD_SET_AIRP2P_CHANNEL, (void *)temp, NULL);
    }
    else
    {
...
static int web_interface_connect(int client, char *param)
{
    ...
    for (int i = 0; i < sizeof(hccast_wifi_encrypt_str)/sizeof(hccast_wifi_encrypt_str[0]); i++)
    {
...
static int web_interface_upgrade(int client, char *param)
{
    ...
    httpd_callback(HCCAST_HTTPD_START_ONLINE_UPGRADE, (void *)deurl, NULL);
...
static int web_interface_upgrade_interrupt(int client, char *param)
{
    ...
    ret = httpd_callback(HCCAST_HTTPD_MSG_USER_UPGRADE_ABORT, NULL, NULL);
...
static int web_interface_get_info(int client, char *param)
{
    ...
    int not_4k = 0;
    int airp2p_switch_mode = 0;
    int mirror_mode_en = 0;
    int resolution_en = 0;
    int airp2p_channel = 0;
    //1.0.0-150000
    ...
    airp2p_switch_mode = httpd_callback(HCCAST_HTTPD_GET_AIRP2P_SWITCH_MODE, NULL, NULL);
    resolution_en = httpd_callback(HCCAST_HTTPD_GET_RESOLUTION_CONFIG_EN, NULL, NULL);
    mirror_mode_en = httpd_callback(HCCAST_HTTPD_GET_MIRROR_MODE_CONFIG_EN, NULL, NULL);
    airp2p_channel = httpd_callback(HCCAST_HTTPD_GET_AIRP2P_CHANNEL, NULL, NULL);
    ...
    if ((airp2p_switch_mode == 1) || (airp2p_switch_mode == 2))
    {
        cJSON_AddItemToObject(obj, "airp2p_switch_en", cJSON_CreateString("enabled"));
        cJSON_AddItemToObject(obj, "airp2p_switch_method", cJSON_CreateNumber(airp2p_switch_mode));
    }
    else
    {
        cJSON_AddItemToObject(obj, "airp2p_switch_en", cJSON_CreateString("disenabled"));
    }

    if (airp2p_channel > 0)
    {
        cJSON_AddItemToObject(obj, "airp2p_channel_en", cJSON_CreateString("enabled"));
        cJSON_AddItemToObject(obj, "airp2p_channel", cJSON_CreateNumber(airp2p_channel));
    }
    else
    {
        cJSON_AddItemToObject(obj, "airp2p_channel_en", cJSON_CreateString("disenabled"));
    }

    if (resolution_en == -1)
    {
        cJSON_AddItemToObject(obj, "resolution_en", cJSON_CreateString("disenabled"));
    }
    else
    {
        cJSON_AddItemToObject(obj, "resolution_en", cJSON_CreateString("enabled"));
    }

    if (mirror_mode_en == -1)
    {
        cJSON_AddItemToObject(obj, "mirror_model_en", cJSON_CreateString("disenabled"));
    }
    else
    {
        cJSON_AddItemToObject(obj, "mirror_model_en", cJSON_CreateString("enabled"));
    }    
...
static int web_interface_get_wifi(int client, char *param)
{
    ...
    wifi_connected = httpd_callback(HCCAST_HTTPD_GET_CUR_WIFI_INFO, (void *)&save_ap, NULL);
    ...
    if (scan_res->ap_num > 0)
    {
        ...
        //filter same ssid and WPA3(RTOS).
        for (i = 0; i < scan_res->ap_num; i++)
        {
            for (j = 0; j < i; j++)
            {
                if (strcmp(scan_res->apinfo[i].ssid, scan_res->apinfo[j].ssid) == 0)
                {
#ifdef __LINUX__
                    if (scan_res->apinfo[i].quality < scan_res->apinfo[j].quality)
#else
                    if (scan_res->apinfo[i].encryptMode != HCCAST_WIFI_ENCRYPT_MODE_WPA2PSK_SAE &&
                            scan_res->apinfo[j].encryptMode == HCCAST_WIFI_ENCRYPT_MODE_WPA2PSK_SAE)
                        strcpy(scan_res->apinfo[j].ssid, "");
                    else if (scan_res->apinfo[i].encryptMode == HCCAST_WIFI_ENCRYPT_MODE_WPA2PSK_SAE &&
                            scan_res->apinfo[j].encryptMode != HCCAST_WIFI_ENCRYPT_MODE_WPA2PSK_SAE)
                        strcpy(scan_res->apinfo[i].ssid, "");
                    else if (scan_res->apinfo[i].quality < scan_res->apinfo[j].quality)
#endif
                        strcpy(scan_res->apinfo[i].ssid, "");
                    else
                        strcpy(scan_res->apinfo[j].ssid, "");
        ...
        if (wifi_connected == true)
        {
            strncpy(cur_ssid, save_ap.ssid, WIFI_MAX_SSID_LEN - 1);
            ++j;
            ap_obj = cJSON_CreateObject();
            cJSON_AddItemToObject(ap_obj, "index", cJSON_CreateNumber(j));
            cJSON_AddItemToObject(ap_obj, "ssid", cJSON_CreateString(cur_ssid));
            cJSON_AddItemToObject(ap_obj, "state", cJSON_CreateNumber(1));
            cJSON_AddItemToObject(ap_obj, "saved", cJSON_CreateTrue());
            cJSON_AddItemToObject(ap_obj, "intensity", cJSON_CreateNumber(save_ap.quality));

            if (save_ap.encryptMode)
            {
            ...
            if (wifi_page_get_encrypt_str(save_ap.encryptMode))
            {
                cJSON_AddItemToObject(ap_obj, "encryption_type", cJSON_CreateString(wifi_page_get_encrypt_str(save_ap.encryptMode)));
            }
            else
            {
        ...
        for (i = 0; i < scan_res->ap_num; i++)
        {
            ...
            if (strlen(scan_res->apinfo[i].ssid) > 0)
            {
                ...
                cJSON_AddItemToObject(ap_obj, "index", cJSON_CreateNumber(j));//for index.
    ...
    {
        hccast_log(LL_NOTICE, "ap_num is 0\n");
        ret = -1;
    }
    ...
    g_scan_count--;
    hccast_log(LL_NOTICE, "g_scan_count:%d\n", g_scan_count);
...
static int web_interface_upload_submit(int client, char *param)
{
    ...
    hccast_web_upload_info_st usr_upg_info;

    usr_upg_info.buf = NULL;
    usr_upg_info.length = length;
    httpd_callback(HCCAST_HTTPD_ALLOC_UPGRADE_BUFF, (void*)(&usr_upg_info), NULL);
    if (usr_upg_info.buf)
        buf = usr_upg_info.buf;
    else
    ...
    while (recv_len < length)
    {
        ...
        if (ret <= 0)
        {
            ...
            if (usr_upg_info.buf)
                httpd_callback(HCCAST_HTTPD_FREE_UPGRADE_BUFF, &usr_upg_info, NULL);
            else
                free(buf);
...
static int web_interface_wechat_connect_wifi(int client, char *param)
{
    int ret = 0;
    bool wifi_connected = 0;
    hccast_wifi_ap_info_t con_ap;
    ...
    while (token != NULL)
    {
        if ((str = strstr(token, "bssid=")) != NULL)
        {
            len = strlen("bssid=");
            str += len;
            urldecode(str, bssid);
...
static web_interface_st g_interface[] =
{
    ...
    {"wifi_connect", web_interface_wechat_connect_wifi},
};
