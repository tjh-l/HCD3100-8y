...
static char m_connecting_ssid[WIFI_MAX_SSID_LEN] = {0};
static int m_probed_wifi_module = 0;
static int m_hostap_connect_count = 0;
static volatile bool m_hostap_discover_ok = false;
static volatile bool m_network_cur_state = true;
static volatile bool m_factary_init = false;
static volatile bool m_network_hotspot_status = false;
static volatile bool m_airp2p_state = false;
static volatile bool m_wifi_resetting = 0;
static volatile bool m_service_enable = true; /*check if the network service can be start freedom.*/
...
int app_get_wifi_connect_status()
{
    ...
    status = m_wifi_config.wifi_connected;
...
{
    pthread_mutex_lock(&m_wifi_state_mutex);
    m_wifi_config.wifi_connected = !!status;
    pthread_mutex_unlock(&m_wifi_state_mutex);
}
...
static int wifi_mgr_callback_func(hccast_wifi_event_e event, void *in, void *out)
{
    ...
    case HCCAST_WIFI_CONNECT:
    {
        ...
        if (in != NULL)
        {
            memcpy(m_connecting_ssid, (char *)in, strlen((char *)in));
        }
    ...
    case HCCAST_WIFI_CONNECT_RESULT: //station
    {
        ...
        if (result)
        {
            ...
            else
            {
                ...
                memset(m_connecting_ssid, 0, sizeof(m_connecting_ssid));
                memset(m_wifi_config.local_ip, 0, sizeof(m_wifi_config.local_ip));
...
static void *wifi_connect_thread(void *args)
{
    ...
    memcpy(m_connecting_ssid, ap_wifi->ssid, sizeof(m_connecting_ssid));

#ifdef HOTSPOT_SUPPORT
    hotspot_stop(NETWORK_WIFI_IFACE_NAME, NETWORK_ETH0_IFACE_NAME);
    m_network_hotspot_status = false;
#endif
    ...
    else
    {
        ...
        usleep(50 * 1000);
        control_msg_t ctl_msg = {0};
        ctl_msg.msg_type = MSG_TYPE_NETWORK_WIFI_CONNECT_FAIL;//MSG_TYPE_NETWORK_WIFI_DISCONNECTED;
        api_control_send_msg(&ctl_msg);
...
{
    hccast_wifi_ap_info_t *wifi_info = (hccast_wifi_ap_info_t*)arg;
    hccast_wifi_ap_info_t conn_wifi_ap;
    control_msg_t ctl_msg = {0};
    app_set_wifi_connect_status(0);
    ...
    app_log(LL_NOTICE, "reconnect ssid:%s, pwd:%s\n", wifi_info->ssid, wifi_info->pwd);
    ...
    memcpy(&conn_wifi_ap, wifi_info, sizeof(hccast_wifi_ap_info_t));
    if (HCCAST_NET_WIFI_ECR6600U == hccast_wifi_mgr_get_wifi_model() \
        && conn_wifi_ap.encryptMode != HCCAST_WIFI_ENCRYPT_MODE_OPEN_WEP \
        && conn_wifi_ap.encryptMode != HCCAST_WIFI_ENCRYPT_MODE_SHARED_WEP \
        )
    ...
    {
        app_log(LL_NOTICE, "connect timeout!\n");
EXIT:
        m_wifi_config.bReconnect = false;
        memset(m_wifi_config.local_ip, 0, MAX_IP_STR_LEN);
        memset(m_connecting_ssid, 0, sizeof(m_connecting_ssid));
...
{
    hccast_wifi_ap_info_t *cur_ap = NULL;
    app_data_t  *app_data = data_mgr_app_get();
    pthread_t thread_id = 0;
    pthread_attr_t attr;

    pthread_mutex_lock(&m_wifi_state_mutex);
    if (m_wifi_config.wifi_connecting || network_get_airp2p_state())
    {
        pthread_mutex_unlock(&m_wifi_state_mutex);
        return 1;
    }
    ...
    if (!wifi_ap)
    {
        if (strlen(app_data->wifi_ap[0].ssid) > 0)
        {
            cur_ap = &app_data->wifi_ap[0];
        }
        else
        {
            m_wifi_config.wifi_connecting = false;
    ...
    if (pthread_create(&thread_id, &attr, wifi_reconnect_thread, (void*)cur_ap) != 0)
    {
        m_wifi_config.wifi_connecting = false;
        m_wifi_config.bReconnect = false;
        pthread_mutex_unlock(&m_wifi_state_mutex);
...
static int httpd_callback_func(hccast_httpd_event_e event, void *in, void *out)
{
    ...
    case HCCAST_HTTPD_WIFI_AP_DISCONNECT:
    {
        ...
        pthread_attr_setdetachstate(&attr, PTHREAD_CREATE_DETACHED);
        memset(m_connecting_ssid, 0, sizeof(m_connecting_ssid));
        pthread_create(&tid, &attr, wifi_disconnect_thread, NULL);
        pthread_attr_destroy(&attr);
        break;
    }
    ...
    case HCCAST_HTTPD_WIFI_AP_CONNECT:
    {
        ...
        if (con_ap)
        {
            ...
            if (ap_wifi)
            {
                ...
                pthread_attr_setdetachstate(&attr, PTHREAD_CREATE_DETACHED);
                memcpy(ap_wifi, con_ap, sizeof(hccast_wifi_ap_info_t));
                memcpy(m_connecting_ssid, con_ap->ssid, sizeof(m_connecting_ssid));
    ...
    case HCCAST_HTTPD_GET_WIFI_FREQ_MODE_EN:
    {
        ...
        int model = hccast_wifi_mgr_get_wifi_model();
        if (HCCAST_NET_WIFI_8800D == model)
        {
            ret = 0;
        }
        else if (model)
    ...
    case HCCAST_HTTPD_WIFI_SCAN:
        ...
        if (scan_res)
        {
            m_wifi_config.wifi_scanning = true;
            hccast_wifi_mgr_scan(scan_res);
            m_wifi_config.wifi_scanning = false;
        }
        else
        {
    ...
    {
        ret = app_data->airp2p_ch;
        break;
    }
    case HCCAST_HTTPD_SET_AIRP2P_CHANNEL:
    {
        temp = (int)in;
        if (app_data->airp2p_ch != temp)
        {
            app_data->airp2p_ch = temp;
            data_mgr_save();
        }
        break;
    }
...
{
    return m_hostap_connect_count;
}

static void hostap_set_connect_count(int count)
{
    m_hostap_connect_count = count;
}
...
static int hostap_connect_update(void)
{
    ...
    if ((hccast_wifi_mgr_get_hostap_status() == 0) || hccast_wifi_mgr_p2p_get_connect_stat() \
        || m_factary_init || network_get_airp2p_state())
    {
        needfresh = 0;
...
static void *hostap_connect_thread(void *args)
{
    ...
    while (1)
    {
        ...
        if (cast_get_p2p_switch_enable() && !m_wifi_resetting && cast_detect_p2p_exception())
        {
            ...
            m_wifi_resetting = 1;
            ctl_msg.msg_type = MSG_TYPE_AIRP2P_RESET;
            api_control_send_msg(&ctl_msg); 
        }
        api_sleep_ms(500);
    }
...
#ifdef HOTSPOT_SUPPORT
static int hotspot_dhcpc_update_cb(unsigned int event, void *in, void *out);
static void hotspot_dhcpc_cb(unsigned int data);

static udhcp_conf_t g_eth_udhcpc_conf =
{
    .func   = hotspot_dhcpc_cb,
    .ifname = UDHCP_IF_NONE,
    .pid    = 0,
    .run    = 0,
    .option = UDHCPC_HOTSPOT_EN,
    //.priv.func = hotspot_dhcpc_update_cb,
};

int app_hotspot_start()
{
    hccast_net_set_if_updown(NETWORK_ETH0_IFACE_NAME, HCCAST_NET_IF_DOWN);
    usleep(1 * 1000);
    hccast_net_set_if_updown(NETWORK_ETH0_IFACE_NAME, HCCAST_NET_IF_UP_ETH);
    usleep(1 * 1000);
    hotspot_start(NETWORK_WIFI_IFACE_NAME, NETWORK_ETH0_IFACE_NAME);

    udhcpc_stop(&g_eth_udhcpc_conf);
    udhcpc_start(&g_eth_udhcpc_conf);
}
...
{
    return m_network_hotspot_status;
}

static bool hotspot_update(char *ifname, short *oflags)
{
    short flags;
    bool ret = false;
    *oflags &=(IFF_RUNNING | IFF_UP);

    flags = hccast_net_get_if_flags(ifname);
    if(flags < 0)
    {
        return false;
    }

    flags &=(IFF_RUNNING | IFF_UP);
    if(*oflags != flags)
    {
        app_log(LL_NOTICE, "flags: %2x, oflags: %02x", flags, *oflags);
        ret = true;
    }
...
static int hotspot_dhcpc_update_cb(unsigned int event, void *in, void *out)
{
    ...
    switch (event)
    {
        case UDHCP_EVENT_CUSTOM_HOTSPOT:
        {
            if (in && hotspot_update(NETWORK_ETH0_IFACE_NAME, in))
            {
                ret = 1;
                hotspot_start(NETWORK_WIFI_IFACE_NAME, NETWORK_ETH0_IFACE_NAME);
            }
...
static int hotspot_dhcpd_update_cb(unsigned int event, void *in, void *out)
{
    int ret = 0;
    ...
    switch (event)
    {
        case UDHCP_EVENT_CUSTOM_HOTSPOT:
        {
            if (out)
            {
                ret = hotspot_get_ip4_dns((uint32_t*)out, 8);
            }
...
static void hotspot_dhcpc_cb(unsigned int data)
{
    hccast_udhcp_result_t *result = (hccast_udhcp_result_t *) data;
    if (result->stat)
    {
        m_network_hotspot_status = true;
        struct in_addr ip_addr;
        uint32_t cnts = 0;
        uint32_t dns[sizeof(result->dns)/sizeof(uint32_t)];
        char *start = &result->dns[0];
        char dns_str[32];
        char *space;

        inet_aton(result->gw, &ip_addr);
        app_log(LL_NOTICE, "got peer ip: %s", inet_ntoa(ip_addr));
        hccast_net_ifconfig(result->ifname, result->ip, result->mask, result->gw);

        while(1)
        {
            space = strchr(start, ' ');
            if(space == NULL && !start[0])
            {
                break;
            }

            memset(dns_str, 0, sizeof(dns_str));
            strncpy(dns_str, start, space - start);
            app_log(LL_INFO, "dns_str: %s", dns_str);
            inet_pton(AF_INET, dns_str, &dns[cnts]);
            app_log(LL_INFO, "[%lu]: %08lx\n", cnts, dns[cnts]);
            cnts++;
            start = space + 1;
        }
        ...
        if(cnts > 0)
        {
            uint32_t dns_zero[2] = {0};
            inet_pton(AF_INET, "0.0.0.0", &dns_zero[0]);
            inet_pton(AF_INET, "0.0.0.0", &dns_zero[1]);
            hotspot_set_ip4_dns(dns_zero, 2);

            hotspot_set_ip4_dns(dns, cnts);
        }

        cast_restart_services();
    }
}

void *hotspot_hotplug_thread()
{
    short flags = 0;
    while (1)
    {
        if (hotspot_update(NETWORK_ETH0_IFACE_NAME, &flags))
        {
            if ((flags & IFF_RUNNING) && (flags & IFF_UP))
            {
                while(hccast_wifi_mgr_is_scaning() || hccast_wifi_mgr_is_connecting())
                {
                ...
                hccast_net_set_if_updown(NETWORK_ETH0_IFACE_NAME, HCCAST_NET_IF_DOWN);
                usleep(10 * 1000);
                hccast_net_set_if_updown(NETWORK_ETH0_IFACE_NAME, HCCAST_NET_IF_UP);
                usleep(100 * 1000);
                hotspot_start(NETWORK_WIFI_IFACE_NAME, NETWORK_ETH0_IFACE_NAME);
...
void app_hotspot_init()
{
    ...
    if (pthread_create(&pid, &attr, hotspot_hotplug_thread, NULL) < 0)
    {
        app_log(LL_ERROR,"Create hotspot_hotplug_thread error.");
    }
...
{
    static bool m_network_inited = false;
...
#ifdef HOTSPOT_SUPPORT
        .option             = UDHCPC_HOTSPOT_EN,
        .priv.func          = hotspot_dhcpd_update_cb,
#endif
    };
...
#ifdef HOTSPOT_SUPPORT
    uint8_t mac[6];
    if(hccast_net_get_mac_by_ifname(NETWORK_WIFI_IFACE_NAME, mac) < 0)
    {
        for(int i = 0; i< 6; i++)
    ...
    hccast_net_set_mac_by_ifname(NETWORK_ETH0_IFACE_NAME, mac);
    app_hotspot_init();
#endif
...
int network_airp2p_start(void)
{
    ...
    char hostname[64] = {0};
    app_data_t *app_data = data_mgr_app_get();
    int channel = app_data->airp2p_ch;
...
static volatile bool network_first_flag = true;
...
static void *network_connect_task(void *arg)
{
    ...
    if (data_mgr_wifi_ap_get(&wifi_ap))
    {
        ...
        app_wifi_switch_work_mode(WIFI_MODE_STATION);
        memcpy(m_connecting_ssid, wifi_ap.ssid, sizeof(m_connecting_ssid));
...
int network_plugout_reboot(void)
{
    ...
    pthread_attr_setstacksize(&attr, 0x2000);
    pthread_attr_setdetachstate(&attr, PTHREAD_CREATE_DETACHED); //release task resource itself
    pthread_create(&tid, &attr, network_plugout_reboot_task, NULL);
    pthread_attr_destroy(&attr);
}
