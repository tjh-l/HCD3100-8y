...
static void *airp2p_mode_switch_task(void *arg)
{
    ...
    while(1)
    {
        ...
        {
            if ((airp2p_mode_switch_status_check() == 0) && (api_get_key_switch_enable()))
            {
                ...
                if ((api_get_time_ms() - m_mode_switch_tick) > KEY_MODE_SWITCH_TIME)
                {
                    ...
                    if (data_mgr_cast_airp2p_get())
                    {
                        ...
                        msg.msg_type = MSG_TYPE_KEY_SWITCH_CAST_MODE;
...
static void key_cast_rotate(bool is_active)
{
    ...
    if (!api_get_key_switch_enable())
    {
        printf("%s(), line:%d. skip key rotate!\n", __func__, __LINE__);
        return ;
    }
