...
static int m_first_logo_show = 0;
...
static void win_cast_no_wifi_device_show()
{
    ...
    lv_obj_set_style_text_font(lv_obj_get_child(m_label_wifi_ssid, 0), FONT_SIZE_MID, 0);
...
static int win_cast_open(void *arg)
{
    ...
    api_set_aspect_mode(DIS_TV_16_9, DIS_NORMAL_SCALE, DIS_SCALE_ACTIVE_IMMEDIATELY);//16:9 as default.
    if (m_first_logo_show == 0)
    {
        if (!api_logo_state_get())
        {
            api_logo_show(NULL);
        }
        m_first_logo_show = 1;
...
#if 1//CASTING_CLOSE_FB_SUPPORT	    
    api_osd_show_onoff(true);
    api_dis_show_onoff(true);
#endif
...
static win_ctl_result_t win_cast_msg_proc(void *arg1, void *arg2)
{
    ...
    switch (ctl_msg->msg_type)
    {
        ...
        {
            win_msgbox_msg_open("Connect timeout or\npassword error", 5000, NULL, NULL);
            win_cast_connect_state_upate(false);
            break;
        }
        ...
        case MSG_TYPE_CAST_MIRACAST_CONNECTED:
            lv_label_set_text_fmt(m_label_state_msg, "Miracast is obtaining IP (%d)...", \
                hccast_wifi_mgr_get_current_freq_p2p());
            break;
        case MSG_TYPE_CAST_MIRACAST_GOT_IP:
            lv_label_set_text_fmt(m_label_state_msg, "Miracast is obtaining stream (%d) ...", \
                hccast_wifi_mgr_get_current_freq_p2p());
            break;
        ...
        case MSG_TYPE_KEY_SWITCH_CAST_MODE:
            ...
            cur_win = &g_win_cast_airp2p_root;
            cur_win->param = (void*)ctl_msg->msg_type;
            menu_mgr_pop_all();
            menu_mgr_push(cur_win);
            ret = WIN_CTL_PUSH_CLOSE;
...
bool win_cast_root_wait_open(uint32_t timeout)
{
    ...
    while(count--)
    {
        if (m_win_cast_open || !wifi_module)
            break;
        api_sleep_ms(20);
    }
