...
#define USB_UPG_FILE_WILDCARD	"HCFOTA"//"hc_upgrade"
...
static int sys_upg_usb_filepath_get(uint32_t timeout, char *file_path)
{
    ...
    while (1) {
        ...
        if (0 == strncasecmp(entry->d_name, USB_UPG_FILE_WILDCARD, strlen(USB_UPG_FILE_WILDCARD)))
        {
            ...
            {
                if(fw_header.ignore_version_check || 
                    (fw_header.version != local_version))
                {
...
static void *sys_upg_usb_task(void *arg)
{
    ...
    upg_buf = api_upgrade_buffer_alloc(file_len);    
    if (NULL == upg_buf){
...
usb_upg_ext:
    if (fp)
        fclose(fp);
    if(upg_buf)
        api_upgrade_buffer_free(upg_buf);
...
int sys_upg_flash_burn(char *buff, uint32_t length)
{
    ...
    ret = hcfota_url_checkonly(UPGRADE_APP_TEMP_OTA_BIN, 0);
#else
    ret = hcfota_memory_checkonly(buff, length, 0);
#endif

    if (0 == ret) {
    #ifdef APPMANAGER_SUPPORT //only support in linux
        ret = api_app_command_upgrade(UPGRADE_APP_TEMP_OTA_BIN);
        if (0 == ret){
