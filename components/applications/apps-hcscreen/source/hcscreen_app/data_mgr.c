...
static void data_mgr_default_init(void)
{
    ...
    m_app_data.browserlang = 1;//1-englist;2-chn;3-traditional chn
    m_app_data.mirror_frame = 1;
    m_app_data.mirror_mode = 1;
    m_app_data.aircast_mode = 1; //mirror-only.
    m_app_data.resolution = APP_TV_SYS_1080P;//APP_TV_SYS_AUTO;
    m_app_data.mirror_rotation = 0; //default disable.
    m_app_data.mirror_vscreen_auto_rotation = 1; //default enable.
    m_app_data.mirror_full_vscreen = 1; //default enable.
    m_app_data.mira_continue_on_error = 2; // show mosaic
    ...
    m_app_data.airp2p_en = 1;
    snprintf(m_app_data.airp2p_pin,MAX_PIN_LEN,"%s",AIRP2P_PIN);
    m_app_data.airp2p_mode_switch = 1; //default manual.
    m_app_data.airp2p_ch = 149; //default manual.
}
...
{
    m_app_data.airp2p_en = enable;
    data_mgr_item_save(offsetof(app_data_t, airp2p_en), 
        sizeof(m_app_data.airp2p_en), PERSISTENTMEM_NODE_ID_CASTAPP);
}

int data_mgr_cast_airp2p_get(void)
{
    return m_app_data.airp2p_en;
}
