...
#define MAX_FACTORYSET_CHANNEL 4
...
typedef struct
{
    ...
    int cur_index;
    play_info_t play_node[MAX_FACTORYSET_CHANNEL];
} play_info_data;
...
int _sys_get_sysdata_adc_adjust_value(uint8_t *value);
int _sys_set_sysdata_adc_adjust_value(uint8_t value);
int sys_data_get_media_play_num(void);
void sys_data_set_media_info(play_info_t media_info_data);
int sys_data_get_media_info(play_info_t *media_info_data);
void sys_data_set_media_play_cur_time(int type, int cut_time);
char *sys_data_get_last_play_name(int type);
// void sys_data_set_ebook_offset(ebook_page_info_t *media_info_data);
