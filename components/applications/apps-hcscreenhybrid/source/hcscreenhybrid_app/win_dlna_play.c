...
static int m_seek_position = 0;
static int m_seeking = 0;
...
#define STR_MP_UI_ERR       ("Playback Error (Code: %d-%d)")
#define STR_MP_UI_ERR_RETRY ("Playback Error,try again (Code: %d-%d)")
...
static void sec_timer_cb(lv_timer_t * t)
{
    ...
    if (m_seeking)
    {
        play_time = m_seek_position/1000;
    }
    else
    {
...
static void create_win_player(lv_obj_t *parent)
{
    ...
    else if (MSG_TYPE_CAST_DIAL_START == m_push_type)
        lv_label_set_text(m_lable_play_name, "DIAL");
...
static win_ctl_result_t win_dlna_msg_act(control_msg_t *ctl_msg)
{
    ...
    {
        if (ctl_msg->msg_type == MSG_TYPE_CAST_DLNA_SEEK)
        {
            m_seek_position = ctl_msg->msg_code;
            m_seeking = 1;
        }
        else if (ctl_msg->msg_type == MSG_TYPE_CAST_DLNA_PLAY)
        {
            m_seek_position = 0;
            m_seeking = 0;
        }
    ...
    case MSG_TYPE_CAST_DIAL_START:
        ...
        else if(ctl_msg->msg_type == MSG_TYPE_CAST_DIAL_START)
        {
            lv_label_set_text(m_lable_play_name, "DIAL");
        }
        else
    ...
    case MSG_TYPE_MEDIA_BUFFERING:
        ...
        break;
    case MSG_TYPE_CAST_PARSE_END:
        meida_play_infor_update(0, m_dlna_media_type);
        mp_err_time = 0;
        break;
    case MSG_TYPE_MEDIA_VIDEO_DECODER_ERROR:
    {
        char tips_str[MAX_UI_TIPS_STR_LEN] = {0};
        if (mp_err_time <= 3)
        {
            mp_err_time++;
            if (HCCAST_MEDIA_PHOTO == m_dlna_media_type)
            {
                snprintf(tips_str, sizeof(tips_str), STR_MP_UI_ERR_RETRY, MP_UI_ERR_IMG_DECODE_ERROR, 0);
            }
            else
            {
                snprintf(tips_str, sizeof(tips_str), STR_MP_UI_ERR_RETRY, MP_UI_ERR_VIDEO_DECODE_ERROR, 0);
            }
        }
        else
        {
            if (HCCAST_MEDIA_PHOTO == m_dlna_media_type)
            {
                snprintf(tips_str, sizeof(tips_str), STR_MP_UI_ERR, MP_UI_ERR_IMG_DECODE_ERROR, 0);
            }
            else
            {
                snprintf(tips_str, sizeof(tips_str), STR_MP_UI_ERR, MP_UI_ERR_VIDEO_DECODE_ERROR, 0);
            }
        }
        win_msgbox_msg_open(tips_str, 0, NULL, NULL);
        break;
    }
    case MSG_TYPE_MEDIA_AUDIO_DECODER_ERROR:
    {
        ...
        {
            mp_err_time++;
            snprintf(tips_str, sizeof(tips_str), STR_MP_UI_ERR_RETRY, MP_UI_ERR_AUDIO_DECODE_ERROR, 0);
        }
        else
        {
            snprintf(tips_str, sizeof(tips_str), STR_MP_UI_ERR, MP_UI_ERR_AUDIO_DECODE_ERROR, 0);
        }
        win_msgbox_msg_open(tips_str, 10000, NULL, NULL);
        break;
    }
    case MSG_TYPE_MEDIA_VIDEO_NOT_SUPPORT:
    {
        ...
        app_log(LL_WARNING, "Video format(%s/%d) not support!", code_id_str, value);

        char tips_str[MAX_UI_TIPS_STR_LEN] = {0};
        if (mp_err_time <= 3)
        {
            mp_err_time++;
            snprintf(tips_str, sizeof(tips_str), STR_MP_UI_ERR_RETRY, MP_UI_ERR_VIDEO_NOT_SUPPORT, value);
        }
        else
        {
            snprintf(tips_str, sizeof(tips_str), STR_MP_UI_ERR, MP_UI_ERR_VIDEO_NOT_SUPPORT, value);
        }
    ...
    case MSG_TYPE_MEDIA_AUDIO_NOT_SUPPORT:
    {
        ...
        app_log(LL_WARNING, "Audio format(%s/%d) not support!", code_id_str, value);

        char tips_str[MAX_UI_TIPS_STR_LEN] = {0};
        if (mp_err_time <= 3)
        {
            mp_err_time++;
            snprintf(tips_str, sizeof(tips_str), STR_MP_UI_ERR_RETRY, MP_UI_ERR_AUDIO_NOT_SUPPORT, value);
        }
        else
        {
            snprintf(tips_str, sizeof(tips_str), STR_MP_UI_ERR, MP_UI_ERR_AUDIO_NOT_SUPPORT, value);
        }
    ...
    case MSG_TYPE_MEDIA_NOT_SUPPORT:
    {
        ...
        {
            mp_err_time++;
            snprintf(tips_str, sizeof(tips_str), STR_MP_UI_ERR_RETRY, MP_UI_ERR_CONTAINER_NOT_SUPPORT, 0);
        }
        else
        {
            snprintf(tips_str, sizeof(tips_str), STR_MP_UI_ERR, MP_UI_ERR_CONTAINER_NOT_SUPPORT, 0);
        }

        win_msgbox_msg_open(tips_str, 5000, msgbox_exit_func, NULL);
        break;
    }
...
bool win_dlna_wait_open(uint32_t timeout)
{
    ...
    while(count--){
        play_en = api_get_wireless_playback_enable();
