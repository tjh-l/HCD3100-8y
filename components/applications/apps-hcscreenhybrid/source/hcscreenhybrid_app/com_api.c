...
static bool m_wirless_playback_enable = true;
static pthread_mutex_t m_wl_playback_enable_mutex = PTHREAD_MUTEX_INITIALIZER;
...
bool api_get_wireless_playback_enable(void)
{
    int enable;
    
    pthread_mutex_lock(&m_wl_playback_enable_mutex);
    enable = m_wirless_playback_enable;
    pthread_mutex_unlock(&m_wl_playback_enable_mutex);
    
    return enable;
}

void api_set_wireless_playback_enable(bool val)
{
    pthread_mutex_lock(&m_wl_playback_enable_mutex);
    m_wirless_playback_enable = val;
    pthread_mutex_unlock(&m_wl_playback_enable_mutex);
}
