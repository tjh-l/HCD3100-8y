...
#define IOS_UPGRADE_URL	"http://172.16.12.81/hccast/upgrade/link/upgrade.html?"
static void win_um_update_qr_code(qr_um_show_type_t qr_type)
{
    ...
    switch (qr_type)
    {
        ...
        case QR_UM_SHOW_UPGRADE:
            ...
            printf("qr_txt:%s\n",qr_txt);	
            break;
        case QR_UM_SHOW_APK_URL:    
            lv_obj_align(m_label_qr_msg, LV_ALIGN_BOTTOM_RIGHT, -28, -10);
            snprintf(qr_txt, sizeof(qr_txt), "%s", AUM_APK_URL);
            lv_label_set_text(m_label_qr_msg, "Android APK");
            printf("qr_txt:%s\n",qr_txt);
            break;
        default:
            break;
...
static int win_um_play_open(void *arg)
{
    ...
    lv_obj_align_to(m_cast_qr, m_label_qr_msg, LV_ALIGN_OUT_TOP_RIGHT, -15, -20);
    ...
    if (!g_um_plugined)
    {
        win_um_update_qr_code(QR_UM_SHOW_APK_URL);
    }
#endif
    ...
    if ((g_win_um_timer_start == 0) && (g_um_plugined == 0))
    {
        ...
        if (pthread_create(&tid, &attr, um_timer_process, NULL) != 0)
        {
            g_win_um_timer_start = 0;
        }
...
static win_ctl_result_t win_um_play_control(void *arg1, void *arg2)
{
...
#ifdef APP_UM_SHOW_LOGO
        com_logo_show(UM_LOGO);
#endif
    }
    else if(ctl_msg->msg_type == MSG_TYPE_CAST_IUSB_START ||ctl_msg->msg_type == MSG_TYPE_CAST_AUSB_START)
    {
    	m_playing = true;
    ...
    else if(ctl_msg->msg_type == MSG_TYPE_AUM_DEV_ADD ||ctl_msg->msg_type == MSG_TYPE_IUM_DEV_ADD)
    {
        pthread_mutex_lock(&g_win_um_menu_mutex);
        g_um_plugined = 1;
        
        if (ctl_msg->msg_type == MSG_TYPE_IUM_DEV_ADD)
        {
            m_um_type = UM_YTPE_I;
            ium_dev_add = 1;
        }    
    ...
    else if(ctl_msg->msg_type == MSG_TYPE_CAST_IUSB_NO_DATA)
    {
        if(ium_dev_add)
        {
            lv_label_set_text(m_msg_show, "");
            win_lable_pop_msg_open("Connect fail! Please reboot or reconnect the phone.");
        }
    }
