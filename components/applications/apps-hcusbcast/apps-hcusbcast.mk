...
ifneq ($(BR2_PACKAGE_HCCAST_USBMIRROR_SLAVE),y)
ifeq ($(CONFIG_APPS_IUM)$(BR2_PACKAGE_WOLFSSL),yy)
APPS_HCUSBCAST_DEPENDENCIES += wolfssl
endif
ifeq ($(CONFIG_APPS_IUM)$(BR2_PACKAGE_LIBOPENSSL),yy)
APPS_HCUSBCAST_DEPENDENCIES += libopenssl
endif
endif
...
APPS_HCUSBCAST_BUILD_CMDS = rsync -au --chmod=u=rwX,go=rX  --exclude .svn --exclude .git --exclude .hg --exclude .bzr --exclude CVS $(TOPDIR)/components/applications/apps-hcusbcast/source/ $(@D) && \
			 ...
			 cp $(@D)/hcusbcast_app/HCCAST_1920x1080.264 ${IMAGES_DIR}/fs-partition1-root/HCCAST_1920x1080.264;\
			 mkdir -p $(@D)/src;\
...
define APPS_HCUSBCAST_ENTRY_ADDR_FROM_DTS
	...
	echo "" >> $(APPS_HCUSBCAST_DIR)/.tmp-for-load-addr/$(strip $(APPS_HCUSBCAST_DTS_NAME)).dts
