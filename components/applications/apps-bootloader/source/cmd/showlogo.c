...
void showlogo_get_rotate_by_flip_mode(flip_mode_e mode, int *p_rotate_mode, int *p_h_flip, int *p_v_flip)
{
...
void showlogo_transfer_rotate_mode_for_screen(int init_rotate, int init_h_flip, int init_v_flip, int *p_rotate_mode,
					      int *p_h_flip, int *p_v_flip, int *p_fbdev_rotate)
{
...
int showlogo_get_flip_info(int init_rotate, int init_h_flip, int init_v_flip, int *rotate_type, int *flip_type)
{
	int rotate = 0, h_flip = 0, v_flip = 0;
	...
	showlogo_get_rotate_by_flip_mode(flip_flag, &rotate, &h_flip, &v_flip);

	showlogo_transfer_rotate_mode_for_screen(init_rotate, init_h_flip, init_v_flip, &rotate, &h_flip, &v_flip,
						 NULL);
...
static int lcd_get_init_rotate(int init_rotate)
{
	int ret = ROTATE_TYPE_0;
	switch (init_rotate) {
	case 0:
		ret = ROTATE_TYPE_0;
		break;
...
static int get_video_rotate(void)
{
...
#ifdef CONFIG_BOOT_LCD_ROTATE
	video_rotate_mode = lcd_get_init_rotate(init_rotate);
#else
	...
	log_d("%s:%d: video_rotate=%d video_rotate_mode=%d video_mirror_mode %d init_rotate %d flip_mode %d\n",
	      __func__, __LINE__, video_rotate, video_rotate_mode, video_mirror_mode, init_rotate, flip_mode);
...
#if defined(CONFIG_BOOT_ZOOM)
static void api_set_display_zoom(int s_x, int s_y, int s_w, int s_h, int d_x, int d_y, int d_w, int d_h)
{
...
static int sys_param_get_disp_rect(uint16_t *x, uint16_t *y, uint16_t *w, uint16_t *h)
{
	...
	*x = sys_data.disp_rect_x;
	*y = sys_data.disp_rect_y;
	*w = sys_data.disp_rect_w;
	*h = sys_data.disp_rect_h;
	printf("disp_rect: x: %d, y: %d, w: %d, h: %d\n", *x, *y, *w, *h);
	close(fd);
	return 0;
}

static void boot_logo_zoom(void)
{
	uint16_t x = 0;
	uint16_t y = 0;
	uint16_t w = 0;
	uint16_t h = 0;
	if (sys_param_get_disp_rect(&x, &y, &w, &h) == 0) {
		if (w > 0 && h > 0) {
			api_set_display_zoom(0, 0, 1920, 1080, x, y, w, h);
		}
	}
}
...
int start_show_logo(void *file, hc_avread avread)
{
	...
	ret = xTaskCreate(showlogo_thread, "showlogo_thread", 0x2000, logo, portPRI_TASK_NORMAL, &logo->task_id);
	if (ret != pdTRUE) {
		free(logo);
		logo = NULL;
	} else {
