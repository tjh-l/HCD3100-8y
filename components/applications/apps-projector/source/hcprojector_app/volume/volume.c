        ...
        if (key == LV_KEY_UP || key==LV_KEY_DOWN){
            ...
            if (volume_num > -1 && volume_num < 101){
                ...
                lv_label_set_text_fmt(label, "%d", volume_num/3);
                    ...
                    bluetooth_set_volume_level(volume_num);//zhp 2023-07-15
                    projector_set_some_sys_param(P_VOLUME, volume_num);
                #else
...
#ifdef BT_AC6956C_GX
static void snddev_volume_pre_process(uint8_t vol)
{
...
#ifdef SUPPORT_BT_CTRL_VOLUME_LEVER    
    set_volume1(100);
    bluetooth_set_volume_level(projector_get_some_sys_param(P_VOLUME));//zhp 2023-07-15
#else
#ifdef BT_AC6956C_GX
    // set soc snddev max vol and ctrl vol by bluetooth
    snddev_volume_pre_process(100);
#endif 
...
#ifdef SUPPORT_BT_CTRL_VOLUME_LEVER
        bluetooth_set_bt_mute_support(0x06);
...
#ifdef SUPPORT_BT_CTRL_VOLUME_LEVER
        bluetooth_set_bt_mute_support(0x07);
