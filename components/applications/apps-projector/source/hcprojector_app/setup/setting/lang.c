...
void change_language(){
    ...
    need to call this func to refresh label obj*/ 
    _change_language1(ui_ctrl_bar);
}
#ifdef LANGUAGE_UI_CHANGE

#define LANGUAGE_LENGTH LANGUAGE_MAX+(LANGUAGE_MAX%3==0 ? (LANGUAGE_MAX/3-1): LANGUAGE_MAX%3==1 ? (LANGUAGE_MAX/3 + 2): (LANGUAGE_MAX/3 + 1))
// 语言总数 加上 "\n" 再加上 " " 的数量 

static const char* osd_lang_vec[LANGUAGE_LENGTH + 1] = {0}; 
void osd_language_widget(lv_obj_t* btn){
    lv_obj_t *osd_lang_wid = create_new_widget(60,90);    //  整个语言部件占全屏的百分比 宽高可调
    lv_obj_set_style_opa(osd_lang_wid, LV_OPA_100, 0);

	create_widget_head(osd_lang_wid, STR_OSD_LANG_TITLE, 15);      // 头所占 语言部件百分比 可调 

    lv_obj_t *osd_body = lv_obj_create(osd_lang_wid);
    set_pad_and_border_and_outline(osd_body);
    lv_obj_set_style_pad_ver(osd_body, 0, 0);
	lv_obj_set_size(osd_body,LV_PCT(100),LV_PCT(73));
...
//    int len_pct = LANGUAGE_MAX <= 3 ? 100 : LANGUAGE_MAX*100/3;
    ...
    for(i = 0; i<LANGUAGE_MAX+(LANGUAGE_MAX%3==0?LANGUAGE_MAX/3-1:LANGUAGE_MAX/3); i++){  // 制作映射表，每三个语言加"\n" 
		if ((i-3)%4 == 0){
			osd_lang_vec[i] = "\n";
			continue;
			}
        osd_lang_vec[i] = api_rsc_string_get_by_langid(j, STR_LANG);
		j++;
    }


	for(; i < LANGUAGE_LENGTH; i++)  // 矩阵最后的一个或两个空位 加 " " 
		osd_lang_vec[i] = " ";
	
    osd_lang_vec[LANGUAGE_LENGTH]="";    //  结尾符 
    ...
    create_widget_foot(osd_lang_wid, 12, btn);       // 尾所占 语言部件百分比 可调 
   ...
   static int prev_lang_id = 0;
    if(code == LV_EVENT_KEY){
		...
		if (key == LV_KEY_UP || key == LV_KEY_DOWN){
            if (key == LV_KEY_DOWN)
                {
                    //index = (index+3)<LANGUAGE_MAX? index+3:index<24? 24:0;
                    index = (index+3)<LANGUAGE_MAX ? index+3:
								LANGUAGE_MAX%3 == 0 ? index%3 :
								LANGUAGE_MAX%3 == 1 ? (index<LANGUAGE_MAX-1? LANGUAGE_MAX-1:index%3):
								/*LANGUAGE_MAX%3 == 2 ?*/ (index<LANGUAGE_MAX-2? LANGUAGE_MAX-1:index%3);
                }
            else if (key == LV_KEY_UP)
                {
                    //index = (index-3)>=0?index-3:24;
					index = (index-3)>=0?index-3:
								LANGUAGE_MAX%3 == 0 ? index+(LANGUAGE_MAX-3) :
								LANGUAGE_MAX%3 == 1 ? (LANGUAGE_MAX-1):
								/*LANGUAGE_MAX%3 == 2 ?*/(index==0? LANGUAGE_MAX-2:LANGUAGE_MAX-1);					
                }
        ...
        if(key == LV_KEY_RIGHT || key == LV_KEY_LEFT){
            if (key == LV_KEY_RIGHT){
#ifdef LANGUAGE_UI_CHANGE               // 关闭语言左右滑动 
				if(1){
					index = (index+1)%LANGUAGE_MAX;
#else
                if(LANGUAGE_MAX<3){
                    index = (index+1)%2;
#endif
...
#ifdef LANGUAGE_UI_CHANGE			 // 关闭语言左右滑动 
				if(1){
					index = (index+LANGUAGE_MAX-1)%LANGUAGE_MAX;
#else
            ...
            if(index<LANGUAGE_MAX){
                //projector_set_some_sys_param(P_OSD_LANGUAGE, index);
                //change_language();
                //lv_label_set_text(lv_obj_get_child(btn, 1), osd_lang_vec[index]);
                lv_obj_t *label = lv_obj_get_child(lv_obj_get_child(target->parent->parent->parent, 0), 0);
                set_label_text2(label, GET_LABEL_ID((int)label->user_data), GET_FONT_ID((int)label->user_data)); 
            ...
            if(prev_lang_id != index){
                ...
                //change_language();
                //lv_label_set_text(lv_obj_get_child(btn, 1), osd_lang_vec[prev_lang_id]);
            }
...
// for lang test
              ...
              
            {
                ...
                change_language();
#ifdef LANGUAGE_UI_CHANGE     // 菜单页 语言选项 右侧 文本乱码问题（之前加的 "\n" 被输出了） 
				lv_label_set_text(lv_obj_get_child(btn, 1), osd_lang_vec[id + id/3]);
...
#ifdef LANGUAGE_UI_CHANGE   // 在语言尾部加多余的" " 按钮死机问题 （因为给空格加上的“FONT_NORMAL”） 
        if(dsc->id < LANGUAGE_MAX)
#endif
        ...
        lv_btnmatrix_set_selected_btn(target, index);
        lv_btnmatrix_set_btn_ctrl(target, index, LV_BTNMATRIX_CTRL_CHECKED);
#ifndef LANGUAGE_UI_CHANGE  		 // 关闭语言左右滑动 
