...
static void set_pq_setting(int pq_fd, const struct pq_settings *pq)
{
    ioctl(pq_fd, PQ_SET_PARAM, pq);
    kaf_set_pq_param(pq);
}
...
int set_color_temperature(int mode)
{
    ...
    factory_set_some_sys_param(P_COLOR_TEMP, mode);
#endif
    ...
    switch(mode)
    {
        case COLOR_TEMP_STANDARD:
            set_pq_setting(pq_fd , &pqReg.pq_design_reg[0]);
            break;
        case COLOR_TEMP_WARM:
            set_pq_setting(pq_fd , &pqReg.pq_design_reg[2]);
            break;
         case COLOR_TEMP_COLD:
            set_pq_setting(pq_fd , &pqReg.pq_design_reg[1]);
            break;
        default:
            break;
    }
