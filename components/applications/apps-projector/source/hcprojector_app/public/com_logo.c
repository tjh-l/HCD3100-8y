...
static int com_logo_dis_backup(void)
{
    int fd = -1;
...
static int com_logo_h264_video_open(int rotate_type, int mirror_type)
{
    ...
    mvcfg.rotate_type = rotate_type;
    mvcfg.mirror_type = mirror_type;  
	mvcfg.kshm_size = 0x100000;   

    if (api_video_pbp_get_support()){
        mvcfg.pbp_mode = VIDEO_PBP_2P_ON;

        if (APP_DIS_TYPE_HD == api_video_pbp_get_dis_type(VIDEO_PBP_SRC_MP))
            mvcfg.dis_type = DIS_TYPE_HD;
        else
            mvcfg.dis_type = DIS_TYPE_UHD;

        if (APP_DIS_LAYER_MAIN == api_video_pbp_get_dis_layer(VIDEO_PBP_SRC_MP))
            mvcfg.dis_layer = DIS_LAYER_MAIN;
        else
            mvcfg.dis_layer = DIS_LAYER_AUXP;
    }

#ifdef SYS_ZOOM_SUPPORT
    if (get_display_h() != 1920 || get_display_v() != 1080){
        mvcfg.preview = 1;
        mvcfg.src_area.x = 0;
        mvcfg.src_area.y = 0;
        mvcfg.src_area.w = 1920;
        mvcfg.src_area.h = 1080;
        mvcfg.dst_area.x = get_display_x();
        mvcfg.dst_area.y = get_display_y();
        mvcfg.dst_area.w = get_display_h();
        mvcfg.dst_area.h = get_display_v();
    }
#endif
...
static int com_logo_h264_video_feed(char *data, unsigned int len, int rotate_type, int mirror_type)
{
    AvPktHd pkthd = { 0 };    

    //set pkthd.pts to -1, so that av is freerun. do not need to av sync.
    pkthd.pts = -1;
    pkthd.dur = 0;
    pkthd.size = len;
    pkthd.flag = AV_PACKET_ES_DATA;
    pkthd.video_rotate_mode = (uint16_t)rotate_type;
    pkthd.video_mirror_mode = (uint16_t)mirror_type;
...
static void com_logo_h264_video_close(int closevp, int fillblack)
{
...
int com_logo_show(const char *file_path, int rotate_type, int mirror_type)
{
    ...
    if(com_logo_h264_video_open(rotate_type, mirror_type) < 0)
    {
        fclose(h264file);
        free(data);
        return -1;
