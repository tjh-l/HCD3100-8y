...
static void win_dlna_key_seek_do(uint32_t key)
{
    ...
    if (m_seeking)
    {
        play_time = m_seek_position;
    }
    else
    {
        play_time = hccast_media_get_position();
    }
...
static void win_dlna_msg_act(control_msg_t *ctl_msg)
{
    ...
    case MSG_TYPE_CAST_AIRCAST_VOL_SET:
    {
        uint8_t vol = (uint8_t)ctl_msg->msg_code;
        if (vol > 0 && ui_mute_get()){
            ui_mute_set();
        }
        win_volume_set(vol, 1);
        break;
    }
    case MSG_TYPE_CAST_DLNA_MUTE:
    #ifdef MUTE_CONTROL_BY_SYS        
        api_key_queue_send(KEY_MUTE, 1);
    #else
        win_mute_on_off(true);
    #endif        
        break;
    ...
    case MSG_TYPE_MEDIA_VIDEO_DECODER_ERROR:
    {
        if (mp_err_time <= 3)
        {
            ...
            if (HCCAST_MEDIA_PHOTO == m_dlna_media_type)
            {
                win_msgbox_msg_fmt_open(0, NULL, NULL, STR_MP_UI_ERR_RETRY, MP_UI_ERR_IMG_DECODE_ERROR, 0);
            }
            else
            {
                win_msgbox_msg_fmt_open(0, NULL, NULL, STR_MP_UI_ERR_RETRY, MP_UI_ERR_VIDEO_DECODE_ERROR, 0);
            }
        }
        else
        {
            if (HCCAST_MEDIA_PHOTO == m_dlna_media_type)
            {
                win_msgbox_msg_fmt_open(0, NULL, NULL, STR_MP_UI_ERR, MP_UI_ERR_IMG_DECODE_ERROR, 0);
            }
            else
            {
                win_msgbox_msg_fmt_open(0, NULL, NULL, STR_MP_UI_ERR, MP_UI_ERR_VIDEO_DECODE_ERROR, 0);
            }
        }
        break;
    }
    case MSG_TYPE_MEDIA_AUDIO_DECODER_ERROR:
    {
        ...
        {
            mp_err_time++;
            win_msgbox_msg_fmt_open(10000, NULL, NULL, STR_MP_UI_ERR_RETRY, MP_UI_ERR_AUDIO_DECODE_ERROR, 0);
        }
        else
        {
            win_msgbox_msg_fmt_open(10000, NULL, NULL, STR_MP_UI_ERR, MP_UI_ERR_AUDIO_DECODE_ERROR, 0);
        }
        break;
    }
    case MSG_TYPE_MEDIA_VIDEO_NOT_SUPPORT:
    {
        ...
        {
            mp_err_time++;
            win_msgbox_msg_fmt_open(0, NULL, NULL, STR_MP_UI_ERR_RETRY, MP_UI_ERR_VIDEO_NOT_SUPPORT, value);
        }
        else
        {
            win_msgbox_msg_fmt_open(0, NULL, NULL, STR_MP_UI_ERR, MP_UI_ERR_VIDEO_NOT_SUPPORT, value);
        }
    ...
    case MSG_TYPE_MEDIA_AUDIO_NOT_SUPPORT:
    {
        ...
        {
            mp_err_time++;
            win_msgbox_msg_fmt_open(10000, NULL, NULL, STR_MP_UI_ERR_RETRY, MP_UI_ERR_AUDIO_NOT_SUPPORT, value);
        }
        else
        {
            win_msgbox_msg_fmt_open(10000, NULL, NULL, STR_MP_UI_ERR, MP_UI_ERR_AUDIO_NOT_SUPPORT, value);
        }
    ...
    case MSG_TYPE_MEDIA_NOT_SUPPORT:
    {
        ...
        {
            mp_err_time++;
            win_msgbox_msg_fmt_open(5000, msgbox_exit_func, NULL, STR_MP_UI_ERR_RETRY, MP_UI_ERR_CONTAINER_NOT_SUPPORT, 0);
        }
        else
        {
            win_msgbox_msg_fmt_open(5000, msgbox_exit_func, NULL, STR_MP_UI_ERR, MP_UI_ERR_CONTAINER_NOT_SUPPORT, 0);
        }
