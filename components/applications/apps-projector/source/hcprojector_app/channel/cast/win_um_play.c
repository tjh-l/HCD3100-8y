...
#define QR_UM_BOX_Y        (QR_UM_MSG_Y+260)
#define QR_UM_BOX_W        160
#endif
...
static int m_um_upgrade = 0;
static int64_t m_um_rotate_tick  = 0;
...
void win_um_set_upg_state(int state)
{
    m_um_upgrade = state;
}
...
static void win_um_update_qr_code(qr_um_show_type_t qr_type)
{
    ...
    case QR_UM_SHOW_APK:
        ...
        sprintf(qr_txt, HCCAST_APK_PACKAGE);
        break;
    default:
        break;
    }
...
static int win_um_play_open(void *arg)
{
    ...
    api_set_display_aspect(zoom_mode, DIS_NORMAL_SCALE);    
    ...
    lv_obj_set_width(m_msg_show, 1000);
    lv_obj_set_height(m_msg_show, 32);
    lv_obj_set_x(m_msg_show, 228);
    lv_obj_set_y(m_msg_show, 274);
    lv_obj_set_align(m_msg_show, LV_ALIGN_LEFT_MID);
    //lv_label_set_text(m_msg_show, "1.Please scan the code of Android device to install software");
  //  win_cast_label_txt_set(m_msg_show, STR_INSERT_ANDROID_OR_APPLE);
    ...
    lv_obj_set_style_text_color(m_msg_show, lv_color_hex(0xFFFFFF), LV_PART_MAIN | LV_STATE_DEFAULT);
    lv_obj_set_style_text_opa(m_msg_show, 255, LV_PART_MAIN | LV_STATE_DEFAULT);
    ...
    lv_obj_set_height(m_label_qr_msg, 32);
    lv_obj_set_x(m_label_qr_msg, 228);
    lv_obj_set_y(m_label_qr_msg, 210);
    ...
    //lv_obj_set_style_text_font(m_label_qr_msg, &lv_font_montserrat_32, LV_PART_MAIN | LV_STATE_DEFAULT); 
#else
	m_label_qr_msg = lv_label_create(m_um_play_root);
...
static void win_um_play_control(void *arg1, void *arg2)
{
    ...
    if(ctl_msg->msg_type == MSG_TYPE_CAST_IUSB_STOP ||
        ...
        m_um_upgrade = 1;
        ui_um_upgrade_init();
        _ui_screen_change(ui_um_upgrade,0,0);
    }
    else if (ctl_msg->msg_type == MSG_TYPE_CAST_IUSB_COPYRIGHT_PROTECTION){
        win_msgbox_msg_open(STR_LIMITED, 3000, NULL, NULL);
    }
}
    ...
    if (ta == ui_um_play){
        ...
        if(event == LV_EVENT_KEY && key_indev->proc.state == LV_INDEV_STATE_PRESSED){
            ...
            if (value == LV_KEY_ESC)
            {
                ...
                if (m_playing && (api_get_sys_clock_time() - m_um_rotate_tick >= 500))
                {
                    win_cast_mirror_rotate_switch();
                    m_um_rotate_tick = api_get_sys_clock_time();
                }    
            }
        }
