...
static void win_um_upgrade_progress_update(int progress)
{
    int percent;
...
static int win_um_upgrade_open(void *arg)
{
    printf("%s()!\n", __FUNCTION__);
    ...
    //upgrade frame
    m_obj_upg_root = lv_obj_create(ui_um_upgrade);
    ...
    win_um_upgrade_progress_update(0);

    return API_SUCCESS;
}

static int win_um_upgrade_close(void *arg)
{
    printf("%s()!\n", __FUNCTION__);
    m_flash_burning = false;
...
static void win_um_upgrade_status_update(uint32_t status)
{
...
static void event_handler(lv_event_t * e)
{
    ...
    if (ta == ui_um_upgrade){
        if(event == LV_EVENT_SCREEN_LOAD_START) {
            win_um_upgrade_open(NULL);
        } else if (event == LV_EVENT_SCREEN_UNLOAD_START) {
            win_um_upgrade_close(NULL);
        } 
...
static void win_um_upgrade_msg_proc(void *arg1, void *arg2)
{
    (void)arg2;
    ...
    switch (ctl_msg->msg_type)
    {
        case MSG_TYPE_UPG_STATUS:
            win_um_upgrade_status_update(ctl_msg->msg_code);
            break;
        case MSG_TYPE_UPG_DOWNLOAD_PROGRESS:
            win_um_upgrade_progress_update(ctl_msg->msg_code);
            break;
        ...
        {
            extern void win_um_set_upg_state(int state);
            win_um_set_upg_state(0);
...
static void win_um_upgrade_control(void *arg1, void *arg2)
{
    ...
    {
        win_um_upgrade_msg_proc(arg1, NULL);
    }
    else
    {
        // ret = WIN_CTL_SKIP;
...
void ui_um_upgrade_init(void)
{
    ...
    static int ui_um_upgrade_inited = 0;

    if (ui_um_upgrade_inited)
    {
        return ;
    }

    ui_um_upgrade_inited = 1;
    ui_um_upgrade = lv_obj_create(NULL);

    lv_obj_clear_flag(ui_um_upgrade, LV_OBJ_FLAG_SCROLLABLE);

    lv_obj_add_event_cb(ui_um_upgrade, event_handler, LV_EVENT_ALL, NULL);
    lv_obj_set_style_bg_opa(ui_um_upgrade, LV_OPA_TRANSP, 0);

    upgrade_entry.screen = ui_um_upgrade;
    upgrade_entry.control = win_um_upgrade_control;
