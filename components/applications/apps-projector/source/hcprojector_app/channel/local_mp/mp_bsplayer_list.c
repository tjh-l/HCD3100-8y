...
static lv_obj_t* lastfocus_obj = NULL;
...
int draw_musiclist(void)
{
    ...
    int node_start = bs_listctrl.top;
    int foucus_idx = bs_play_filelist.item_index-node_start;
    clear_musiclist();
    for(int i =0;i<MAX_MPLIST_ITEM;i++){
...
{
    if(musiclist_obj && lv_obj_is_valid(musiclist_obj)){
...
int win_music_recources_ui_update(int argc ,void* argv)
{
    ...
    if(argc == MSG_TYPE_USB_UNMOUNT || argc == MSG_TYPE_USB_UNMOUNT_FAIL||
       argc == MSG_TYPE_SD_UNMOUNT || argc == MSG_TYPE_SD_UNMOUNT_FAIL){
        if(musiclist_obj && lv_obj_is_valid(musiclist_obj)){
            char* win_music_path = bs_play_filelist.dir_path;
            if(strcmp(win_music_path,MOUNT_ROOT_DIR) == 0){
                clear_all_bsplayer_mem();
                memset(&bs_play_filelist,0,sizeof(file_list_t));
                musiclist_win_add_data();
            }else{
                if(strstr(win_music_path,msg_code)){
                    clear_all_bsplayer_mem();
                    memset(&bs_play_filelist,0,sizeof(file_list_t));
                    lv_obj_del(musiclist_obj);    
                    musiclist_obj = NULL;
                    lv_group_focus_obj(lastfocus_obj);
                }
            }
        }
    }
