    ...
    switch(scr_id)
    {
        case SCR_HOME:
            if(!guider_ui.home) {
                ...
                //lv_img_set_src(guider_ui.home_bg_img, "I:blue_bg.jpg");
                //lv_img_set_src(guider_ui.home_bg_img, &blue_bg);
                //lv_img_set_src(guider_ui.home_bg_img, "I:bg.png");
                home_event_init();
...
static void home_sw_img_bg_event_handler (lv_event_t *event){
    lv_event_code_t code = lv_event_get_code(event);
    lv_obj_t * obj = lv_event_get_target(event);
    if (code == LV_EVENT_VALUE_CHANGED) {
        if(lv_obj_has_state(obj, LV_STATE_CHECKED)){
            lv_obj_clear_flag(guider_ui.home_img_bg1, LV_OBJ_FLAG_HIDDEN);
            lv_obj_clear_flag(guider_ui.home_img_bg2, LV_OBJ_FLAG_HIDDEN);
            lv_obj_clear_flag(guider_ui.home_img_bg3, LV_OBJ_FLAG_HIDDEN);
        }else{
        ...
        lv_obj_invalidate(guider_ui.home);
    }
}
...
{
    lv_demo_printer_anim_in_all(guider_ui.home, 200);
    lv_obj_add_event_cb(guider_ui.home_sw_img_bg, home_sw_img_bg_event_handler, LV_EVENT_ALL, NULL);
}
...
void custom_init(lv_ui *ui)
{
    setup_scr_home(ui);
    //lv_img_set_src(guider_ui.home_bg_img, "I:bg.png");
    // lv_img_set_src(guider_ui.home_bg_img, "I:\NXP\blue_bg.jpg");
    //lv_img_set_src(guider_ui.home_bg_img, &blue_bg);
    event_cb();
    home_event_init();
    lv_anim_set_var(&ani_en_btn_click, guider_ui.home_imgbtncopy);
