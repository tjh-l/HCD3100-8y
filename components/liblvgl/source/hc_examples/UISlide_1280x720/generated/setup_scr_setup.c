...
void setup_scr_setup(lv_ui *ui)
{
	...
	lv_obj_set_size(ui->setup, 1280, 720);
	...
	lv_obj_set_size(ui->setup_cont0, 1280, 720);
	...
	lv_obj_set_pos(ui->setup_btnsetback, 428, 518);
	lv_obj_set_size(ui->setup_btnsetback, 356, 103);
	...
	lv_obj_set_style_radius(ui->setup_btnsetback, 137, LV_PART_MAIN|LV_STATE_DEFAULT);
	...
	lv_obj_set_style_text_font(ui->setup_btnsetback, &lv_font_simsun_30, LV_PART_MAIN|LV_STATE_DEFAULT);
	...
	lv_obj_set_pos(ui->setup_label2, 26, 386);
	lv_obj_set_size(ui->setup_label2, 1226, 79);
	...
	lv_obj_set_style_text_font(ui->setup_label2, &lv_font_arial_40, LV_PART_MAIN|LV_STATE_DEFAULT);
	...
	lv_img_set_src(ui->setup_printer, &_printer2_alpha_160x145);
	lv_img_set_pivot(ui->setup_printer, 0,0);
	lv_img_set_angle(ui->setup_printer, 0);
	lv_obj_set_pos(ui->setup_printer, 410, 184);
	lv_obj_set_size(ui->setup_printer, 160, 145);
	...
	lv_img_set_src(ui->setup_img, &_no_internet_alpha_63x63);
	lv_img_set_pivot(ui->setup_img, 0,0);
	lv_img_set_angle(ui->setup_img, 0);
	lv_obj_set_pos(ui->setup_img, 577, 163);
	lv_obj_set_size(ui->setup_img, 63, 63);
	...
	lv_img_set_src(ui->setup_cloud, &_cloud_alpha_146x105);
	lv_img_set_pivot(ui->setup_cloud, 0,0);
	lv_img_set_angle(ui->setup_cloud, 0);
	lv_obj_set_pos(ui->setup_cloud, 687, 79);
	lv_obj_set_size(ui->setup_cloud, 146, 105);
