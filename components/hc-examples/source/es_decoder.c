...
static bool g_h264_preview_enable = false;
static vdec_dis_rect_t g_h264_rect = { {0,0,1920,1080},{0,0,1920,1080} };
struct h264_decoder {
...
void h264_decoder_set_preview(bool preview, vdec_dis_rect_t dis_rect)
{
    g_h264_preview_enable = preview;
    memcpy(&g_h264_rect.src_rect , &dis_rect.src_rect , sizeof(struct av_area));
    memcpy(&g_h264_rect.dst_rect , &dis_rect.dst_rect , sizeof(struct av_area));
}
void *h264_decode_init(int width , int height , 
                       uint8_t *extradata , 
                       int size, 
                       int8_t rotate_enable,
                       int8_t quick_mode_enable,
                       bool b_aux_layer)
{
    ...
    p->cfg.b_aux_layer = b_aux_layer;
    p->cfg.preview = g_h264_preview_enable;
    if (p->cfg.preview == true)
    {
        p->cfg.src_area = g_h264_rect.src_rect;
        p->cfg.dst_area = g_h264_rect.dst_rect;
    }
