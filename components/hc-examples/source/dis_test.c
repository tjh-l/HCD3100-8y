...
static int unreg_vga_dac(void)
{
    struct dis_dac_param dac_param = { 0 };
    int fd;

    printf("unreg_vga_dac\n");
    dac_param.distype = DIS_TYPE_HD;
    dac_param.info.type = DIS_DAC_RGB;
...
int aspect_test(int argc , char *argv[])
{
	...
	while((opt = getopt(argc , argv , "d:l:t:m:")) != EOF) {
		switch(opt) {
			case 't':
...
int rotate_test(int argc , char *argv[])
{
	...
	while((opt = getopt(argc , argv , "r:m:d:n:l:")) != EOF) {
		switch(opt) {
			case 'r':
			...
			case 'd':
				...
				break;
			case 'n':
				yuv.enable = (bool)atoi(optarg); 
				break;
			case 'l':  
				yuv.layer = (enum DIS_LAYER)atoi(optarg);
				break;
			default:
				break;
		}
...
{
	struct dis_layer_blend_order blend_order = { 0 };
	int fd;
	int opt;
	...
	while((opt = getopt(argc , argv , "d:o:l:r:g:")) != EOF) {
		switch(opt) {
			case 'o':
                blend_order.main_layer = atoi(optarg);
				break;
			case 'l':
                blend_order.auxp_layer = atoi(optarg);
				break;
			case 'r':
                blend_order.gmas_layer = atoi(optarg);
				break;
			case 'g':
                blend_order.gmaf_layer = atoi(optarg);
				break;
			case 'd':
                blend_order.distype = atoi(optarg);
				break;
			default:
				break;
		}
	...
	if(blend_order.distype != DIS_TYPE_SD && blend_order.distype != DIS_TYPE_HD &&
       blend_order.distype != DIS_TYPE_UHD) {
		printf("error display type %d\n" , blend_order.distype);
		return -1;
	}
	...
	ioctl(fd , DIS_SET_LAYER_ORDER , &blend_order);
...
{
	struct dis_cutoff dis_cutoff = { 0 };
	int fd;
	int opt;
	...
	while((opt = getopt(argc , argv, "d:c:")) != EOF) {
		switch(opt) {
			case 'd':
                dis_cutoff.distype = atoi(optarg);
				break;
			case 'c':
                dis_cutoff.cutoff = atoi(optarg);
				break;
			default:
				break;
		}
	...
	if(dis_cutoff.distype != DIS_TYPE_SD && 
       dis_cutoff.distype != DIS_TYPE_HD &&
       dis_cutoff.distype != DIS_TYPE_UHD) {
		printf("error display type %d\n" , dis_cutoff.distype);
		return -1;
	}
	...
	ioctl(fd , DIS_SET_CUTOFF , &dis_cutoff);
...
{
	struct dis_win_autoonoff win_autoonoff = { 0 };
	int fd;
	int opt;
	...
	while((opt = getopt(argc , argv, "d:w:")) != EOF) {
		switch(opt) {
			case 'd':
                win_autoonoff.distype = atoi(optarg);
				break;
			case 'w':
                win_autoonoff.enable = atoi(optarg);
				break;
			default:
				break;
		}
	...
	if(win_autoonoff.distype != DIS_TYPE_SD && win_autoonoff.distype != DIS_TYPE_HD &&
       win_autoonoff.distype != DIS_TYPE_UHD) {
		printf("error display type %d\n" , win_autoonoff.distype);
		return -1;
	}
	...
	ioctl(fd , DIS_SET_WIN_AUTOONOFF , &win_autoonoff);
...
{
	struct dis_single_output single_output = { 0 };
	int fd;
	int opt;
	...
	while((opt = getopt(argc , argv, "d:o:")) != EOF) {
		switch(opt) {
			case 'd':
                single_output.distype = atoi(optarg);
				break;
			case 'o':
                single_output.enable = atoi(optarg);
				break;
			default:
				break;
		}
	...
	if(single_output.distype != DIS_TYPE_SD && single_output.distype != DIS_TYPE_HD &&
       single_output.distype != DIS_TYPE_UHD) {
		printf("error display type %d\n" , single_output.distype);
		return -1;
	}
	...
	ioctl(fd , DIS_SET_SINGLE_OUTPUT , &single_output);
...
{
	struct dis_keystone_param keystone_param = { 0 };
    keystone_param.distype = 100;
	int fd;
	int opt;
	...
	while((opt = getopt(argc , argv, "d:e:w:i:c:y:b:r:h:o:")) != EOF) {
		switch(opt) {
			case 'd':
                keystone_param.distype = atoi(optarg);
				break;
			case 'c':
                keystone_param.info.bg_enable = atoi(optarg);
				break;
			case 'e':
                keystone_param.info.enable = atoi(optarg);
				break;
			case 'w':
                keystone_param.info.width_up = atoi(optarg);
				break;
			case 'i':
                keystone_param.info.width_down = atoi(optarg);
				break;
			case 'y':
                keystone_param.info.bg_ycbcr.y = atoi(optarg);
				break;
			case 'b':
                keystone_param.info.bg_ycbcr.cb = atoi(optarg);
				break;
			case 'r':
                keystone_param.info.bg_ycbcr.cr = atoi(optarg);
				break;
            case 'h':
                keystone_param.info.height = atoi(optarg);
                break;
            case 'o':
                keystone_param.info.offset = atoi(optarg);
                break;
			default:
				break;
		}
	...
	if(keystone_param.distype != DIS_TYPE_SD && 
       keystone_param.distype != DIS_TYPE_HD &&
       keystone_param.distype != DIS_TYPE_UHD) {
		printf("error display type or config no distype para %d\n" , keystone_param.distype);
		return -1;
	}
	...
	ioctl(fd , DIS_SET_KEYSTONE_PARAM , &keystone_param);
...
{
	struct dis_bg_color bg_color = { 0 };
	int fd;
	int opt;
	...
	while((opt = getopt(argc , argv, "d:y:b:r:")) != EOF) {
		switch(opt) {
			case 'd':
                bg_color.distype = atoi(optarg);
				break;
			case 'y':
                bg_color.ycbcr.y = atoi(optarg);
				break;
			case 'b':
                bg_color.ycbcr.cb = atoi(optarg);
				break;
			case 'r':
                bg_color.ycbcr.cr = atoi(optarg);
				break;
			default:
				break;
		}
	...
	if(bg_color.distype != DIS_TYPE_SD && bg_color.distype != DIS_TYPE_HD &&
       bg_color.distype != DIS_TYPE_UHD) {
		printf("error display type %d\n" , bg_color.distype);
		return -1;
	}
	...
	ioctl(fd ,DIS_SET_BG_COLOR , &bg_color);
...
int reg_dac_test(int argc , char *argv[])
{
	...
	while((opt = getopt(argc , argv, "d:t:e:f:s:h:p:v:")) != EOF) {
		switch(opt) {
			case 'd':
                dac_param.distype = atoi(optarg);
				break;
			case 't':
                dac_param.info.type = atoi(optarg);
				break;
			case 'e':
                dac_param.info.dac.enable = atoi(optarg);
				break;
			case 'f':
				if(dac_param.info.type == DIS_DAC_CVBS) {
                    dac_param.info.dac.dacidx.cvbs.cv = atoi(optarg);
				} else if (dac_param.info.type == DIS_DAC_SVIDEO) {
                    dac_param.info.dac.dacidx.svideo.sv1 = atoi(optarg);
				} else if (dac_param.info.type == DIS_DAC_YUV) {
                    dac_param.info.dac.dacidx.yuv.y = atoi(optarg);
				} else {
                    dac_param.info.dac.dacidx.rgb.r = atoi(optarg);
				}
				break;
			case 's':
				if (dac_param.info.type == DIS_DAC_SVIDEO) {
                    dac_param.info.dac.dacidx.svideo.sv2 = atoi(optarg);
				} else if (dac_param.info.type == DIS_DAC_YUV) {
                    dac_param.info.dac.dacidx.yuv.u = atoi(optarg);
				} else if (dac_param.info.type == DIS_DAC_RGB) {
                    dac_param.info.dac.dacidx.rgb.g = atoi(optarg);
				}
				break;
			case 'h':
				if (dac_param.info.type == DIS_DAC_YUV) {
                    dac_param.info.dac.dacidx.yuv.v = atoi(optarg);
				} else if (dac_param.info.type == DIS_DAC_RGB) {
                    dac_param.info.dac.dacidx.rgb.b = atoi(optarg);
				}
				break;
			case 'p':
                dac_param.info.dac.progressive = atoi(optarg);
				break;
			default:
				break;
		}
	...
	if(dac_param.distype != DIS_TYPE_SD && 
       dac_param.distype != DIS_TYPE_HD &&
       dac_param.distype != DIS_TYPE_UHD) {
		printf("error display type %d\n" , dac_param.distype);
		return -1;
	}
...
int denh_enable_test(int argc , char *argv[])
{
    ...
    while((opt = getopt(argc , argv , "d:o:")) != EOF)
    {
        ...
        {
            case 'o':
                dyn_enh.onoff = atoi(optarg);
                break;
            case 'd':
                dyn_enh.distype = atoi(optarg);
                break;
            default:
                break;
        }
...
int dis_get_blend_order(int argc , char *argv[])
{
    (void)argc;
    (void)argv;
    struct dis_layer_blend_order blend_order = { 0 };
    ...
    blend_order.distype = DIS_TYPE_HD;
    fd = open("/dev/dis" , O_WRONLY);
    if (fd < 0) {
        return -1;
    }
    ioctl(fd , DIS_GET_LAYER_ORDER , &blend_order);
    close(fd);

    printf("get_blend_order MP:%d auxp:%d gs:%d gf:%d\n", blend_order.main_layer, blend_order.auxp_layer, blend_order.gmas_layer, blend_order.gmaf_layer);
    return 0;
}

int dis_set_cvbs_source_layer(int argc , char *argv[])
{
    int fd;
    int opt;
    opterr = 0;
    optind = 0;
    uint32_t cvbs_source_layer = 0;

    while ((opt = getopt(argc , argv , "l:")) != EOF) {
        switch (opt) {
            case 'l':
                cvbs_source_layer = atoi(optarg);
                break;
            default:
                break;
        }
    ...
    ioctl(fd , DIS_SET_CVBS_SOURCE_LAYER , cvbs_source_layer);
    close(fd);
    return 0;
}

int global_alpha_test(int argc , char *argv[])
{
    struct dis_alpha_param global_alpha = { 0 };
    int fd;
    int opt;
    ...
    global_alpha.distype = DIS_TYPE_HD;
    global_alpha.layer = DIS_LAYER_MAIN;
    while ((opt = getopt(argc , argv , "l:d:e:a:")) != EOF) {
        switch (opt) {
            case 'l':
                global_alpha.layer = atoi(optarg);
                break;
            case 'd':
                global_alpha.distype = atoi(optarg);
                break;
            case 'e':
                global_alpha.enable = atoi(optarg);
                break;
            case 'a':
                global_alpha.alpha = atoi(optarg);
                break;
            default:
                break;
        }
    ...
    if (global_alpha.distype != DIS_TYPE_SD && global_alpha.distype != DIS_TYPE_HD &&
        global_alpha.distype != DIS_TYPE_UHD) {
        printf("error display type %d\n" , global_alpha.distype);
        return -1;
    }

    if (global_alpha.layer != DIS_LAYER_MAIN && global_alpha.layer != DIS_LAYER_AUXP) {
        printf("error display layer %d\n" , global_alpha.layer);
        return -1;
    }
    ...
    ioctl(fd , DIS_SET_GLOBAL_ALPHA , &global_alpha);
