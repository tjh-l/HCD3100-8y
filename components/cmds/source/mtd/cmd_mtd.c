...
{
	bool has_pages = mtd->type == MTD_NANDFLASH || mtd->type == MTD_MLCNANDFLASH;
	uint npages = mtd_len_to_pages(mtd, len);
	uint page;
				 ...
				 bool write_empty_pages, bool woob)
{
	...
	if (!write_empty_pages && mtd_oob_write_is_empty(io_op)) {
		...
		if (mtd->type == MTD_NORFLASH)
			ret = MTD_WRITE(mtd, off, io_op->len, io_op->datbuf);
		else
...
int do_mtd_io(int argc, char *argv[])
{
	...
	io_op.datbuf = buf;
	io_op.oobbuf = woob ? &buf[io_op.len] : NULL;
	...
	while (remaining) {
		...
		if (read) {
			if (mtd->type == MTD_NORFLASH)
				ret = MTD_READ(mtd, off, io_op.len, io_op.datbuf);
			else
		...
		remaining -= io_op.len;
		if (woob)
			io_op.datbuf += io_op.len + mtd->oobsize;
		else
			io_op.datbuf += io_op.len;
		io_op.oobbuf += io_op.len + mtd->oobsize;
	}
...
static inline uint32_t cmd_mtd_div_by_eb(uint64_t sz, struct mtd_dev_s *mtd)
{
...
int do_mtd_erase(int argc, char *argv[])
{
	...
	printf("Erasing 0x%08llx ... 0x%08llx (%ld eraseblock(s))\n",
	       off, off + len - 1, cmd_mtd_div_by_eb(len, mtd));
...
int do_mtd_bad(int argc, char *argv[])
{
	...
	for (off = 0; off < (loff_t)mtd->size; off += mtd->erasesize) {
...
CONSOLE_CMD(read, "mtd", do_mtd_io, CONSOLE_CMD_MODE_SELF, "<name> <addr> [<off> [<size>]]")
CONSOLE_CMD(read_raw, "mtd", do_mtd_io, CONSOLE_CMD_MODE_SELF, "<name> <addr> [<off> [<size>]]")
CONSOLE_CMD(read_oob, "mtd", do_mtd_io, CONSOLE_CMD_MODE_SELF, "<name> <addr> [<off> [<size>]]")
CONSOLE_CMD(dump, "mtd", do_mtd_io, CONSOLE_CMD_MODE_SELF, "<name>        [<off> [<size>]]")
CONSOLE_CMD(dump_raw, "mtd", do_mtd_io, CONSOLE_CMD_MODE_SELF, "<name>        [<off> [<size>]]")
CONSOLE_CMD(dump_oob, "mtd", do_mtd_io, CONSOLE_CMD_MODE_SELF, "<name>        [<off> [<size>]]")
CONSOLE_CMD(write, "mtd", do_mtd_io, CONSOLE_CMD_MODE_SELF, "<name> <addr> [<off> [<size>]]")
CONSOLE_CMD(write_raw, "mtd", do_mtd_io, CONSOLE_CMD_MODE_SELF, "<name> <addr> [<off> [<size>]]")
CONSOLE_CMD(write_oob, "mtd", do_mtd_io, CONSOLE_CMD_MODE_SELF, "<name> <addr> [<off> [<size>]]")
CONSOLE_CMD(write_oob_dontskipff, "mtd", do_mtd_io, CONSOLE_CMD_MODE_SELF, "<name> <addr> [<off> [<size>]]")
CONSOLE_CMD(write_raw_dontskipff, "mtd", do_mtd_io, CONSOLE_CMD_MODE_SELF, "<name> <addr> [<off> [<size>]]")
CONSOLE_CMD(write_dontskipff, "mtd", do_mtd_io, CONSOLE_CMD_MODE_SELF, "<name> <addr> [<off> [<size>]]")
CONSOLE_CMD(erase, "mtd", do_mtd_erase, CONSOLE_CMD_MODE_SELF, "<name>        [<off> [<size>]]")
CONSOLE_CMD(bad, "mtd", do_mtd_bad, CONSOLE_CMD_MODE_SELF, "<name>")
