...
static void *alac_decoder_init(int bits, int channels, int samplerate)
{
...
static int alac_decode(void *phandle, uint8_t *audio_frame, size_t packet_size)
{
...
static void alac_decoder_flush(void *phandle)
{
    struct alac_decoder *p = (struct alac_decoder *)phandle;
    ioctl(p->fd, AUDDEC_FLUSH, 0);
}

static void alac_decoder_destroy(void *phandle)
{
    struct alac_decoder *p = (struct alac_decoder *)phandle;
    if( !p )
        return;
...
static int alac_test (int argc, char *argv[])
{
	(void)argc;
	(void *)argv;
