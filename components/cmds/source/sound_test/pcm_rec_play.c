...
static int pcm_rec_play_test(int argc, char *argv[])
{
    int ret;
...
static int stop_pcm_rec_play_test(int argc, char *argv[])
{
	stop_pcmi = 1;
	return 0;
}
...
CONSOLE_CMD(pcm_rec_play, "snd", pcm_rec_play_test, CONSOLE_CMD_MODE_SELF, "do pcm rec play test")
CONSOLE_CMD(stop_pcm_rec_play, "snd", stop_pcm_rec_play_test, CONSOLE_CMD_MODE_SELF, "stop pcm rec play test")
