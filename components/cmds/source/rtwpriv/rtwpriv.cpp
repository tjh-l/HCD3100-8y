...
int rtwpriv_main(int argc, char *argv[])
{
	...
	if (argc < 7) {
		if (argc > 2) {
			...
			if (argc > 1) {
				if ((strncmp(argv[1], "-v", 2) == 0) || (strncmp(argv[1], "v", 1) == 0))
					...
					RTW_FPRINT(stderr, "Interface doesn't accept private ioctl...parameter Error\n");
					if (argv[2]!=NULL)
						RTW_FPRINT(stderr, "%s: %s\n", argv[2], rtw_strerror(errno));
					return -EINVAL;
				}
			} else
				rtw_help();
		...
		if (inCH > 177 || inCH < 0) {
			//rtw_help();
			RTW_FPRINT(stderr, "Interface doesn't accept private ioctl...parameter Error, channel \n");
			return -EINVAL;
		}
		if (argc > 3)
			inBW  = atoi(argv[3]);
		if (inBW > 3 || inBW <0) {
			rtw_help();
			RTW_FPRINT(stderr, "Interface doesn't accept private ioctl...parameter Error, bandwidth\n");
			return -EINVAL;
		}
		if (argc > 4)
		...
		if (argc > 8) {
			if ((strncmp(argv[8], "ppdu", 4) == 0)) {
				...
				if (pktlen <= 100) {
					RTW_FPRINT(stderr, "Interface doesn't accept private ioctl... pktlen <= 100 parameter Error\n");
					return -EINVAL;
				}
			}
		}
