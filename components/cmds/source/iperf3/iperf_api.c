...
iperf_print_results(struct iperf_test *test)
{
    ...
    end_time = iperf_time_in_secs(&temp_time);
        ...
        SLIST_FOREACH(sp, &test->streams, streams) {
            //printf("-----%s[%d]%d-%d----------\n", __FUNCTION__, __LINE__, sp->sender, stream_must_be_sender);
            if (sp->sender == stream_must_be_sender) {
                if (test->json_output) {
...
#else
                                iperf_printf(test, report_bw_format, 
                                    sp->socket, mbuf, start_time, sender_time, ubuf, nbuf, report_sender);
#endif									
                            }
                    }
                } else {
                    ...
                    if (test->json_output) {
                        ...
                        if (test->role == 's' && !sp->sender) {
                            ...
                            iperf_printf(test, report_bw_udp_format, 
                            			sp->socket, mbuf, start_time, sender_time, ubuf, nbuf, 0, 0, (sender_packet_count - sp->omitted_packet_count), 0, report_sender);
#endif
                        }
...
#else
                            iperf_printf(test, report_bw_udp_format, 
                            			sp->socket, mbuf, start_time, receiver_time, ubuf, nbuf, (int)(sp->jitter * 1000.0), (sp->cnt_error - sp->omitted_cnt_error), (receiver_packet_count - sp->omitted_packet_count), (int)lost_percent, report_receiver);
#endif
                        }
                    }
                }
	...
	if (sp->sender) {
		...
		lost_percent = 0.0;
			...
			cJSON_AddItemToArray(json_interval_streams, iperf_json_printf("socket: %d  start: %f  end: %f  seconds: %f  bytes: %d  bits_per_second: %f  jitter_ms: %f  lost_packets: %d  packets: %d  lost_percent: %f  omitted: %b sender: %b", (int64_t) sp->socket, (double) st, (double) et, (double) irp->interval_duration, (int64_t) irp->bytes_transferred, bandwidth * 8, (double) irp->jitter * 1000.0, (int64_t) irp->interval_cnt_error, (int64_t) irp->interval_packet_count, (double) lost_percent, irp->omitted, sp->sender));
				...
				iperf_printf(test, report_bw_udp_format, sp->socket, mbuf, st, et, ubuf, nbuf, (int)(irp->jitter * 1000.0), irp->interval_cnt_error, irp->interval_packet_count, (int)lost_percent, irp->omitted?report_omitted:"");
		    }
		}
    }
...
#else
        if (test->settings->blksize > 16*1024) {
            test->settings->blksize = 16*1024; //8K
