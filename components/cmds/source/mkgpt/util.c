...
static int block_device_size(struct image *image, const char *blkdev, unsigned long long *size)
{
	...
	rc = open_blockdriver(blkdev, MS_RDONLY, &blkdrv);
	if (rc < 0) {
		ret = -errno;
		goto out;
	}
	...
	*size = (unsigned long long)geo.geo_nsectors * (unsigned long long)geo.geo_sectorsize;
	ret = 0;
...
static uint32_t __crc32(const void *data, size_t len)
{
	return nuttx_crc32part(data, len, ~0l) ^ ~0l;
}
