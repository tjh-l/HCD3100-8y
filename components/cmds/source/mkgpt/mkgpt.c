...
static struct gpt_partition_entry table[GPT_ENTRIES];
static int hdimage_insert_gpt(struct image *image, struct list_head *partitions)
{
	...
	header.table_crc = htole32(__crc32(table, sizeof(table)));

	header.header_crc = htole32(__crc32(&header, sizeof(header)));
...
static int mkgpt(int argc, char *argv[])
{
	...
	while (1) {
		...
		c = getopt_long(argc, argv, "d:s:p:h", lopts, NULL);
		...
		case 'p':
			...
			printf("part size %llx name %s, autoresize %d\r\n", part->size, part->name, part->autoresize);
			break;
		case 's':
			image->size = strtoll(optarg, &p, 0);
			unit = *p;
			if (unit == 'm' || unit == 'M') {
				image->size *= 0x100000;
			} else if (unit == 'k' || unit == 'K') {
	...
	if (is_block_device(imageoutfile(image))) {
		rc = open_blockdriver(imageoutfile(image), 0, &blkdrv);
		if (rc < 0)
			goto error_ret;
		blkdrv->u.i_bops->write(blkdrv, image->outbuf, 0, (image->outbuf_size + 511) / 512);
		...
		if (fd < 0)
			goto error_ret;
		lseek(fd, 0, SEEK_SET);
		write(fd, image->outbuf, image->outbuf_size);
		close(fd);
	}
...
CONSOLE_CMD(mkgpt, NULL, mkgpt, CONSOLE_CMD_MODE_SELF, "mkgpt command")
