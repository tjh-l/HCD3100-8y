...
void cb(uvc_frame_t *frame, void *ptr)
{
	...
	static const char *MJPEG_FILE = ".jpg";
	char filename[32];
	...
	printf("callback! frame_format = %d, width = %ld, height = %ld, length = %lu, ptr = %p\n", frame->frame_format,
	       frame->width, frame->height, frame->data_bytes, ptr);
	...
	case UVC_COLOR_FORMAT_MJPEG:
		sprintf(filename, "%s/%3.3d%s", path_save, jpeg_count++, MJPEG_FILE);
		fp = fopen(filename, "w");
		if (fp) {
			fwrite(frame->data, 1, frame->data_bytes, fp);
			fclose(fp);
...
int libuvc_demo(int argc, char **argv)
{
	uvc_context_t *ctx;
	uvc_device_t *dev;
...
CONSOLE_CMD(uvc_demo, NULL, libuvc_demo, CONSOLE_CMD_MODE_SELF, "libuvc example 1")
