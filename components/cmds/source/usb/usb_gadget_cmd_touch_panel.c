...
#define REPORT_BUF_LEN 54
#define INPUT_EVENT_NUM 256
#define TP_REPORT_ID 0x91

static bool g_force_exit = false;

struct tp_contact_st {
    union { /* byte 0 */
        struct {
    ...
    int8_t contact_identifier; /* byte 1 */
    uint16_t x; /* byte 2 & 3 */
    uint16_t y; /* byte 4 & 5 */
    uint16_t width; /* byte 6 & 7 */
    uint16_t height; /* byte 8 & 9 */
};

struct tp_report_st {
    uint8_t report_id;
    struct tp_contact_st contact[CONTACT_TOTAL_CNT];
    uint8_t contact_count;
    uint16_t scan_time; // unit 100us
};
...
struct contact_ctl {
    ...
    uint32_t rtos_time; // time stamp
    uint32_t last_time; // time stamp
    int8_t cur_slot; // current valid contact slot
};

static void copy_to_buffer(struct tp_report_st *report, uint8_t *buffer)
{
    size_t offset = 0;

    // Copy report_id
    memcpy(buffer + offset, &(report->report_id),
           sizeof(report->report_id));
    offset += sizeof(report->report_id);

    // Copy each tp_contact_st
    for (int i = 0; i < 5; i++) {
        memcpy(buffer + offset, &(report->contact[i]),
               sizeof(struct tp_contact_st));
        offset += sizeof(struct tp_contact_st);
    }

    // Copy contact_count
    memcpy(buffer + offset, &(report->contact_count),
           sizeof(report->contact_count));
    offset += sizeof(report->contact_count);

    // Copy scan_time
    memcpy(buffer + offset, &(report->scan_time),
           sizeof(report->scan_time));
    offset += sizeof(report->scan_time);
}
...
static void dump_hex(uint8_t *data, size_t size)
{
    ...
    printf("%02x \n", data[i++]); // report ID
    for (loop = 0; loop < 5; loop++) { // contact info
        for (j = 0; j < 10; j++)
            printf("%02x ", data[i++]);
        printf("\n");
    }
    for (j = 0; j < 3; j++)
        printf("%02x ", data[i++]); // contact number + time stamp
    printf("\n");
}

static void dump_contact_report(struct tp_report_st *p_report)
{
    int i;
    printf("-------------------------------------------\n");
    printf("reportID: 0x%x\n", p_report->report_id);
    for (i = 0; i < CONTACT_TOTAL_CNT; i++) {
        if (p_report->contact[i].touch_valid == 0)
            continue;
        printf("[%d] id:%d, 0x%x, 0x%x, x:%u, y:%u, w:0x%x, h:0x%x\n",
               i, p_report->contact[i].contact_identifier,
               p_report->contact[i].tip_switch,
               p_report->contact[i].touch_valid, p_report->contact[i].x,
               p_report->contact[i].y, p_report->contact[i].width,
               p_report->contact[i].height);
    }
    printf("cnt: %u\n", p_report->contact_count);
    printf("time: 0x%x\n", p_report->scan_time);
    printf("-------------------------------------------\n");
}
                  ...
                  struct input_event *events, int event_cnt)
{
    ...
    if (is_none_valid_contact(p_contact))
        p_contact->rtos_time = 0;
...
__update_exit:
    ...
    /* update time stamp */
    if (p_contact->rtos_time == 0)
        p_contact->rtos_time += 130;
    else
        p_contact->rtos_time +=
            (get_time_in_us() - p_contact->last_time);
    p_contact->last_time = get_time_in_us();
...
static int fill_hid_report(struct contact_ctl *p_contact,
               struct tp_report_st *report)
{
    int i;
    struct contact_handle *p_handle;

    memset(report, 0, sizeof(struct tp_report_st));

    report->report_id = TP_REPORT_ID;
    for (i = 0; i < CONTACT_TOTAL_CNT; i++) {
        p_handle = &p_contact->contact_pool[i];
        report->contact[i].tip_switch = p_handle->down;
        report->contact[i].touch_valid =
            (p_handle->slot >= 0) ? 0x1 : 0x0;
        report->contact[i].contact_identifier = i;
        report->contact[i].x = (uint16_t)(p_handle->x * 1);
        report->contact[i].y = (uint16_t)(p_handle->y * 1);
        report->contact[i].width = 0x1;
        report->contact[i].height = 0x1;
    }
    report->contact_count = CONTACT_TOTAL_CNT;
    report->scan_time = p_contact->rtos_time;
    return 0;
}
...
static void *tp_thread(void *args)
{
    ...
    struct tp_report_st report;
    uint8_t report_buf[REPORT_BUF_LEN];
    int to_send;
    ...
    while (1) {
        ...
        */
        ...
        ret = fill_hid_report(p_contact, &report);
        if (ret == -1)
            break;
        cleanup_contact_status(p_contact);
        // dump_contact_report(&report);

        copy_to_buffer(&report, report_buf);
        // dump_hex(report_buf, REPORT_BUF_LEN);

        /*
         * send usb report buffer
         */
        if (write(fdg, (uint8_t *)&report_buf, REPORT_BUF_LEN) !=
            REPORT_BUF_LEN) {
            log_e("Write %s error\n", hidg_name);
            ret = -1;
            goto __tp_exit;
...
int hid_gadget_tp_demo(int argc, char **argv)
{
...
CONSOLE_CMD(usbg_tp, NULL, hid_gadget_tp_demo, CONSOLE_CMD_MODE_SELF,
        "usb gadget touch panel")
