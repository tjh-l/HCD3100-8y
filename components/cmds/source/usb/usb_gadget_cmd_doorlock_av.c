...
static struct __uac_demo_st {
    /* mircophone */
    uint8_t *buf_mic;
    uint8_t *buf_mic_mono;
    struct pcm_decoder *i2si_hdl;

    /* speaker */
    xQueueHandle xQueue;
    uint8_t *buf_speaker;
    struct pcm_decoder *i2so_hdl;

} g_uac_demo;

static struct pcm_decoder *i2so_pcm_decoder_init(int bits, int channels, int samplerate)
{
	struct pcm_decoder *p = (struct pcm_decoder *)malloc(sizeof(struct pcm_decoder));
    int fd, ret;

    memset(p, 0, sizeof(struct pcm_decoder));

    p->fd = -1;
	p->params.access = SND_PCM_ACCESS_RW_INTERLEAVED;
	p->params.format = SND_PCM_FORMAT_S16_LE;
	p->params.sync_mode = AVSYNC_TYPE_FREERUN;
	p->params.align = SND_PCM_ALIGN_LEFT;
	p->params.rate = samplerate;
	p->params.channels = channels;
	p->params.period_size = channels * (bits / 8) * 160;
	p->params.periods = 8;
	p->params.bitdepth = bits;
	p->params.start_threshold = 4; //2;

    fd = open("/dev/sndC0i2so", O_WRONLY);
    if(fd < 0) {
        printf("[Error] Cannot open /dev/sndC0i2so\n");
        return NULL;
    }

	printf("speaker: channels:%d, sampleRate:%d, bitsPerSample:%d\n", 
			p->params.channels, p->params.rate, p->params.bitdepth);    

	ret = ioctl(fd, SND_IOCTL_HW_PARAMS, &p->params);
	if (ret < 0) {
    ...
    memset(&p->pfd, 0, sizeof(struct pollfd));
	p->pfd.fd = fd;
	p->pfd.events = POLLOUT | POLLWRNORM;
    p->fd = fd;
    p->is_on = false;

    printf("[demo] i2so pcm decode init finish (fd:%d)\n", p->fd);

	return p;
}

static struct pcm_decoder *i2si_pcm_decoder_init(int bits, int channels, int samplerate)
{
	struct pcm_decoder *p = (struct pcm_decoder *)malloc(sizeof(struct pcm_decoder));
    int fd, ret;
	...
	p->params.format = SND_PCM_FORMAT_S16_LE;
	p->params.sync_mode = AVSYNC_TYPE_FREERUN;
	p->params.align = SND_PCM_ALIGN_RIGHT;
	p->params.rate = samplerate;
	p->params.channels = channels;
	p->params.period_size = 1536; // channels * (bits / 8) * 160;
	p->params.periods = 16;
	p->params.bitdepth = bits;
	p->params.start_threshold = 1;
	p->params.pcm_source = SND_PCM_SOURCE_AUDPAD;
	p->params.pcm_dest = SND_PCM_DEST_DMA;

    fd = open("/dev/sndC0i2si", O_WRONLY);
    if(fd < 0) {
        printf("[Error] Cannot open /dev/sndC0i2si\n");
        return NULL;
    }

	printf(" ==> rec: period_size = %ld\n", p->params.period_size);
	printf(" ==> rec: periods = %ld\n", p->params.periods);
	printf(" ==> rec: start_threshold = %d\n", p->params.start_threshold);
	printf(" ==> rec: bitdepth = %d\n", p->params.bitdepth);
	printf(" ==> rec: rate = %d\n", p->params.rate);
	printf(" ==> rec: channels = %d\n", p->params.channels);

	printf("mic: channels:%d, sampleRate:%d, bitsPerSample:%d\n", 
			p->params.channels, p->params.rate, p->params.bitdepth);        
    ...
    memset(&p->pfd, 0, sizeof(struct pollfd));
	p->pfd.fd = fd;
	p->pfd.events = POLLIN | POLLRDNORM;
    p->fd = fd;
    p->is_on = false;

    printf("[demo] i2si pcm decode init finish (fd:%d)\n", p->fd);

	return p;
}

static int i2so_pcm_decode(struct pcm_decoder *phandle, uint8_t *audio_frame, int packet_size)
{
	struct pcm_decoder *p = phandle;
    int ret;
    ...
    if(p->is_on == false) {
        ret = ioctl(p->fd, SND_IOCTL_START, 0);
        if (ret < 0) {
            printf("snd (i2so) start failed\n");
            return -1;
        } else
            printf("snd (i2so) start success\n");
        p->is_on = true;
    }

    do {
        struct snd_xfer xfer = {0};
        xfer.data = audio_frame;
        xfer.frames = (packet_size / p->params.channels) >> 1;
        xfer.tstamp_ms = 0;

        if(packet_size > 1024) {
            printf(" [demo] warning -- i2so xfer, packet size %d, fd %d\n", 
                    packet_size, p->fd);
            break;
        }

        ret = ioctl(p->fd, SND_IOCTL_XFER, &xfer);
        if (ret < 0) {
            printf("--> i2so error (%d)\n", ret);
            poll(&p->pfd, 1, 100);
        }
...
static int i2si_pcm_decode(struct pcm_decoder *phandle, uint8_t *audio_frame, int packet_size)
{
	struct pcm_decoder *p = phandle;
    int ret;
    ...
    if(p->is_on == false) {
        ret = ioctl(p->fd, SND_IOCTL_START, 0);
        if (ret < 0) {
            printf("snd (i2si) start failed\n");
            return -1;
        } else
            printf("snd (i2si) start success\n");
        p->is_on = true;
    }
...
static void __mic_process(uint16_t *src, uint16_t *dest, int bytes)
{
    while(bytes--) {
        src++;
        *dest++ = *src++;
...
// static void __mic_backup(uint8_t *stereo, uint8_t *mono, int bytes)
// {
//     int fd_stereo = -1;
//     int fd_mono = -1;

//     fd_stereo = open("/media/sda1/ttoonnyy_strero.pcm", O_RDWR | O_CREAT | O_APPEND);
//     fd_mono = open("/media/sda1/ttoonnyy_mono.pcm", O_RDWR | O_CREAT | O_APPEND);

//     if(fd_stereo > 0) {
//         write(fd_stereo, stereo, bytes);
//         close(fd_stereo);
//     }
//     if(fd_mono > 0) {
//         write(fd_mono, mono, bytes / 2);
//         close(fd_mono);        
//     }
// }

static int uac_mic_hook(uint8_t **frame, int *frame_sz)
{   
    int ret;
    struct pcm_decoder *hdl = g_uac_demo.i2si_hdl;
    struct snd_pcm_params *params = &g_uac_demo.i2si_hdl->params;
    int frames = params->period_size /
             (params->channels * params->bitdepth / 8);

    ret = i2si_pcm_decode(hdl, g_uac_demo.buf_mic, frames);
    if(ret)
        return -1;

    __mic_process((uint16_t *)g_uac_demo.buf_mic, 
                (uint16_t *)g_uac_demo.buf_mic_mono, 
                params->period_size);

    // __mic_backup(g_uac_demo.buf_mic, g_uac_demo.buf_mic_mono, 
    //             params->period_size);            

    // printf(" ==> mic: %ld, frames %d\n", params->period_size, frames);
   
    *frame = g_uac_demo.buf_mic_mono;
    *frame_sz = params->period_size / 2;
    return 0;
}
...
{
    struct pcm_decoder *hdl = g_uac_demo.i2so_hdl;
    ...
    for (;;) {
        ...
        // printf(" [uac] speaker -- %d\n", cnt);
        if(i2so_pcm_decode(hdl, (uint8_t *)buf, cnt))
...
static int hcusb_gadget_uac_env(void)
{
    int fd;
    xQueueHandle xQueue;
    struct pcm_decoder *hdl;

    hdl = i2so_pcm_decoder_init(16, 1, 8000);   // 16 bits, 1 channel, sample rate 8K
    if(hdl == NULL) {
        printf(" ==> cannot init pcm decoder\n");
        return -1;
    }
    g_uac_demo.i2so_hdl = hdl;

    // !note: 由于智能门锁板上的ADC的硬件特性原因，所以必须走双通道
    hdl = i2si_pcm_decoder_init(16, 2, 8000);   // 16 bits, 2 channel, sample rate 8K
    if(hdl == NULL) {
        printf(" ==> cannot init pcm decoder\n");
        return -1;
    }
    g_uac_demo.i2si_hdl = hdl;

    xQueue = xQueueCreate(4 * 1024, 4); // total size: 4K * 4 = 16KB
    if(!xQueue) {
    ...
    g_uac_demo.buf_mic = malloc(12 * 1024);  //TODO: just for debug
    g_uac_demo.buf_mic_mono = malloc(6 * 1024); //TODO: just for debug
    g_uac_demo.buf_speaker = malloc(64 * 1024); //TODO: just for debug
    ...
    xTaskCreate(uac_speaker_daemon , "uac speaker daemon" ,
                        0x1000 , NULL, portPRI_TASK_HIGH + 1 , NULL);
}
...
static int uvc_get_frame(uint8_t **frame, int *frame_sz)
{
...
static int hcusb_gadget_uvc_env(void)
{
...
// static void is2o_swith(bool is_on)
// {
//     struct pcm_decoder *p = g_uac_demo.i2so_hdl;
//     int ret;
...
//     }
//     else {
//         ioctl(p->fd, SND_IOCTL_DROP, 0);
//     }
// }

// static void is2i_disable(bool is_on)
// {
//     struct pcm_decoder *p = g_uac_demo.i2si_hdl;
//     int ret;
...
#if 0 // !note: 只是用于测试, 固定输出一个图像

#include "tt_jpg.h"
static int __uvc_get_frame(uint8_t **frame, int *frame_sz)
{
    *frame = &tt_jpg[0];
    *frame_sz = tt_jpg_len;
    msleep(20);
...
int door_lock_av_demo(int argc, char **argv)
{
    // elog_set_filter_tag_lvl("composite", ELOG_LVL_WARN);

	hcusb_gadget_uac_env();
    hcusb_gadget_uvc_env();

    hcusb_gadget_av_specified_init(get_udc_name(USB_PORT_1),
                                av_switch_hook,
                                uvc_get_frame, //__uvc_get_frame,
                                uac_mic_hook,
                                uac_speaker_hook);
...
CONSOLE_CMD(lock_av, NULL, door_lock_av_demo, CONSOLE_CMD_MODE_SELF,
        "door lock demo (support video and audio)")
