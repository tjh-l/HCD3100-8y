...
struct flash_type {
	...
	int8_t read_otp_cmd;		int8_t read_otp_dummy_num;		uint32_t otp_addr;
	int8_t enter_otp_mode_cmd;	int8_t exit_otp_mode_cmd;
	int8_t write_otp_cmd;
	int8_t erase_otp_cmd;
};
...
int hc_norflash_write_otp_data(uint8_t cmd, uint32_t addr, uint8_t *data,
			       uint32_t len)
{
	int ret, fd;
	uint8_t tx[4 + len];
...
int hc_norflash_erase_otp_data(uint8_t cmd, uint32_t addr)
{
	int ret, fd;
	uint8_t tx[10] = {0};
	tx[0] = cmd;
...
int hc_norflash_erase_OTP(char flash_name[])
{
	uint8_t i;
	int number = 0;

        if(flash_name == NULL){
                printf("err: flash_name is null, unkown jedec_id, jedec_id=%ld\n", jedec_id);
                return -1;
        }
	...
	if (FLASH_TYPE[number].enter_otp_mode_cmd != NONE)
		hc_norflash_send_one_cmd(FLASH_TYPE[number].enter_otp_mode_cmd);

	hc_norflash_erase_otp_data(FLASH_TYPE[number].erase_otp_cmd,
				   FLASH_TYPE[number].otp_addr);

	if (FLASH_TYPE[number].exit_otp_mode_cmd != NONE)
		hc_norflash_send_one_cmd(FLASH_TYPE[number].exit_otp_mode_cmd);

	return 0;
}

int hc_norflash_lock_OTP(char flash_name[])
{
	int number = 0;
	...
	hc_norflash_read_register(0x35, &register_status);

	switch (FLASH_TYPE[number].otp_addr) {
	case 0x1000: {
		lock_otp_area = 0x08;
		printf("\nThe Security Registers 1 will be lock forever!!!\n");
		break;
	}
	case 0x2000: {
		lock_otp_area = 0x10;
		printf("\nThe Security Registers 2 will be lock forever!!!\n");
		break;
	}
	case 0x3000: {
		lock_otp_area = 0x20;
		printf("\nThe Security Registers 3 will be lock forever!!!\n");
		break;
	}
	}

	hc_norflash_send_one_cmd(0x06);
	hc_norflash_read_register(0x05, &register_status);

	register_status = register_status | lock_otp_area;
	hc_norflash_write_register(0x31, &register_status);

	hc_norflash_read_register(0x05, &register_status);
	while ((register_status >> 0x0) & 0x01) {
		hc_norflash_read_register(0x05, &register_status);
	}
	if (FLASH_TYPE[number].exit_otp_mode_cmd != NONE)
		hc_norflash_send_one_cmd(FLASH_TYPE[number].exit_otp_mode_cmd);
...
int hc_norflash_write_OTP(char flash_name[], uint8_t *data, uint32_t len)
{
	uint8_t i;
	int number = 0;
	...
	hc_norflash_write_otp_data(FLASH_TYPE[number].write_otp_cmd,
				   FLASH_TYPE[number].otp_addr,
				   data, len);
...
int nor_flash_read_OTP(char flash_name[], uint8_t *otp_data, uint32_t data_len)
{
	uint32_t i;
	int number = 0;
...
int nor_flash_write_otp_example(int argc, char **argv)
{
	uint8_t register_status = 0;
	uint8_t flash_otp_wdata[256] = { 0 };
	uint8_t flash_otp_rdata[256] = { 0 };

	/* otp data to write */
	memset(flash_otp_wdata, 0xcc, sizeof(flash_otp_wdata));
	...
	/* erase */
	hc_norflash_erase_OTP(flash_name);
	/* wait erase finish */
	hc_norflash_read_register(0x05, &register_status);
	while ((register_status >> 0x0) & 0x01) {
		hc_norflash_read_register(0x05, &register_status);
	...
	/* write */
	hc_norflash_write_OTP(flash_name, flash_otp_wdata, sizeof(flash_otp_wdata));
	/* wait write finish */
	hc_norflash_read_register(0x05, &register_status);
	while ((register_status >> 0x0) & 0x01) {
		hc_norflash_read_register(0x05, &register_status);
	...
	/* read otp data */
	nor_flash_read_OTP(flash_name, flash_otp_rdata, sizeof(flash_otp_rdata));

	/* func: nor_flash_lock_OTP
	 * warnning !!! if this function run, the otp area can`t be write forever
	 */
	//hc_norflash_lock_OTP(flash_name);
...
int read_nor_flash_OTP_data_test(int argc, char **argv)
{
	...
	nor_flash_read_OTP(flash_name, flash_otp_data, sizeof(flash_otp_data));
	return 0;
}
...
CONSOLE_CMD(write_otp_example, "spi", nor_flash_write_otp_example,
		CONSOLE_CMD_MODE_SELF, "write  nor flash otp")
