...
ifneq ("$(origin SOLUTION)", "command line")
SOLUTION?=./
else
override SOLUTION:=$(SOLUTION)/
MAKEOVERRIDES :=
endif

ifneq ("$(origin O)", "command line")
O?=output
CONFIG_DIR:=$(SOLUTION)$(O)
else
override O:=$(O)
...
export CDPATH:=
BASE_DIR := $(shell mkdir -p $(SOLUTION)$(O) && cd $(SOLUTION)$(O) >/dev/null && pwd)
$(if $(BASE_DIR),, $(error output directory "$(SOLUTION)$(O)" does not exist))
...
ifeq ($(filter /%,$(SOLUTION)output),$(SOLUTION)output)
IMAGES_DIR := $(SOLUTION)output/images
DATA_DIR := $(SOLUTION)output/images/user_data
else
IMAGES_DIR := $(TOPDIR)/$(SOLUTION)output/images
DATA_DIR := $(TOPDIR)/$(SOLUTION)output/images/user_data
endif
...
menuconfig: $(BUILD_DIR)/buildroot-config/mconf outputmakefile
	...
	make SOLUTION=$(SOLUTION) O=$(O) syncconfig

oldconfig syncconfig: $(BUILD_DIR)/buildroot-config/conf outputmakefile
...
%_defconfig: $(BUILD_DIR)/buildroot-config/conf configs/%_defconfig outputmakefile
...
defconfig-update:
	...
	for D in $(shell ls configs/*defconfig); do \
	  ...
	  make SOLUTION=$(SOLUTION) O=$(O) `basename $${D}` && cp -vf $(CONFIG_DIR)/.config $${D}; \
	done; \
	)

%defconfig-update:
	@$(call MESSAGE, "updating $(@:%-update=%)")
	make SOLUTION=$(SOLUTION) O=$(O) $(@:%-update=%)
...
lastdefconfig: $(BUILD_DIR)/buildroot-config/conf
	...
	@echo -e "#\n# Using $(LASTDEFCONFIG)\n#"
	make SOLUTION=$(SOLUTION) O=$(O) $(LASTDEFCONFIG)
...
outputmakefile:
	$(Q)$(TOPDIR)/build/scripts/mkmakefile2 $(TOPDIR) $(O) $(SOLUTION)
